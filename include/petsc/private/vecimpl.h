...
#define VecCheckSameLocalSize(x,ar1,y,ar2) do { \
    if ((x)->map->n != (y)->map->n) SETERRQ4(PETSC_COMM_SELF,PETSC_ERR_ARG_INCOMP,"Incompatible vector local lengths parameter # %d local size %" PetscInt_FMT " != parameter # %d local size %" PetscInt_FMT "", ar1,(x)->map->n, ar2,(y)->map->n); \
  } while (0)

#define VecCheckSameSize(x,ar1,y,ar2) do { \
    if ((x)->map->N != (y)->map->N) SETERRQ4(PetscObjectComm((PetscObject)x),PETSC_ERR_ARG_INCOMP,"Incompatible vector global lengths parameter # %d global size %" PetscInt_FMT " != parameter # %d global size %" PetscInt_FMT "", ar1,(x)->map->N, ar2,(y)->map->N); \
    VecCheckSameLocalSize(x,ar1,y,ar2); \
  } while (0)

#define VecCheckLocalSize(x,ar1,n) do { \
    if ((x)->map->n != n) SETERRQ3(PETSC_COMM_SELF,PETSC_ERR_ARG_INCOMP,"Incorrect vector local size: parameter # %d local size %" PetscInt_FMT " != %" PetscInt_FMT "",ar1,(x)->map->n,n); \
  } while (0)

#define VecCheckSize(x,ar1,n,N) do { \
    if ((x)->map->N != N) SETERRQ3(PetscObjectComm((PetscObject)x),PETSC_ERR_ARG_INCOMP,"Incorrect vector global size: parameter # %d global size %" PetscInt_FMT " != %" PetscInt_FMT "",ar1,(x)->map->N, N); \
