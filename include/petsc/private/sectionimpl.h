...
#  define PetscSectionCheckValid_(description,item,nitems) do {         \
    if (PetscUnlikely(((item) < 0) || ((item) >= (nitems)))) {          \
      SETERRQ2(PETSC_COMM_SELF, PETSC_ERR_ARG_OUTOFRANGE,               \
               "Invalid " description " %" PetscInt_FMT "; not in "     \
               " [0, %" PetscInt_FMT ")", (item), (nitems));            \
    }                                                                   \
  } while (0)

#  define PetscSectionCheckValidFieldComponent(comp,nfieldcomp) \
  PetscSectionCheckValid_("section field component",comp,nfieldcomp)

#  define PetscSectionCheckValidField(field,nfields)    \
  PetscSectionCheckValid_("field number",field,nfields)
