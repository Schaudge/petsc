...
PETSC_STATIC_INLINE PetscErrorCode PCGAMGHashTableFind(PCGAMGHashTable *a_tab, PetscInt a_key, PetscInt *a_data)
{
  ...
  PetscFunctionBegin;
  if (PetscUnlikely(a_key<0)) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_USER,"Negative key %" PetscInt_FMT ".",a_key);
  ...
  if (PetscUnlikely(kk==a_tab->size)) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_USER,"key %" PetscInt_FMT " not found in table",a_key);
