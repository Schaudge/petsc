...
*/
  ...
  #define PetscStackPop_Private(stack__, func__) \
    do { \
      ...
      if (--stack__.currentsize < PETSCSTACKSIZE) { \
        PetscCheckAbort(!stack__.check || stack__.petscroutine[stack__.currentsize] != 1 || (void *)stack__.function[stack__.currentsize] == (void *)(func__), PETSC_COMM_SELF, PETSC_ERR_PLIB, "Invalid stack: push from %s %s:%d. Pop from %s %s:%d.\n", \
