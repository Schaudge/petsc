...
#define PetscLogStateStageEventActive(state, stage, event) ((state) && PetscBTLookup((state)->active, (stage)) && PetscBTLookup((state)->active, (stage) + (event + 1) * (state)->bt_num_stages))
