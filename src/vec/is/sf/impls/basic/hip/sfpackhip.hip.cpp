...
static PetscErrorCode Pack(PetscInt count,const PetscInt *idx,PetscSFPack link,PetscSFPackOpt opt,const void *unpacked,void *packed)
{
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(d_Pack<Type,BS,EQ>), dim3(nblocks), dim3(nthreads), 0, link->stream, count,idx,link->bs,unpacked,packed);
...
static PetscErrorCode UnpackAndOp(PetscInt count,const PetscInt *idx,PetscSFPack link,PetscSFPackOpt opt,void *unpacked,const void *packed)
{
  ...
  PetscFunctionBegin;
  nblocks = PetscMin(nblocks,link->maxResidentThreadsPerGPU/nthreads);
  hipLaunchKernelGGL(HIP_KERNEL_NAME(d_UnpackAndOp<Type,Op,BS,EQ>), dim3(nblocks), dim3(nthreads), 0, link->stream, count,idx,link->bs,unpacked,packed);
  err = hipGetLastError();CHKERRCUDA(err);
  PetscFunctionReturn(0);
}
...
static PetscErrorCode FetchAndOp(PetscInt count,const PetscInt *idx,PetscSFPack link,PetscSFPackOpt opt,void *unpacked,void *packed)
{
  ...
  PetscFunctionBegin;
  nblocks = PetscMin(nblocks,link->maxResidentThreadsPerGPU/nthreads);
  hipLaunchKernelGGL(HIP_KERNEL_NAME(d_FetchAndOp<Type,Op,BS,EQ>), dim3(nblocks), dim3(nthreads), 0, link->stream, count,idx,link->bs,unpacked,packed);
  err = hipGetLastError();CHKERRCUDA(err);
  PetscFunctionReturn(0);
}
