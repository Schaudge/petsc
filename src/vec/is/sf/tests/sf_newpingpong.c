...
int main(int argc,char **argv)
{
  ...
  for (n=1,j=0; n<=maxn; n*=2,j++) {
    ...
    for (i=0; i<niter + nskip; i++) {
      ...
      ierr = PetscSFBcastAndOpBegin(sf[j],MPIU_SCALAR,rootdata,leafdata,MPIU_REPLACE);CHKERRQ(ierr); /* rank 0->1, root->leaf*/
      ierr = PetscSFBcastAndOpEnd(sf[j],MPIU_SCALAR,rootdata,leafdata,MPIU_REPLACE);CHKERRQ(ierr);
      ierr = PetscSFReduceBegin(sf[j],MPIU_SCALAR,leafdata,rootdata,MPIU_REPLACE);CHKERRQ(ierr); /* rank 1->0, leaf->root */
      ierr = PetscSFReduceEnd(sf[j],MPIU_SCALAR,leafdata,rootdata,MPIU_REPLACE);CHKERRQ(ierr);
    }
    cerr    = cudaDeviceSynchronize();CHKERRCUDA(cerr);
    ierr    = MPI_Barrier(PETSC_COMM_WORLD);CHKERRQ(ierr);
    t_end   = MPI_Wtime();
