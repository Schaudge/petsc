static const char help[] = "PetscSF Scatter test\n\n";
...
int main(int argc,char **argv)
{
  ...
  char           mstring[16]={0};
  PetscBool      flg,set;
  PetscInt       nprof=4096; /* Note nprof*8 is the message size */
  ...
  for (n=1,j=0; n<=maxn; n*=2,j++) {
    ...
    for (i=0; i<niter + nskip; i++) {
      ...
      ierr = PetscSFBcastAndOpBegin(sf[j],MPIU_SCALAR,rootdata,leafdata,MPIU_SUM);CHKERRQ(ierr); /* rank 0->1, root->leaf*/
      ierr = PetscSFBcastAndOpEnd(sf[j],MPIU_SCALAR,rootdata,leafdata,MPIU_SUM);CHKERRQ(ierr);
      ierr = PetscSFReduceBegin(sf[j],MPIU_SCALAR,leafdata,rootdata,MPIU_SUM);CHKERRQ(ierr); /* rank 1->0, leaf->root */
      ierr = PetscSFReduceEnd(sf[j],MPIU_SCALAR,leafdata,rootdata,MPIU_SUM);CHKERRQ(ierr);
    }
    cerr    = cudaDeviceSynchronize();CHKERRCUDA(cerr);
    ierr    = MPI_Barrier(PETSC_COMM_WORLD);CHKERRQ(ierr);
    t_end   = MPI_Wtime();
