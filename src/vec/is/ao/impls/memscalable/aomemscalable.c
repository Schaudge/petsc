...
static PetscErrorCode AOMap_MemoryScalable_private(AO ao, PetscInt n, PetscInt *ia, const PetscInt *maploc)
{
  ...
  PetscMPIInt         *owner, *start, j;
  PetscInt           nmax,  *sindices, *rindices,  idx, lastidx, *sindices2, *rindices2, *sizes;
  PetscInt           nreceives, nsends;
  ...
  */
  for (PetscMPIInt i = 0, count = 0; i < (PetscMPIInt)nreceives; i++) PetscCallMPI(MPIU_Irecv(rindices + nmax * i, nmax, MPIU_INT, MPI_ANY_SOURCE, tag1, comm, recv_waits + count++));
  ...
  */
  start[0] = 0;
  for (PetscMPIInt i = 1; i < size; i++) PetscCall(PetscMPIIntCast(start[i - 1] + sizes[2 * i - 2],&start[i]));
  for (PetscInt i = 0; i < n; i++) {
    j = owner[i];
  ...
  PetscCheck(nsends == count, comm, PETSC_ERR_SUP, "nsends %" PetscInt_FMT " != count %d", nsends, count);
