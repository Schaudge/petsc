...
#include <petscviewer.h>
  ...
  #define veccreatewitharraysandmemtype_ VECCREATEWITHARRAYSANDMEMTYPE
...
PETSC_EXTERN void veccreatewitharraysandmemtype_(MPI_Comm *comm, PetscInt *bs, PetscInt *n, PetscInt *N, PetscScalar *harray, PetscMemType *mtype, PetscScalar *darray, Vec *V, PetscErrorCode *ierr)
{
  CHKFORTRANNULLSCALAR(harray);
  CHKFORTRANNULLSCALAR(darray);
  *ierr = VecCreateWithArraysAndMemType(MPI_Comm_f2c(*(MPI_Fint *)&*comm), *bs, *n, *N, harray, *mtype, darray, V);
}
