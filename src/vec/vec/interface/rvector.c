...
PetscErrorCode VecGetArray(Vec x,PetscScalar **a)
{
  ...
  if (x->petscnative) {
    ...
    if (x->offloadmask == PETSC_OFFLOAD_GPU) {
...
#endif
#if defined(PETSC_HAVE_LIBAXB)
      ierr = PetscObjectTypeCompareAny((PetscObject)x,&is_axb,VECSEQAXB,VECMPIAXB,VECAXB,"");CHKERRQ(ierr);
      if (is_axb) {
        ierr = VecAXBCopyFromGPU(x);CHKERRQ(ierr);
      }
#endif
#if defined(PETSC_HAVE_CUDA)
      ...
      ierr = PetscObjectTypeCompareAny((PetscObject)x,&is_axb,VECSEQAXB,VECMPIAXB,VECAXB,"");CHKERRQ(ierr);
      if (is_axb) {
         ierr = VecAXBAllocateCheckHost(x);CHKERRQ(ierr);
      }      
#endif
#if defined(PETSC_HAVE_CUDA)
      ierr = PetscObjectTypeCompareAny((PetscObject)x,&is_cuda,VECSEQCUDA,VECMPICUDA,VECCUDA,"");CHKERRQ(ierr);
