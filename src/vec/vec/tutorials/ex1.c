...
int main(int argc,char **argv)
{
  ...
  */
  ...
  v    = norm*norm-4.0*(PetscReal)n; if (v > -PETSC_SMALL && v < PETSC_SMALL) v = 0.0;
  ...
  v    = norm*norm-64.0*(PetscReal)n; if (v > -PETSC_SMALL && v < PETSC_SMALL) v = 0.0;
  ierr = PetscPrintf(PETSC_COMM_WORLD,"VecAXPY %g\n",(double)v);CHKERRQ(ierr);

  ierr = VecAYPX(y,two,x);CHKERRQ(ierr);
  ierr = VecNorm(y,NORM_2,&norm);CHKERRQ(ierr);
  v    = norm*norm-18.0*18.0*(PetscReal)n; if (v > -PETSC_SMALL && v < PETSC_SMALL) v = 0.0;
  ...
  ierr = VecNorm(x,NORM_2,&norm);CHKERRQ(ierr);
  v =    norm*norm-18.0*(PetscReal)n; if (v > -PETSC_SMALL && v < PETSC_SMALL) v = 0.0;
  ierr = PetscPrintf(PETSC_COMM_WORLD,"VecSwap  %g\n",(double)v);CHKERRQ(ierr);

  ierr = VecWAXPY(w,two,x,y);CHKERRQ(ierr);
  ierr = VecNorm(w,NORM_2,&norm);CHKERRQ(ierr);
  v    = norm*norm-38.0*38.0*(PetscReal)n; if (v > -PETSC_SMALL && v < PETSC_SMALL) v = 0.0;
  ierr = PetscPrintf(PETSC_COMM_WORLD,"VecWAXPY %g\n",(double)v);CHKERRQ(ierr);

  ierr = VecPointwiseMult(w,y,x);CHKERRQ(ierr);
  ierr = VecNorm(w,NORM_2,&norm);CHKERRQ(ierr);
  v    = norm*norm-36.0*36.0*(PetscReal)n; if (v > -PETSC_SMALL && v < PETSC_SMALL) v = 0.0;
  ierr = PetscPrintf(PETSC_COMM_WORLD,"VecPointwiseMult %g\n",(double)v);CHKERRQ(ierr);

  ierr = VecPointwiseDivide(w,x,y);CHKERRQ(ierr);
  ierr = VecNorm(w,NORM_2,&norm);CHKERRQ(ierr);
  v    = norm*norm-9.0*9.0*(PetscReal)n; if (v > -PETSC_SMALL && v < PETSC_SMALL) v = 0.0;
  ...
  v    = norm*norm-(PetscReal)n; if (v > -PETSC_SMALL && v < PETSC_SMALL) v = 0.0;
  ierr = VecNorm(z[1],NORM_2,&norm);CHKERRQ(ierr);
  v1   = norm*norm-4*(PetscReal)n; if (v1 > -PETSC_SMALL && v1 < PETSC_SMALL) v1 = 0.0;
  ierr = VecNorm(z[2],NORM_2,&norm);CHKERRQ(ierr);
  v2   = norm*norm-9*(PetscReal)n; if (v2 > -PETSC_SMALL && v2 < PETSC_SMALL) v2 = 0.0;
