...
PetscBool   PetscHIPInitialized  = PETSC_FALSE;
...
PetscBool PetscHIPSynchronize = PETSC_FALSE;
...
#endif
#if defined(PETSC_HAVE_HIP)
  ierr = PetscOptionsBegin(comm,NULL,"HIP initialize","Sys");CHKERRQ(ierr);
  ierr = PetscOptionsBool("-hip_initialize","Initialize the HIP devices and cuBLAS during PetscInitialize()",NULL,initHip,&initHip,NULL);CHKERRQ(ierr);
  ierr = PetscOptionsEnd();CHKERRQ(ierr);
  if (initHip) {ierr = PetscHIPInitialize(PETSC_COMM_WORLD);CHKERRQ(ierr);}
#endif
