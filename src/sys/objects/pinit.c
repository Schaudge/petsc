...
static PetscMPIInt petsc_comm_world_size = 0;
static PetscBool   initSendHook = PETSC_FALSE, initRecvHook = PETSC_FALSE;
static HookInfo    sendHookInfo, recvHookInfo;

PETSC_STATIC_INLINE PetscErrorCode PetscMPICommHookCreate(HookInfo *ctx, PetscBool send, PetscBool *init)
{
  PetscFunctionBegin;
  PetscValidPointer(ctx,1);
  PetscValidBoolPointer(init,2);
  *ctx = NULL;
  if (!(*init)) {
    PetscErrorCode ierr;
    HookInfo       info;

    if (!petsc_comm_world_size){
      ierr = MPI_Comm_size(PETSC_COMM_WORLD, &petsc_comm_world_size);CHKERRQ(ierr);
    }
    ierr = PetscNew(&info);CHKERRQ(ierr);
    info->prevTag = -8675309;
    info->historySize = 200;
    info->numCycles = -1;
    info->type = send;
    ierr = PetscCalloc1(info->historySize, &info->line);CHKERRQ(ierr);
    ierr = PetscCalloc1(info->historySize, &info->func);CHKERRQ(ierr);
    ierr = PetscCalloc1(info->historySize, &info->file);CHKERRQ(ierr);
    ierr = PetscCalloc1(info->historySize, &info->commHistory);CHKERRQ(ierr);
    for (PetscInt i = 0; i < info->historySize; ++i) {
      /* Number of comm partners cannot be greater than sizeof(PETSC_COMM_WORLD) */
      ierr = PetscCalloc1(petsc_comm_world_size, &(info->commHistory[i]));CHKERRQ(ierr);
    }
    *init = PETSC_TRUE;
    *ctx = info;
  }
...
PETSC_STATIC_INLINE PetscErrorCode PetscMPICommHookDestroy(HookInfo *ctx, PetscBool *init)
{
  PetscFunctionBegin;
  PetscValidBoolPointer(init,2);
  ...
  if (*init) {
    ...
    for (PetscInt i = 0; i < (*ctx)->historySize; ++i) {
      /* Number of comm partners cannot be greater than sizeof(PETSC_COMM_WORLD) */
      ierr = PetscFree((*ctx)->func[i]);CHKERRQ(ierr);
      ierr = PetscFree((*ctx)->file[i]);CHKERRQ(ierr);
      ierr = PetscFree((*ctx)->commHistory[i]);CHKERRQ(ierr);
    }
    ierr = PetscFree((*ctx)->line);CHKERRQ(ierr);
    ierr = PetscFree((*ctx)->func);CHKERRQ(ierr);
    ierr = PetscFree((*ctx)->file);CHKERRQ(ierr);
    ierr = PetscFree((*ctx)->commHistory);CHKERRQ(ierr);
...
PETSC_STATIC_INLINE PetscErrorCode PetscMPICommHook(int count, MPI_Datatype datatype, int dest, int tag, MPI_Request *request, int line, const char *func, const char *file, HookInfo ctx)
{
  PetscErrorCode ierr;
  PetscBool      newTag = PETSC_FALSE;
  PetscMPIInt    dtypeSize;

  PetscFunctionBegin;
  /* How we determine whether we have called from the same place in a loop. It is by no means foolproof */
  if (ctx->prevTag != tag) {
    ++(ctx->numCycles);
    ctx->prevTag = tag;
    newTag = PETSC_TRUE;
  }
  if (ctx->numCycles == ctx->historySize) {
    ctx->historySize *= 2;
    ierr = PetscPrintf(PETSC_COMM_SELF,"Resizing info!\n");CHKERRQ(ierr);
    ierr = PetscRealloc(ctx->historySize*sizeof(int),&ctx->line);CHKERRQ(ierr);
    ierr = PetscRealloc(ctx->historySize*sizeof(char*),&ctx->func);CHKERRQ(ierr);
    ierr = PetscRealloc(ctx->historySize*sizeof(char*),&ctx->file);CHKERRQ(ierr);
    ierr = PetscRealloc(ctx->historySize*sizeof(PetscInt*),&ctx->commHistory);CHKERRQ(ierr);
    for (PetscInt i = ctx->numCycles; i < ctx->historySize; ++i) {
      ierr = PetscCalloc1(petsc_comm_world_size, &(ctx->commHistory[i]));CHKERRQ(ierr);
    }
  }
  if (newTag) {
    ctx->line[ctx->numCycles] = line;
    ierr = PetscStrallocpy(func, &(ctx->func[ctx->numCycles]));CHKERRQ(ierr);
    ierr = PetscStrallocpy(file, &(ctx->file[ctx->numCycles]));CHKERRQ(ierr);
  }
  ierr = MPI_Type_size(datatype, &dtypeSize);CHKERRQ(ierr);
  ctx->commHistory[ctx->numCycles][dest] += count*dtypeSize;
  PetscFunctionReturn(0);
}

PetscErrorCode PetscMPI_isend_hook(int count, MPI_Datatype datatype, int dest, int tag, MPI_Request *request, int line, const char *func,const char *file)
{
  PetscErrorCode ierr;

  PetscFunctionBegin;
  ierr = PetscMPICommHook(count,datatype,dest,tag,request,line,func,file,sendHookInfo);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}

PetscErrorCode PetscMPI_irecv_hook(int count, MPI_Datatype datatype, int source, int tag, MPI_Request *request, int line, const char *func,const char *file)
{
  PetscErrorCode ierr;

  PetscFunctionBegin;
  ierr = PetscMPICommHook(count,datatype,source,tag,request,line,func,file,recvHookInfo);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}

PetscErrorCode PetscMPIHookGetStats(HookInfo *hook, PetscBool send)
{
  PetscFunctionBegin;
  PetscValidPointer(hook,1);
  if (send) *hook = sendHookInfo;
  else *hook = recvHookInfo;
  PetscFunctionReturn(0);
}

PetscErrorCode PetscMPIHookViewFromOptions(MPI_Comm comm, HookInfo hook, const char name[])
{
  PetscViewer       viewer;
  PetscViewerFormat format;
  PetscBool         flg, isAscii;
  PetscErrorCode    ierr;

  PetscFunctionBegin;
  ierr = PetscOptionsGetViewer(comm,NULL,NULL,name,&viewer,&format,&flg);CHKERRQ(ierr);
  if (flg) {
    ierr = PetscViewerPushFormat(viewer,format);CHKERRQ(ierr);
  } else {viewer = PETSC_VIEWER_STDOUT_(comm);}
  ierr = PetscObjectTypeCompare((PetscObject)viewer,PETSCVIEWERASCII,&isAscii);CHKERRQ(ierr);
  if (isAscii) {
    PetscMPIInt rank;

    ierr = MPI_Comm_rank(comm,&rank);CHKERRQ(ierr);
    ierr = PetscViewerASCIIPrintf(viewer,"=== %s ===\n",hook->type ? "MPI_Send" : "MPI_Recv");CHKERRQ(ierr);
    ierr = PetscViewerASCIIPushSynchronized(viewer);CHKERRQ(ierr);
    ierr = PetscViewerASCIISynchronizedPrintf(viewer,"[%d] Total number of communications: %D\n",rank,hook->numCycles+1);CHKERRQ(ierr);
    ierr = PetscViewerFlush(viewer);CHKERRQ(ierr);
    for (PetscInt i = 0; i < hook->numCycles+1; ++i) {
      ierr = PetscViewerASCIISynchronizedPrintf(viewer,"[%d] %s #%D: %s() line %d in %s\n",rank,hook->type ? "MPI_Send" : "MPI_Recv",i,hook->func[i],hook->line[i],hook->file[i]);CHKERRQ(ierr);
      ierr = PetscViewerASCIIPushTab(viewer);CHKERRQ(ierr);
      for (PetscInt j = 0; j < petsc_comm_world_size; ++j) {
        if (hook->commHistory[i][j]) {
          ierr = PetscViewerASCIISynchronizedPrintf(viewer,"[%d] %s [%D] %D bytes\n",rank,hook->type ? "->" : "<-",j,hook->commHistory[i][j]);CHKERRQ(ierr);
        }
      }
...
PetscErrorCode PetscMPIHookZeroCounters(HookInfo hook)
{
  PetscErrorCode ierr;

  PetscFunctionBegin;
  ierr = PetscPrintf(PETSC_COMM_WORLD,"=== CLEARING MPI HOOK COUNTERS ===\n");CHKERRQ(ierr);
  if (hook->numCycles >= 0) {
    ierr = PetscArrayzero(hook->line, hook->numCycles);CHKERRQ(ierr);
    for (PetscInt i = 0; i < hook->numCycles; ++i) {
      ierr = PetscFree(hook->func[i]);CHKERRQ(ierr);
      ierr = PetscFree(hook->file[i]);CHKERRQ(ierr);
      ierr = PetscArrayzero(hook->commHistory[i], petsc_comm_world_size);CHKERRQ(ierr);
    }
    hook->numCycles = -1;
    hook->prevTag = -8675309;
  }
  PetscFunctionReturn(0);
}
/*@C
...
#define PSTRSZ 2
  ...
  */
  ...
  ierr = PetscMPICommHookCreate(&sendHookInfo, PETSC_TRUE, &initSendHook);CHKERRQ(ierr);
  ierr = PetscMPICommHookCreate(&recvHookInfo, PETSC_FALSE, &initRecvHook);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode  PetscFinalize(void)
{
  ...
  ierr = PetscMPICommHookDestroy(&sendHookInfo, &initSendHook);CHKERRQ(ierr);
  ierr = PetscMPICommHookDestroy(&recvHookInfo, &initRecvHook);CHKERRQ(ierr);
