...
/*TEST
   ...
   test:
      ...
      filter: egrep -v "(malloc|nox|display|saws_port|vecscatter|check_pointer_intensity|cuda_initialize|error_output_stdout|use_gpu_aware_mpi|fp_trap)" | egrep "^-"
   testset:
      args: -options_left 0 -options_view
      test:
        suffix: 1
        ...
        filter: Error: egrep -v "(malloc|nox|display|saws_port|vecscatter|check_pointer_intensity|cuda_initialize|error_output_stdout|use_gpu_aware_mpi|fp_trap)" | egrep "ex54options"
      test:
