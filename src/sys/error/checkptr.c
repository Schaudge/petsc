...
PetscBool PetscCheckMpiGpuAwareness()
{
  ...
  cerr = cudaMalloc((void**)&dbuf,sizeof(int)*2);CHKERRCUDA(cerr);
  cerr = cudaMemcpy(dbuf,hbuf,sizeof(int)*2,cudaMemcpyHostToDevice);CHKERRCUDA(cerr);
  ...
  PetscSegvJumpBuf_set = PETSC_FALSE;
  cerr = cudaFree(dbuf);CHKERRCUDA(cerr);
  return awareness;
}
#endif
#else
