...
.vb
  ...
  TaoTermDestroy(&term);
.ve
  If you would like to have a dictionary matrix term $\alpha \tfrac{1}{2}\|D x \|_1^2$, do the same but pass `D` as the map of the term\:
.vb
  MatGetLocalSize(D, &m, NULL);
  MatGetSize(D, &M, NULL);
  TaoTermCreateL1(PetscObjectComm((PetscObject))D), m, M, 0.0, &term);
  TaoAddObjectiveTerm(tao, "reg_", alpha, term, NULL, D);
  TaoTermDestroy(&term);
.ve
