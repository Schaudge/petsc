...
static PetscErrorCode TaoSolve_ASFLS(Tao tao)
{
  ...
  while (1) {
    ...
    ierr = PetscInfo1(tao,"Number of fixed variables: %" PetscInt_FMT "\n", nf);CHKERRQ(ierr);
    ...
    if (innerd >= -asls->delta*PetscPowReal(normd, asls->rho)) {
      ...
      ierr = PetscInfo1(tao, "Iteration %" PetscInt_FMT ": newton direction not descent\n", tao->niter);CHKERRQ(ierr);
