...
PetscErrorCode UserObjGrad_DM(DM dm, Vec X, PetscReal *f, Vec G, void *ptr)
{
  ...
  PetscCall(VecNorm(user->workvec, NORM_2, f));
  *f *= 0.5;
...
static PetscErrorCode Shuffle_List(PetscInt *indices, PetscInt n, PetscRandom rctx)
{
  ...
  PetscFunctionBegin;
  //TODO assert size n>2 or something like that
  for (i=0; i< n-2; i++) {
...
PetscErrorCode DataCreate(AppCtx *user)
{
  ...
  PetscCall(MatScale(user->A, 1/norm));
...
static PetscErrorCode ProcessOptions(MPI_Comm comm, AppCtx *user)
{
  ...
  PetscCall(PetscOptionsEList("-problem_type", "Decide whether to use Tao or DM to setup problem statement.", "fb_example.c", probTypes, 2, probTypes[user->probType], &probtype, NULL));
