...
/*
Description:   Loris-Verhoeven tomography reconstruction example .
...
int main(int argc, char **argv)
{
  ...
  PetscCall(TaoSetType(tao, TAOFB));
  PetscCall(TaoFBSetType(tao, TAO_FB_TYPE_LV));
  ...
  switch (user.reg) {
  case 0:
    PetscCall(TaoFBSetNonSmoothTermWithLinearMap(tao, g_dm, NULL));
    break;
  case 1:
    PetscCall(TaoFBSetNonSmoothTermWithLinearMap(tao, g_dm, user.D));
    break;
  }
