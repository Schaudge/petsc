...
PetscErrorCode Log_UserObjGrad_DM(DM dm, Vec X, PetscReal *f, Vec G, void *ptr)
{
  ...
  PetscCall(VecGetValues(X, 1, ix, &xlast));
...
PetscErrorCode DataCreate(AppCtx *user)
{
  ...
  case PROB_LASSO:
    ...
    PetscCall(VecCreate(PETSC_COMM_WORLD, &user->workvec));
    PetscCall(VecSetSizes(user->workvec, PETSC_DECIDE, user->m));
    PetscCall(VecSetFromOptions(user->workvec));
    PetscCall(VecDuplicate(user->workvec, &user->workvec3));
    PetscCall(VecDuplicate(user->workvec, &user->b));
    ...
    for (i = 0; i < m_mat_local*n_mat_local; i++) {
      PetscReal a[1] = {0.};
