...
static PetscErrorCode TaoSetFromOptions_BQNLS(Tao tao, PetscOptionItems *PetscOptionsObject)
{
  ...
  PetscCall(MatIsHPDKnown(bqnk->B, &is_set, &is_hpd));
  PetscCheck(is_set && is_hpd, PetscObjectComm((PetscObject)tao), PETSC_ERR_ARG_INCOMP, "LMVM matrix must be hermitian positive-definite");
  PetscFunctionReturn(PETSC_SUCCESS);
}
