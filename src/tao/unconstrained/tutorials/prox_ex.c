...
static char help[] = "This example demonstrates various ways to use TaoPD to solve proximal algorithms.. \n";
...
int main(int argc, char **argv)
{
  ...
  PetscCall(TaoSetNumTerms(tao, 2));
  ...
  case 0:
    ...
    switch (user.create) {
    case 0:
      PetscCall(TaoPDCreate_L1(PETSC_COMM_SELF, &pd0, NULL, user.lb, user.ub));
      PetscCall(TaoPDCreate_L2(PETSC_COMM_SELF, &pd1, y, NULL));
      break;
    case 1:
      PetscCall(TaoPDCreate(PETSC_COMM_SELF, &pd0));
      PetscCall(TaoPDCreate(PETSC_COMM_SELF, &pd1));
      PetscCall(TaoPDSetType(pd0, TAOPDL1));
      PetscCall(TaoPDSetType(pd1, TAOPDL2));
      PetscCall(TaoPDSetCentralVector(pd1, y));
      break;
    default:
  ...
  case 1:
    switch (user.create) {
    case 0:
      PetscCall(TaoPDCreate_Simplex(PETSC_COMM_SELF, &pd0, NULL, user.simp, user.tol));
      PetscCall(TaoPDCreate_L2(PETSC_COMM_SELF, &pd1, y, NULL));
      break;
    case 1:
      PetscCall(TaoPDCreate(PETSC_COMM_SELF, &pd0));
      PetscCall(TaoPDCreate(PETSC_COMM_SELF, &pd1));
      PetscCall(TaoPDSetType(pd0, TAOPDSIMPLEX));
      PetscCall(TaoPDSetType(pd1, TAOPDL2));
      PetscCall(TaoPDSetCentralVector(pd1, y));
      break;
    default:
  ...
  PetscCall(TaoSetTotalNumPD(tao, 2));
  PetscCall(TaoSetPDTerm(tao, pd0, 0));
  PetscCall(TaoSetPDTerm(tao, pd1, 1));
  ...
  case 0:
    ...
    break;
  case 1:
    PetscCall(TaoPDApplyProximalMap(pd0, pd1, user.stepsize, y, x, NULL));
    break;
  default:
  ...
  PetscCall(TaoPDDestroy(&pd0));
  PetscCall(TaoPDDestroy(&pd1));
