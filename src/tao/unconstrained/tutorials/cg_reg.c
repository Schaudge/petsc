...
PetscErrorCode L2_ObjGrad_Tao(Tao tao, Vec X, PetscReal *f, Vec G, void *ptr)
{
  ...
  if (user->y_bool) {
    ...
    PetscCall(TaoPDGetCentralVector(reg, &y));
...
PetscErrorCode L2_ObjGrad(TaoPD pd, Vec X, PetscReal *f, Vec G, void *ptr)
{
  AppCtx *user = (AppCtx *)ptr;
...
PetscErrorCode L2_Obj(TaoPD pd, Vec X, PetscReal *f, void *ptr)
{
  AppCtx *user = (AppCtx *)ptr;
...
PetscErrorCode L2_Grad(TaoPD pd, Vec X, Vec G, void *ptr)
{
  AppCtx *user = (AppCtx *)ptr;
...
int main(int argc, char **argv)
{
  ...
  PetscCall(TaoCreate(PETSC_COMM_WORLD, &pd_tao));
  PetscCall(TaoSetFromOptions(pd_tao)); //im not even setting solution vec, nor type... will this work, or do i need to set something?
   ...
   * 2: Built-in type
   * 3: Tao              */
  PetscCall(TaoPDCreate(PetscObjectComm((PetscObject)tao), &pd));

  /* Set PD */
  switch (user.g_type) {
  case 0:
    PetscCall(TaoPDSetObjectiveAndGradient(pd, L2_ObjGrad, (void *)&user));
    break;
  case 1:
    PetscCall(TaoPDSetObjective(pd, L2_Obj, (void *)&user));
    PetscCall(TaoPDSetGradient(pd, L2_Grad, (void *)&user));
    break;
  case 2:
    PetscCall(TaoPDSetType(pd, TAOPDL2));
    break;
  case 3:
    PetscCall(TaoSetObjectiveAndGradient(pd_tao, NULL, L2_ObjGrad_Tao, (void *)&user));
    PetscCall(TaoPDUseTaoRoutines(pd, pd_tao));
    break;
  case 4:
    PetscCall(TaoSetObjective(pd_tao, L2_Obj_Tao, (void *)&user));
    PetscCall(TaoSetGradient(pd_tao, NULL, L2_Grad_Tao, (void *)&user));
    PetscCall(TaoPDUseTaoRoutines(pd, pd_tao));
    break;
  default:
    SETERRQ(PetscObjectComm((PetscObject)tao), PETSC_ERR_USER, "Invalid PD formulation type.");
  }
  // TODO  SCALE?

  /* Solve full version */
  /* Solve Regularizer version */
  PetscCall(TaoPDSetScale(pd, user.stepsize));
  if (user.y_bool) { PetscCall(TaoPDSetCentralVector(pd, user.y)); }
  PetscCall(TaoSetRegularizer(tao, pd));
  ...
  PetscCall(TaoPDDestroy(&pd));
  PetscCall(TaoDestroy(&tao));
  PetscCall(TaoDestroy(&tao_full));
  PetscCall(TaoDestroy(&pd_tao));
