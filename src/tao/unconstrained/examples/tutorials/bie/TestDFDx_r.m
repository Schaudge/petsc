...
for ii=1:nc
    ...
    GG_r{ii} = cylinder_r(rc,xp(ii),(yp(ii)-2*ys));
    GG_r{ii} = curvquad(GG_r{ii},'ptr',N,10);
    GG_r_eps{ii} = cylinder_r(rc,xp(ii) ,(yp(ii) + epsilon - 2*ys));
    GG_r_eps{ii} = curvquad(GG_r_eps{ii},'ptr',N,10);
end
        ...
        %ps= ps+evalCFIEhelm_srcH1(GG{ii}.x,GG{jj},sigma,k,eta);         % evaluate soln
            if (ii == jj)
                ps_eps = ps_eps + 2/epsilon*( (evalCFIEhelm_src_F_K(GG_eps{ii}.x,GG_eps{jj},sigma(1+(jj-1)*N:jj*N),k,eta) - ...
                                         evalCFIEhelm_src_F_K(GG_r_eps{ii}.x,GG_eps{jj},sigma(1+(jj-1)*N:jj*N),k,eta))-...
                                    (evalCFIEhelm_src_F_K(GG{ii}.x,GG{jj},sigma(1+(jj-1)*N:jj*N),k,eta) -...
                                        evalCFIEhelm_src_F_K(GG_r{ii}.x,GG{jj},sigma(1+(jj-1)*N:jj*N),k,eta) ));
                
                 2/epsilon*( (evalCFIEhelm_src_F_K(GG_eps{ii}.x,GG_eps{jj},sigma(1+(jj-1)*N:jj*N),k,eta) - ...
                                          evalCFIEhelm_src_F_K(GG_r_eps{ii}.x,GG_eps{jj},sigma(1+(jj-1)*N:jj*N),k,eta))-...
                                     (evalCFIEhelm_src_F_K(GG{ii}.x,GG{jj},sigma(1+(jj-1)*N:jj*N),k,eta) -...
                                         evalCFIEhelm_src_F_K(GG_r{ii}.x,GG{jj},sigma(1+(jj-1)*N:jj*N),k,eta) ))
 pause
               ...
               % ps = ps+evalCFIEhelm_src_derF_K(GG{ii}.x,GG{jj},sigma(1+(jj-1)*N:jj*N),k,eta,0*dsigma(1+(jj-1)*N:jj*N));
                ...
                ps_eps = ps_eps + 2/epsilon*( (evalCFIEhelm_src_F_S(GG_eps{ii}.x,GG{jj},sigma(1+(jj-1)*N:jj*N),k,eta)-...
                                                evalCFIEhelm_src_F_S(GG_r_eps{ii}.x,GG{jj},sigma(1+(jj-1)*N:jj*N),k,eta) )-...
                                                (evalCFIEhelm_src_F_S(GG{ii}.x,GG{jj},sigma(1+(jj-1)*N:jj*N),k,eta) -...
                                                   evalCFIEhelm_src_F_S(GG_r{ii}.x,GG{jj},sigma(1+(jj-1)*N:jj*N),k,eta) ));   
                ...
                dFdxc_eps(N*(jj-1)+1:N*jj,ii) = 2/epsilon*( (evalCFIEhelm_src_F_S(GG{jj}.x,GG_eps{ii},sigma(1+(ii-1)*N:ii*N),k,eta) -...
                                                        evalCFIEhelm_src_F_S(GG_r{jj}.x,GG_eps{ii},sigma(1+(ii-1)*N:ii*N),k,eta) )-...
                                                        (evalCFIEhelm_src_F_S(GG{jj}.x,GG{ii},sigma(1+(ii-1)*N:ii*N),k,eta) -...
                                                        evalCFIEhelm_src_F_S(GG_r{jj}.x,GG{ii},sigma(1+(ii-1)*N:ii*N),k,eta) )) ;
                %pause
   ...
   % dFdxc(N*(ii-1)+1:N*ii,ii) = ps+2*fgrad(GG{ii}.x);
    ...
    for ii = 1:nc
        ...
        for jj = 1:nc
            if (ii == jj)
                vv_r_diag = evalCFIEhelm_src_derF_K_r(GG_r{ii}.x,GG{jj},sigma(1+(jj-1)*N:jj*N),k,eta);
                
                pff_x = pff_x + 0;
                pff_y = pff_y - 2.0 * vv_r_diag;
       - 2.0 * vv_r_diag
 pause
...
disp(X)
for ii=1:nc
    error(ii) = norm(2*fgrady(GG{ii}.x) - 2*(f(GG_eps{ii}.x)-f(GG{ii}.x))/( epsilon));
end
