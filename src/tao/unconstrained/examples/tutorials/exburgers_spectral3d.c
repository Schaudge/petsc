...
int main(int argc,char **argv)
{
  ...
  appctx.param.Lz    = 6.0;  /* length of the domain */
...
/*
  ...
  //for (ix=1; ix<Nl; ix++)  {A[ix] = A[ix-1]+Nl;}  
  ierr = PetscMalloc1(Nl3,&A[0][0]);CHKERRQ(ierr);

for (ix=1; ix<Nl; ix++)  
     {for (iy=1; iy<Nl; iy++)
     {for (iz=1; iz<Nl; iz++)
     {A[ix-1][iy-1][iz-1]=(&A[0][0][0]+(ix-1)*Nl2+(iy-1)*Nl+iz-1);
      A[ix-1][iy-1][iz-1]=0.0;
      printf("ix[%d]iy[%d]iz[%d]=%f\n",ix,iy,iz,A[ix-1][iy-1][iz-1]);
     } 
     }
    }
...
for (ix=1; ix<Nl; ix++)  
     { A[ix] = A[ix-1]+Nl2;  
       //A[ix][0] = A[ix-1][0]+Nl2;
     for (iy=1; iy<Nl; iy++)
     {A[ix-1][iy] = A[ix-1][iy-1]+Nl;
      A[ix][iy-1] = A[ix-1][iy-1]+Nl;
      A[ix][iy] = A[ix-1][iy-1]+Nl2;
     for (iz=1; iz<Nl; iz++)
     {
      A[ix-1][iy-1][iz-1]=0.0;
      //printf("ix[%d]iy[%d]iz[%d]=%f\n",ix,iy,iz,A[ix-1][iy-1][iz-1]);
...
PetscErrorCode RHSFunction(TS ts,PetscReal t,Vec globalin,Vec globalout,void *ctx)
{
  ...
  DMDACoor3d      ***coors;
  PetscScalar     tt, alpha, beta, tempu, tempv,tempw,xpy; 
  PetscInt        inc;   
  static int its=0;
  char var[12] ;
  ...
  xs=xs/(Nl-1);  xm=xm/(Nl-1);
  ys=ys/(Nl-1);  ym=ym/(Nl-1); 
  zs=zs/(Nl-1);  zm=zm/(Nl-1); 
...
/*
  ierr = PetscMalloc1(appctx->param.N,&temp1);CHKERRQ(ierr);
  ierr = PetscMalloc1(appctx->param.N*appctx->param.N,&temp1[0]);CHKERRQ(ierr);
  for (ix=1; ix<Nl; ix++) temp1[ix] = temp1[ix-1]+Nl;

  ierr = PetscMalloc1(appctx->param.N,&temp2);CHKERRQ(ierr);
  ierr = PetscMalloc1(appctx->param.N*appctx->param.N,&temp2[0]);CHKERRQ(ierr);
  for (ix=1; ix<Nl; ix++) temp2[ix] = temp2[ix-1]+Nl;
*/
  alpha = 1.0;
  beta  = 0.0;
       ...
       {for (jx=0; jx<Nl; jx++) 
         {for (jy=0; jy<Nl; jy++) 
           {for (jz=0; jz<Nl; jz++) 
           {
            ...
            //printf("ulbff[%d][%d][%d]=%f\n",jx,jy,jz,ulb[jx][jy][jz]);
            //printf("mass[%d][%d]=%f\n",jx,jy,mass[jx][jy]);
          }}}

        //here the stifness matrix in 2d
        //first product (B x B x K_yy)u=W2 (u_zz)
        alpha=1.0; //appctx->param.Lex/2.0;
        BLASgemm_("N","N",&Nl,&Nl2,&Nl,&alpha,&mass[0][0],&Nl,&ulb[0][0][0],&Nl,&beta,&wrk1[0][0][0],&Nl);
        for (jx=0; jx<Nl; jx++) 
        {temp1=&wrk1[0][0][0]+jx*Nl2;
         temp2=&wrk2[0][0][0]+jx*Nl2;
         BLASgemm_("T","T",&Nl,&Nl,&Nl,&alpha,temp1,&Nl,&mass[0][0],&Nl,&beta,temp2,&Nl);
         //*wrk2[jx]=*temp2;
        fp = fopen("arrays.m", "w+");
        for (jy=0; jy<Nl; jy++) 
          {for (jz=0; jz<Nl; jz++) 
            {
            fprintf(fp,"wrk1(%d,%d,%d)=%f;",jx+1,jy+1,jz+1,wrk1[jx][jy][jz]);
            fprintf(fp,"ulb(%d,%d,%d)=%f;",jx+1,jy+1,jz+1,ulb[jx][jy][jz]);
            fprintf(fp,"wrk2(%d,%d,%d)=%f;",jx+1,jy+1,jz+1,wrk2[jx][jy][jz]);
            fprintf(fp,"temp2(%d,%d)=%f;",jy+1,jz+1,temp2[jy*Nl+jz]);
            fprintf(fp,"massg(%d,%d)=%f;",jy+1,jz+1,mass[jy*Nl+jz]);
            //fprintf(fp,"massgo(%d,%d)=%f;",jy+1,jz+1,mass[jy][jz]);
            fprintf(fp,"temp1(%d,%d)=%f;\n",jy+1,jz+1,temp1[jy*Nl+jz]);
          }}
         fclose(fp);
        }
        //exit(1);
        alpha=1.0; //2./appctx->param.Ley;
        BLASgemm_("N","T",&Nl2,&Nl,&Nl,&alpha,&wrk2[0][0][0],&Nl2,&stiff[0][0],&Nl,&beta,&wrk3[0][0][0],&Nl2);
        ...
        {for (jy=0; jy<appctx->param.N; jy++)  
         {for (jz=0; jz<appctx->param.N; jz++)  
           {indx=ix*(appctx->param.N-1)+jx;
            ...
            outl[indx][indy][indz].u +=wrk3[jx][jy][jz];
            outl[indx][indy][indz].v +=ul[indx][indy][indz].v;
            outl[indx][indy][indz].w +=ul[indx][indy][indz].w;
           }}
           }
        }
      }
