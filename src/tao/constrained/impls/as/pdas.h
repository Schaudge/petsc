...
/*
 Context for Primal-Dual Active Set Method
*/
...
typedef struct {
  ...
  /* Scalars */
  PetscReal L;     /* Lagrangian = f(x) - lambdae^T*ce(x) - lambdai^T*(ci(x) - z) */
  ...
  /* Active inequality constraint locations */
  PetscInt *idxineq_act; /* idxineq_act[i] = 0 if constraint inactive, else 1 - active */
...
PETSC_INTERN PetscErrorCode TaoSNESFunction_AS(SNES,Vec,Vec,void*);

#endif /* ifndef __TAO_PDAS_H */
