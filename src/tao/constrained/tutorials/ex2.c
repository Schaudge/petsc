...
/* problem replicated from ex1 but defined with double-sided inequalities */

#include <petsctao.h>

static  char help[]= "Solves constrained optimiztion problem using TAOMAD.\n\
...
PetscErrorCode main(int argc,char **argv)
{
  ...
  ierr = TaoSetType(tao,TAOMAD);CHKERRQ(ierr);
...
PetscErrorCode InitializeProblem(AppCtx *user)
{
  ...
  ierr = VecCreate(PETSC_COMM_WORLD,&user->ci);CHKERRQ(ierr); /* a 1x1 vec for inequality constraints */
...
{
  AppCtx            *user=(AppCtx*)ctx;
  PetscInt          rows[1],cols[2],min,max;
  PetscScalar       vals[2];
  const PetscScalar *x;
  PetscErrorCode    ierr;
  Vec               Xseq=user->Xseq;
...
PetscErrorCode FormEqualityJacobian(Tao tao,Vec X,Mat JE,Mat JEpre,void *ctx)
{
  ...
  if (!rank) {
    ...
    cols[0] = 0;  cols[1] = 1;
    vals[0] = 2*x[0];  vals[1] = 1.0;
    ierr = MatSetValues(JE,1,rows,2,cols,vals,INSERT_VALUES);CHKERRQ(ierr);
