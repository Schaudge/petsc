...
/*@
  DMTaoSetScale - Sets scale of `DMTao` object.
...
@*/
PetscErrorCode DMTaoSetScale(DM dm, PetscReal scale)
{
  DMTao tdm;

  PetscFunctionBegin;
  PetscValidHeaderSpecific(dm, DM_CLASSID, 1);
  PetscValidLogicalCollectiveReal(dm, scale, 2);
  PetscCall(DMGetDMTaoWrite(dm, &tdm));
  tdm->scale = scale;
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
@*/
PetscErrorCode DMTaoGetScale(DM dm, PetscReal *scale)
{
  DMTao tdm;
...
.seealso: [](ch_tao), `Tao`, `DMTao`, `DMTaoCreate()`
@*/
PetscErrorCode DMTaoApplyProximalMap(DM dm0, DM dm1, PetscReal lambda, Vec y, Vec x, PetscBool is_cj, void *ctx)
{
