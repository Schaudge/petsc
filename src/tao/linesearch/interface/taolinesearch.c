...
/*@C
   TaoLineSearchSetPreCheck - Sets a user function that is called after the initial search direction has been computed but
         before the line search routine has been applied. Allows the user to adjust the result of  that
         determined the search direction.

   Logically Collective on TaoLineSearch

   Input Parameters:
+  linesearch - the TaoLineSearch context
.  func - [optional] function evaluation routine, see TaoLineSearchPreCheck() for the calling sequence
-  ctx        - [optional] user-defined context for private data for the function evaluation routine (may be NULL)

   Level: intermediate

.seealso: TaoGetLineSearch(), TaoLineSearchPreCheck(), TaoLineSearchGetPreCheck()
@*/
PetscErrorCode  TaoLineSearchSetPreCheck(TaoLineSearch linesearch, PetscErrorCode (*func)(TaoLineSearch,Vec,Vec,PetscBool*,void*),void *ctx)
{
  PetscFunctionBegin;
  PetscValidHeaderSpecific(linesearch,TAOLINESEARCH_CLASSID,1);
  if (func) linesearch->ops->precheck = func;
  if (ctx) linesearch->userctx_precheck = ctx;
  PetscFunctionReturn(0);
}

/*@C
   TaoLineSearchGetPreCheck - Gets the pre-check function for the line search routine.

   Input Parameters:
.  linesearch - the TaoLineSearch context

   Output Parameters:
+  func       - [optional] function evaluation routine, see TaoLineSearchPreCheck() for calling sequence
-  ctx        - [optional] user-defined context for private data for the function evaluation routine (may be NULL)

   Level: intermediate

.seealso: TaoGetLineSearch(), TaoLineSearchPreCheck(), TaoLineSearchSetPreCheck()
@*/
PetscErrorCode  TaoLineSearchGetPreCheck(TaoLineSearch linesearch, PetscErrorCode (**func)(TaoLineSearch,Vec,Vec,PetscBool*,void*),void **ctx)
{
  PetscFunctionBegin;
  PetscValidHeaderSpecific(linesearch,TAOLINESEARCH_CLASSID,1);
  if (func) *func = linesearch->ops->precheck;
  if (ctx) *ctx = linesearch->userctx_precheck;
  PetscFunctionReturn(0);
}

/*@
   TaoLineSearchPreCheck - Allows the user to change the linesearch direction picked by the Tao optimization algorithm before the line search is applied
...
.seealso: TaoGetLineSearch(), TaoLineSearchSetPreCheck(), TaoLineSearchGetPreCheck()
@*/
PetscErrorCode TaoLineSearchPreCheck(TaoLineSearch linesearch,Vec X,Vec Y,PetscBool *changed)
{
  PetscErrorCode ierr;
  ...
  *changed = PETSC_FALSE;
  if (linesearch->ops->precheck) {
    ierr = (*linesearch->ops->precheck)(linesearch, X, Y, changed, linesearch->userctx_precheck);CHKERRQ(ierr);
    PetscValidLogicalCollectiveBool(linesearch,*changed,4);
    if (*changed) {
      ierr = PetscInfo((PetscObject)linesearch,"User has changed search direction generated by Tao optimization algorithm in TaoLineSearchPreCheck()\n");CHKERRQ(ierr);
    }
  }
  PetscFunctionReturn(0);
}
