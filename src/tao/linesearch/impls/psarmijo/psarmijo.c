...
-  step - initial estimate of step length (Set via TaoLSSetInitialStep)
   ...
   Output parameters:
+  f    - f(x_{k+1})
.  s    - x_{k+1} that satisfies the condition
...
@ */
static PetscErrorCode TaoLineSearchApply_PSArmijo(TaoLineSearch ls, Vec x, PetscReal *f, Vec g, Vec s)
{
  TaoLineSearch_PSARMIJO *armP = (TaoLineSearch_PSARMIJO *)ls->data;
  PetscInt                i, its = 0;
  PetscReal               ref, cert;
  ...
  if (!armP->work) {
    ...
    PetscCall(VecDuplicate(x, &armP->work2));
    armP->x = x;
  ...
  //TODO check if taols isusing taoroutine
  ...
  //TODO what do we want for minstep?
  //TODO assert stepsize >0?
  PetscCall(VecWAXPY(armP->work, -ls->step, g, x));
  while (ls->step >= owlqn_minstep && ls->nfeval < ls->max_funcs) {
    /* Calculate iterate */
    ++its;

    /* Calculate function at new iterate */
    PetscCall(TaoLineSearchComputeObjective(ls, s, &fk1));
    /* work2 : x_{k+1} - x_k */
    PetscCall(VecWAXPY(armP->work2, -1., x, s));
    ...
    if (cert <=0) {
      ls->step *= 0.5;
      PetscCall(TaoComputeFixedPoint(ls->tao, armP->work, s, ls->step, NULL));
    } else { ls->step *= 0.5; }
