...
static char help[] = "Run SINDy on data generated from dx/dt = [-sin(x), cos(x)+t].\n";
...
{ 
  ...
  for (r = 0; r < runs; r++) {
    ...
    all_t[i] = TSGetTime(ts, &all_t[i]);
    i++;
    for (t = 1; t < steps; t++) {
      ierr = TSStep(ts);CHKERRQ(ierr);
      ierr = VecCopy(X, all_x[i]);CHKERRQ(ierr);
      ierr = TSGetTime(ts, &all_t[i]);CHKERRQ(ierr);
      i++;
    }
  }
