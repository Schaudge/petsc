...
PetscErrorCode RHSJacobian(TS ts, PetscReal t, Vec X, Mat J, Mat B, void* ctx) {
  ...
  PetscFunctionBegin;
  ierr = VecGetArrayRead(X, &x);CHKERRQ(ierr);
  ierr = MatSetValue(J, 0, 0, 0, INSERT_VALUES);CHKERRQ(ierr);
  ierr = MatSetValue(J, 0, 1, 1, INSERT_VALUES);CHKERRQ(ierr);
  ierr = MatSetValue(J, 1, 0, -PetscCosReal(x[0]), INSERT_VALUES);CHKERRQ(ierr);
  ierr = MatSetValue(J, 1, 1, 0, INSERT_VALUES);CHKERRQ(ierr);
  ierr = VecRestoreArrayRead(X, &x);CHKERRQ(ierr);
...
PetscErrorCode DataInitialize(Data* data, Vec X)
{
  ...
  ierr = PetscOptionsBegin(PETSC_COMM_WORLD,"","Data generation options","");CHKERRQ(ierr);
  {
    ...
    ierr = PetscOptionsReal("-v0","initial velocity","",data->v0,&data->v0,NULL);CHKERRQ(ierr);
  }
...
{ 
  ...
  for (r = 0; r < data.runs; r++) {
    /* Set the initial angles centered around zero. */
