...
typedef struct {
  ...
  Vec            x_old,x_work,x_work2,r_work,diag,y,y_work,xk,u,z_old,xzdiff,zdiff,ztemp;  /* x, r=J*x, and y=D*x have size N, M, and K respectively. */
  Tao            subsolver,parent,admm_subsolver;
  PetscReal      lambda,epsilon,aug_lag,abstol,reltol; /* lambda is regularizer weight for both L2-norm Gaussian-Newton and L1-norm, ||x||_1 is approximated with sum(sqrt(x.^2+epsilon^2)-epsilon). aug_lag is augmented lagrangian multiplier*/
  PetscInt       reg_type;
  PetscInt       admm_iter;
  PetscBool      use_admm;
} TAO_BRGN;
