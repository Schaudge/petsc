...
/* Computes the parameters of the quadratic Q(x) = c + g'*x + 0.5*x*G*x'
  ...
  if (PetscUnlikely(info != 0)) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_LIB,"LAPACK routine getrf returned with value %" PetscBLASInt_FMT "\n",info);
  ...
  if (np == mfqP->n+1) {
    ...
    if (PetscUnlikely(info != 0)) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_LIB,"LAPACK routine potrf returned with value %" PetscBLASInt_FMT "\n",info);
  }
  ...
  for (k=0;k<mfqP->m;k++) {
    if (np != mfqP->n+1) {
      ...
      if (PetscUnlikely(info != 0)) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_LIB,"LAPACK routine potrs returned with value %" PetscBLASInt_FMT "\n",info);
    ...
    if (PetscUnlikely(info != 0)) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_LIB,"LAPACK routine getrs returned with value %" PetscBLASInt_FMT "\n",info);
...
static PetscErrorCode morepoints(TAO_POUNDERS *mfqP)
{
  ...
  while (mfqP->nmodelpoints < mfqP->npmax && point>=0) {
    ...
    if (PetscUnlikely(info != 0)) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_LIB,"LAPACK routine geqrf returned with value %" PetscBLASInt_FMT "\n",info);
    ...
    if (PetscUnlikely(info != 0)) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_LIB,"LAPACK routine ormqr returned with value %" PetscBLASInt_FMT "\n",info);
    ...
    if (PetscUnlikely(info != 0)) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_LIB,"LAPACK routine gesvd returned with value %" PetscBLASInt_FMT "\n",info);
  ...
  for (i=mfqP->nHist-1;i>=0;i--) {
    ...
    if (normd <= c) {
      ...
      if (!mfqP->q_is_I) {
        ...
        if (PetscUnlikely(info < 0)) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_LIB,"ormqr returned value %" PetscBLASInt_FMT "\n",info);
      }
      ...
      if (proj >= mfqP->theta1) { /* add this index to model */
        ...
        if (PetscUnlikely(info < 0)) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_LIB,"geqrf returned value %" PetscBLASInt_FMT "\n",info);
        mfqP->q_is_I = 0;
      }
