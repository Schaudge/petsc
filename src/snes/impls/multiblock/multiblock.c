...
struct _BlockDesc {
  ...
  PetscObject solver;    // subTS in case the SNES in embedded in a TS loop
...
static PetscErrorCode SNESDestroy_Multiblock(SNES snes)
{
  ...
  while (blocks) {
    ...
    PetscCall(PetscObjectDestroy(&blocks->solver));
...
static PetscErrorCode SNESMultiblockResetDM_Private(SNES snes, DM dm)
{
  ...
  while (blocks) {
    ...
    // This callback handles a SNES embedded in a TS
    PetscCall(DMSubFieldsCreateSolver(dm, subdm, &blocks->solver));
...
static PetscErrorCode SNESSolve_Multiblock(SNES snes)
{
  ...
  F = snes->vec_func; // residual
  {
    ...
    PetscCall(SNESGetDM(snes, &dm));
    PetscCall(DMSNESGetFunction(dm, NULL, (void **)&solver));
    while (blocks) {
      DM  subdm;
      Vec tmpX;

      PetscCall(DMSubFieldsRestrict(dm, solver, blocks->is, blocks->solver));
      // Set the solution as an auxiliary vec on the subdm with a special key
      PetscCall(SNESGetDM(blocks->snes, &subdm));
      PetscCall(VecDuplicate(X, &tmpX));
      PetscCall(DMSetAuxiliaryVec(subdm, NULL, 0, 1024, tmpX));
      PetscCall(DMSetSubdofIS(subdm, blocks->is));
      blocks = blocks->next;
    }
  }
...
#if 0
    // TODO Need a way to check that SNES and TS DMs match
    {
      DM dm, tdm;
      TS ts;
