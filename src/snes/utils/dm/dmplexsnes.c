...
. ctx - the context
  ...
  Developer Note:
  The naming of this set of routines is wrong. It creates a `DMInterpolationInfo` object but is called `DMInterpolationCreate()`. Either Info
  needs to be appended to all the routine names or Info needs to be dropped from `DMInterpolationInfo`.
...
.seealso: [](ch_dmbase), `DM`, `DMInterpolationInfo`, `DMInterpolationGetVector()`, `DMInterpolationAddPoints()`, `DMInterpolationCreate()`
@*/
