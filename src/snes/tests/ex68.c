...
PetscErrorCode CheckProblem1(Mat A, Vec b, Vec u)
{
  ...
  if (error/norm > 1000.*PETSC_MACHINE_EPSILON) SETERRQ1(PetscObjectComm((PetscObject)A), PETSC_ERR_ARG_WRONG, "Relative error %g is too large", (double)(error/norm));
...
PetscErrorCode CheckProblem2(Mat A, Vec b, Vec u)
{
  ...
  if (error/norm > 10000*PETSC_MACHINE_EPSILON) SETERRQ1(PetscObjectComm((PetscObject)A), PETSC_ERR_ARG_WRONG, "Relative error %g is too large", (double)(error/norm));
  error = 0.0;
