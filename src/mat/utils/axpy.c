...
PetscErrorCode MatAXPY(Mat Y,PetscScalar a,Mat X,MatStructure str)
{
  ...
  if (M1 != M2 || N1 != N2) {
    ...
    ierr = MatView(Y,PETSC_VIEWER_STDOUT_SELF);CHKERRQ(ierr);
    SETERRQ4(PetscObjectComm((PetscObject)Y),PETSC_ERR_ARG_SIZ,"Non conforming matrix add: global sizes X: %" PetscInt_FMT " x %" PetscInt_FMT ", Y: %" PetscInt_FMT " x %" PetscInt_FMT,M1,N1,M2,N2);
  }
