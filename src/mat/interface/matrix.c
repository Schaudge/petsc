...
-  prefix - the prefix to prepend to all option names for the factored matrix
   ...
   .seealso: `PetscOptionsCreate()`, `PetscOptionsDestroy()`, `PetscObjectSetOptionsPrefix()`, `PetscObjectPrependOptionsPrefix()`,
             ...
             `MatSetOptionsPrefix()`
@*/
PetscErrorCode MatAppendOptionsPrefixFactor(Mat obj,const char prefix[])
{
  char           *buf = obj->factorprefix;
  size_t         len1,len2;
  ...
  if (!buf) {
    PetscCall(MatSetOptionsPrefixFactor(obj,prefix));
    PetscFunctionReturn(0);
  }
  PetscCheck(prefix[0] != '-',PetscObjectComm((PetscObject)obj),PETSC_ERR_ARG_WRONG,"Options prefix should not begin with a hyphen");

  PetscCall(PetscStrlen(prefix,&len1));
  PetscCall(PetscStrlen(buf,&len2));
  PetscCall(PetscMalloc1(1+len1+len2,&obj->factorprefix));
  PetscCall(PetscStrcpy(obj->factorprefix,buf));
  PetscCall(PetscStrcat(obj->factorprefix,prefix));
