...
.ve
    ...
    Developer Note:
    Fortran interface is not autogenerated as the f90
...
-     SOR_ZERO_INITIAL_GUESS - zero initial guess
   ...
   Developer Note:
     We should add block SOR support for AIJ matrices with block size set to great than one and no inodes
...
-  B - the matrix we wish to propagate options to
   ...
   Notes:
     Propagates the options associated to MAT_SYMMETRY_ETERNAL, MAT_STRUCTURALLY_SYMMETRIC, MAT_HERMITIAN, MAT_SPD and MAT_SYMMETRIC
