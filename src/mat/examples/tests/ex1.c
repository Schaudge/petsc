...
static char help[] = "Tests LU, Cholesky factorization and MatMatSolve() for a sequential dense matrix. \n\
                      ...
                      For MATSEQDENSECUDA, it used cusolverDn routines \n\n";
...
int main(int argc,char **argv)
{
  ...
  ierr = PetscOptionsGetInt(NULL,NULL,"-ldl",&ldl,NULL);CHKERRQ(ierr);
  ...
  /* create matrix */
  ierr = MatCreateSeqDense(PETSC_COMM_WORLD,n,n,NULL,&mat);CHKERRQ(ierr);
...
/*TEST
   ...
   test:
     requires: cuda
     suffix: seqdensecuda
     args: -mat_type seqdensecuda -rhs_mat_type seqdensecuda -ldl {{0 1}}
