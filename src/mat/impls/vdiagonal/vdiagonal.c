...
static PetscErrorCode MatVecDiagonalValidateInverseDiagonal(Mat A)
{
  Mat_VecDiagonal *ctx = (Mat_VecDiagonal *)A->data;
  PetscFunctionBegin;
...
/*@
  MatVecDiagonalGetInverseDiagonal - Get the inverse diagonal of a `MATVECDIAGONAL`
...
. inv_diag - the `Vec` that defines the inverse diagonal
  ...
  using `MatVecDiagonalGetInverseDiagonal()` and `MatVecDiagonalRestoreInverseDiagonal()` avoids copies
  and avoids any call to `VecReciprocal()`.

.seealso: [](chapter_matrices), `MATVECDIAGONAL`, `MatCreateVecDiagonal()`, `MatVecDiagonalRestoreInverseDiagonal()`, `MatVecDiagonalGetDiagonal()`, `MATLMVMBROYDEN`, `MatSolve()`
@*/
PetscErrorCode MatVecDiagonalGetInverseDiagonal(Mat A, Vec *inv_diag)
{
  PetscFunctionBegin;
  PetscValidHeaderSpecific(A, MAT_CLASSID, 1);
...
static PetscErrorCode MatVecDiagonalGetInverseDiagonal_VecDiagonal(Mat A, Vec *inv_diag)
{
  Mat_VecDiagonal *ctx = (Mat_VecDiagonal *)A->data;

  PetscFunctionBegin;
  PetscCall(MatVecDiagonalValidateInverseDiagonal(A));
  *inv_diag = ctx->inv_diag;
  PetscFunctionReturn(PETSC_SUCCESS);
}

/*@
  MatVecDiagonalRestoreInverseDiagonal - Restore the inverse diagonal of a `MATVECDIAGONAL`

  Input Parameters:
+ A - the `MATVECDIAGONAL`
- inv_diag - the `Vec` obtained from `MatVecDiagonalGetInverseDiagonal()`

  Level: developer

.seealso: [](chapter_matrices), `MATVECDIAGONAL`, `MatCreateVecDiagonal()`, `MatVecDiagonalGetInverseDiagonal()`
@*/
PetscErrorCode MatVecDiagonalRestoreInverseDiagonal(Mat A, Vec *inv_diag)
{
  PetscFunctionBegin;
  PetscValidHeaderSpecific(A, MAT_CLASSID, 1);
  PetscValidPointer(inv_diag, 2);
...
static PetscErrorCode MatDestroy_VecDiagonal(Mat mat)
{
  ...
  PetscCall(PetscObjectComposeFunction((PetscObject)mat, "MatVecDiagonalGetInverseDiagonal_C", NULL));
  PetscCall(PetscObjectComposeFunction((PetscObject)mat, "MatVecDiagonalRestoreInverseDiagonal_C", NULL));
...
PetscErrorCode MatSolve_VecDiagonal(Mat matin, Vec b, Vec x)
{
  ...
  PetscFunctionBegin;
  PetscCall(MatVecDiagonalValidateInverseDiagonal(matin));
...
PETSC_INTERN PetscErrorCode MatCreate_VecDiagonal(Mat A)
{
  ...
  PetscCall(PetscObjectComposeFunction((PetscObject)A, "MatVecDiagonalGetInverseDiagonal_C", MatVecDiagonalGetInverseDiagonal_VecDiagonal));
  PetscCall(PetscObjectComposeFunction((PetscObject)A, "MatVecDiagonalRestoreInverseDiagonal_C", MatVecDiagonalRestoreInverseDiagonal_VecDiagonal));
