const char help[] = "Test MATVECDIAGONAL";
...
int main(int argc, char **argv)
{
  ...
  PetscCall(MatCreateVecDiagonal(a2, &A));
  PetscCall(MatCreateVecDiagonal(b2, &B));
  ...
  PetscCall(MatVecDiagonalGetInverseDiagonal(A, &A_inv_diag));
  PetscCall(VecSet(A_inv_diag, 2.0));
  PetscCall(MatVecDiagonalRestoreInverseDiagonal(A, &A_inv_diag));
  PetscCall(MatVecDiagonalGetDiagonal(A, &A_diag));
  PetscCall(VecAXPY(a2, -1.0, A_diag));
  PetscCall(MatVecDiagonalRestoreDiagonal(A, &A_diag));
  PetscCall(VecNorm(a2, NORM_INFINITY, &vec_norm));
  PetscCheck(vec_norm < PETSC_SMALL, comm, PETSC_ERR_PLIB, "MatVecDiagonalGetInverse gave unexpected result");
