...
PetscErrorCode MatCreateSubMatrix_MPIAIJ_SameRowColDist(Mat mat,IS isrow,IS iscol,MatReuse call,Mat *submat)
{
  ...
  if (call == MAT_REUSE_MATRIX) {
    ...
    if (BsubN > n) {
      ...
      SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_ARG_WRONGSTATE,"Columns of Bsub (%D) cannot be smaller than B's (%D)",BsubN,asub->B->cmap->N);
    }
...
PetscErrorCode MatMPIAIJSetPreallocationCSR_MPIAIJ(Mat B,const PetscInt Ii[],const PetscInt J[],const PetscScalar v[])
{
  ...
  if (PetscDefined(USE_DEBUG)) {
    for (i=0; i<m; i++) {
      ...
      if (nnz && (JJ[0] < 0)) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_ARG_WRONGSTATE,"Row %D starts with negative column index %D",i,JJ[0]);
