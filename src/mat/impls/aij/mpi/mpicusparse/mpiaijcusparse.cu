...
PetscErrorCode MatMult_MPIAIJCUSPARSE(Mat A,Vec xx,Vec yy)
{
  ...
  if (PetscUnlikely(nt != A->cmap->n)) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_ARG_SIZ,"Incompatible partition of A (%" PetscInt_FMT ") and xx (%" PetscInt_FMT ")",A->cmap->n,nt);
  /* If A is bound to the CPU, the local vector used in the matrix multiplies should also be
   * bound to the CPU. */
