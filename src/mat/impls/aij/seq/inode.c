    ...
    for (i=0, row = 0; i<m; i++) {
      ...
      case 1:
        ...
        if (PetscAbsScalar(ibdiag[cnt]) < zeropivot) {
          ...
          } else SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_MAT_LU_ZRPVT,"Zero pivot on row %" PetscInt_FMT "",row);
        }
      ...
      case 5:
        ...
        break;
      default:
        SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_SUP,"Inode size %" PetscInt_FMT " not supported",sizes[i]);
      }
      cnt += sizes[i]*sizes[i];
      row += sizes[i];
    }
...
static PetscErrorCode MatSeqAIJ_Inode_ResetOps(Mat A)
{
  ...
  if (A->factortype) {
    A->ops->solve = MatSolve_SeqAIJ_inplace;
  }
  PetscFunctionReturn(0);
}
