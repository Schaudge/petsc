...
PetscErrorCode MatSetValuesRow_SeqAIJ(Mat A,PetscInt row,const PetscScalar v[])
{
  ...
  if (A->offloadmask != PETSC_OFFLOAD_NONE && ai[row+1]-ai[row]) A->offloadmask = PETSC_OFFLOAD_CPU;
#endif
  PetscFunctionReturn(0);
}
...
#if defined(PETSC_HAVE_VIENNACL) || defined(PETSC_HAVE_CUDA)
  if (A->offloadmask != PETSC_OFFLOAD_NONE && m && n) A->offloadmask = PETSC_OFFLOAD_CPU;
#endif
  return 0;
}
...
#if defined(PETSC_HAVE_VIENNACL) || defined(PETSC_HAVE_CUDA)
  if (A->offloadmask != PETSC_OFFLOAD_NONE && inserted) A->offloadmask = PETSC_OFFLOAD_CPU;
#endif
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatDestroy_SeqAIJ(Mat A)
{
  ...
  ierr = PetscObjectComposeFunction((PetscObject)A,"MatMatMatMult_transpose_seqaij_seqaij_C",NULL);CHKERRQ(ierr);
#endif
...
#if defined(PETSC_HAVE_VIENNACL) || defined(PETSC_HAVE_CUDA)
  if (inA->offloadmask != PETSC_OFFLOAD_NONE) inA->offloadmask = PETSC_OFFLOAD_CPU;
#endif
  PetscFunctionReturn(0);
}
...
PetscErrorCode MatAXPY_SeqAIJ(Mat Y,PetscScalar a,Mat X,MatStructure str)
{
  ...
  if (str == SAME_NONZERO_PATTERN) {
    ...
    /* the MatAXPY_Basic* subroutines calls MatAssembly, so the matrix on the GPU
       will be updated */
#if defined(PETSC_HAVE_VIENNACL) || defined(PETSC_HAVE_CUDA)
    if (Y->offloadmask != PETSC_OFFLOAD_NONE) {
...
#if defined(PETSC_HAVE_VIENNACL) || defined(PETSC_HAVE_CUDA)
  if (mat->offloadmask != PETSC_OFFLOAD_NONE) mat->offloadmask = PETSC_OFFLOAD_CPU;
#endif
#else
  PetscFunctionBegin;
#endif
...
static struct _MatOps MatOps_Values = { MatSetValues_SeqAIJ,
                                ...
                                /* 89*/ MatMatMult_SeqAIJ_SeqAIJ,
                                        ...
                                        MatPtAPSymbolic_SeqAIJ_SeqAIJ_SparseAxpy,
...
/* ----------------------------------------------------------------*/
PETSC_INTERN PetscErrorCode MatMatMult_SeqDense_SeqAIJ(Mat A,Mat B,MatReuse scall,PetscReal fill,Mat *C)
{
  PetscErrorCode ierr;
...
PETSC_EXTERN PetscErrorCode MatCreate_SeqAIJ(Mat B)
{
  ...
  ierr = PetscObjectComposeFunction((PetscObject)B,"MatPtAP_is_seqaij_C",MatPtAP_IS_XAIJ);CHKERRQ(ierr);
