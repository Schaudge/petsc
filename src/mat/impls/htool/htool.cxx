            ...
            if (std::distance(idxr, it) + A->rmap->n <= nrow) { /* long enough IS to store the local diagonal block? */
              ...
              if (flg) { /* complete local diagonal block in IS? */
                ...
                if (m) {
                  ...
                  if (A->symmetric == PETSC_BOOL3_TRUE || A->property[MAT_SYMPROP_HERMITIAN] == PETSC_BOOL3_TRUE) {
                    ...
                    if (A->property[MAT_SYMPROP_HERMITIAN] == PETSC_BOOL3_TRUE && PetscDefined(USE_COMPLEX)) {
...
static PetscErrorCode MatAssemblyEnd_Htool(Mat A, MatAssemblyType)
{
  ...
  PetscInt                                                    *offset;
  PetscMPIInt                                                  size;
  char                                                         S = PetscDefined(USE_COMPLEX) && A->property[MAT_SYMPROP_HERMITIAN] == PETSC_BOOL3_TRUE ? 'H' : (A->symmetric == PETSC_BOOL3_TRUE ? 'S' : 'N'), uplo = S == 'N' ? 'N' : 'U';
