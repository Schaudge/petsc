...
static PetscErrorCode MatMultTransposeKernel_Nest(Mat A, Vec x, Vec y, PetscBool herm)
{
  ...
  for (j = 0; j < nc; j++) {
    ...
    for (i = 0; i < nr; i++) {
      if (!bA->m[i][j]) continue;
      /* y[j] <- y[j] + (A[i][j])^* * x[i] */
      if (herm) PetscCall(MatMultHermitianTransposeAdd(bA->m[i][j], bx[i], by[j], by[j]));
      /* y[j] <- y[j] + (A[i][j])^T * x[i] */
      else PetscCall(MatMultTransposeAdd(bA->m[i][j], bx[i], by[j], by[j]));
    }
  }
...
{
  PetscFunctionBegin;
  MatMultTransposeKernel_Nest(A, x, y, PETSC_FALSE);
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
{
  PetscFunctionBegin;
  MatMultTransposeKernel_Nest(A, x, y, PETSC_TRUE);
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
static PetscErrorCode MatMultTransposeAddKernel_Nest(Mat A, Vec x, Vec y, Vec z, PetscBool herm)
{
  ...
  for (j = 0; j < nc; j++) {
    ...
    for (i = 0; i < nr; i++) {
      if (!bA->m[i][j]) continue;
      /* z[j] <- y[j] + (A[i][j])^* * x[i] */
      if (herm) PetscCall(MatMultHermitianTransposeAdd(bA->m[i][j], bx[i], bz[j], bz[j]));
      /* z[j] <- y[j] + (A[i][j])^T * x[i] */
      else PetscCall(MatMultTransposeAdd(bA->m[i][j], bx[i], bz[j], bz[j]));
    }
  }
  for (i = 0; i < nr; i++) PetscCall(VecRestoreSubVector(x, bA->isglobal.row[i], &bx[i]));
...
{
  PetscFunctionBegin;
  MatMultTransposeAddKernel_Nest(A, x, y, z, PETSC_FALSE);
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
{
  PetscFunctionBegin;
  MatMultTransposeAddKernel_Nest(A, x, y, z, PETSC_TRUE);
  PetscFunctionReturn(PETSC_SUCCESS);
}
