...
PetscErrorCode MatView_MPIBAIJ_Binary(Mat mat,PetscViewer viewer)
{
  ...
  header[1] = M; header[2] = N;
  ierr = MPI_Allreduce(&nz,&header[3],1,MPIU_INT,MPI_SUM,PetscObjectComm((PetscObject)mat));CHKERRQ(ierr);
...
PetscErrorCode MatLoad_MPIBAIJ_Binary(Mat mat,PetscViewer viewer)
{
  ...
  PetscInt       *rowidxs,*colidxs,rs,cs,ce,cnt;
  PetscScalar    *matvals;
  PetscErrorCode ierr;
