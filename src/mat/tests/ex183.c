...
int main(int argc, char **args)
{
  ...
  if (total_subdomains > size) SETERRQ2(PETSC_COMM_WORLD,PETSC_ERR_ARG_WRONG,"Number of subdomains %" PetscInt_FMT " must not exceed comm size %" PetscInt_FMT "",total_subdomains,size);
  if (total_subdomains < 1 || total_subdomains > size) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_ARG_WRONG,"number of subdomains must be > 0 and <= %" PetscInt_FMT " (comm size), got total_subdomains = %" PetscInt_FMT "",size,total_subdomains);
