...
int main(int argc,char *argv[])
{
  ...
  if (m != (n+2)*bs) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_ARG_SIZ,"size of local form %" PetscInt_FMT ", expected %" PetscInt_FMT "",m,(n+2)*bs);
  ...
  for (i=0; i<n; i++) {
    ...
    case 2:                     /* Linear acoustic wave operator in variables [rho, u], central differences, periodic, timestep 1/alpha */
      ...
      xx[2*i+1] = 0;
      break;
    default: SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_SUP,"not implemented for block size %" PetscInt_FMT "",bs);
    }
  }
