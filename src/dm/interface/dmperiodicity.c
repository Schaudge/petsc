...
PetscErrorCode DMLocalizeCoordinates(DM dm)
{
  ...
  PetscBool    isLocalized, useDG = PETSC_FALSE, useDGGlobal;
  ...
  if (plex) {
    ...
    for (h = 0; h <= maxHeight; h++) {
      PetscCall(DMPlexGetHeightStratum(plex, h, &pStart[h], &pEnd[h]));
  ...
  for (h = 0; h <= maxHeight; h++) {
    ...
    for (c = cStart; c < cEnd; ++c) {
      ...
      PetscCall(DMPlexVecGetClosure(plex, cs, coordinates, c, &dof, &cellCoords));
      ...
      PetscCall(DMPlexVecRestoreClosure(plex, cs, coordinates, c, &dof, &cellCoords));
    }
  }
  ...
  for (h = 0; h <= maxHeight; h++) {
    ...
    for (c = cStart; c < cEnd; ++c) {
      ...
      for (d = 0; d < Nc; ++d) anchor[d] = cellCoords[d];
      for (p = 0; p < dof/Nc; ++p) PetscCall(DMLocalizeCoordinate_Internal(dm, Nc, anchor, &cellCoords[p*Nc], &coordsDG[offDG + p*Nc]));
      PetscCall(DMPlexVecRestoreClosure(plex, cs, coordinates, c, &dof, &cellCoords));
    }
  }
