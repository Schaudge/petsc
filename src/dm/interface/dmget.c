...
PetscErrorCode  DMGetLocalVector(DM dm,Vec *g)
{
  ...
  for (i=0; i<DM_MAX_WORK_VECTORS; i++) {
    if (dm->localin[i]) {
      ...
      *g             = dm->localin[i];
      ...
      ((PetscObject)(*g))->classid = VEC_CLASSID;
  ...
  ((PetscObject)(*g))->cannotcompose = PETSC_TRUE;
alldone:
  for (i=0; i<DM_MAX_WORK_VECTORS; i++) {
    if (!dm->localout[i]) {
      dm->localout[i] = *g;
  ...
  for (j=0; j<DM_MAX_WORK_VECTORS; j++) {
    if (*g == dm->localout[j]) {
      ...
      for (i=0; i<DM_MAX_WORK_VECTORS; i++) {
        if (!dm->localin[i]) {
          dm->localin[i] = *g;
          ((PetscObject)(*g))->classid = PETSCFREEDHEADER;
          goto alldone;
        }
      }
    }
...
PetscErrorCode DMGetNamedGlobalVector(DM dm,const char *name,Vec *X)
{
  ...
  for (link=dm->namedglobal; link; link=link->next) {
    ...
    if (match) {
      ...
      ((PetscObject)(link->X))->classid = VEC_CLASSID;
  ...
  dm->namedglobal = link;
  ((PetscObject)link->X)->cannotcompose = PETSC_TRUE;
...
PetscErrorCode DMRestoreNamedGlobalVector(DM dm,const char *name,Vec *X)
{
  ...
  for (link=dm->namedglobal; link; link=link->next) {
    ...
    if (match) {
      ...
      ((PetscObject)(*X))->classid = PETSCFREEDHEADER;
      *X           = NULL;
      PetscFunctionReturn(0);
    }
  }
