...
PetscErrorCode DMCreateGlobalVector_Section_Private(DM dm,Vec *vec)
{
  ...
  if (PetscUnlikely(localSize%blockSize)) SETERRQ2(PetscObjectComm((PetscObject)dm), PETSC_ERR_ARG_WRONG, "Mismatch between blocksize %" PetscInt_FMT " and local storage size %" PetscInt_FMT, blockSize, localSize);
...
PetscErrorCode DMCreateSectionSubDM(DM dm, PetscInt numFields, const PetscInt fields[], IS *is, DM *subdm)
{
  ...
  if (PetscUnlikely(!section)) SETERRQ(PetscObjectComm((PetscObject)dm), PETSC_ERR_ARG_WRONG, "Must set default section for DM before splitting fields");
  if (PetscUnlikely(!sectionGlobal)) SETERRQ(PetscObjectComm((PetscObject)dm), PETSC_ERR_ARG_WRONG, "Must set default global section for DM before splitting fields");
  ierr = PetscSectionGetNumFields(section, &Nf);CHKERRQ(ierr);
  if (PetscUnlikely(numFields > Nf)) SETERRQ2(PetscObjectComm((PetscObject)dm), PETSC_ERR_ARG_WRONG, "Number of requested fields %" PetscInt_FMT " greater than number of DM fields %" PetscInt_FMT, numFields, Nf);
  if (is) {
      ...
      if (dm->probs[0].fields) {
        ...
        for (d = 0, e = 0; d < dm->Nds && e < (*subdm)->Nds; ++d) {
          ...
          /* Translate DM fields to DS fields */
          {
            ...
            if (PetscUnlikely(!nf)) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_PLIB, "DS cannot be supported on 0 fields");
...
PetscErrorCode DMCreateSectionSuperDM(DM dms[], PetscInt len, IS **is, DM *superdm)
{
  ...
  for (i = 0 ; i < len; ++i) {
    ...
    if (PetscUnlikely(!sections[i])) SETERRQ(comm, PETSC_ERR_ARG_WRONG, "Must set default section for DM before splitting fields");
    if (PetscUnlikely(!sectionGlobals[i])) SETERRQ(comm, PETSC_ERR_ARG_WRONG, "Must set default global section for DM before splitting fields");
    ...
    for (i = 0 ; i < len; startf += Nfs[i], ++i) {
      ...
      for (p = pStart, subOff = 0; p < pEnd; ++p) {
        ...
        if (gdof > 0 && gtdof) {
          ...
          if (PetscUnlikely(end-start != gtdof)) SETERRQ4(PETSC_COMM_SELF, PETSC_ERR_ARG_SIZ, "Invalid number of global dofs %" PetscInt_FMT " != %" PetscInt_FMT " for dm %" PetscInt_FMT " on point %" PetscInt_FMT "", end-start, gtdof, i, p);
