...
static PetscErrorCode NetRPCreateLinearStar_Linearized(NetRP rp, PetscInt vdeg, PetscBool *edgein, Vec U, Vec Rhs, Mat A)
{
  ...
  for (i = 1; i < vdeg; i++) {
    ...
    hv       = u[index_hv];
    v        = hv / h;
    eig      = edgein[i] ? ShallowRiemannEig_Left(h, v) : ShallowRiemannEig_Right(h, v); /* replace with RiemannSolver Calls */
  ...
  hv  = u[1];
  v   = hv / h;
  eig = edgein[0] ? ShallowRiemannEig_Left(h, v) : ShallowRiemannEig_Right(h, v); /* replace with RiemannSolver Calls */
