...
static PetscErrorCode PetscFESetUp_Composite(PetscFE fem)
{
  ...
  for (s = 0; s < cmp->numSubelements; ++s) {
    ...
    if (PetscUnlikely(sd != spdim)) SETERRQ3(PetscObjectComm((PetscObject) fem), PETSC_ERR_PLIB, "Subelement %" PetscInt_FMT " has %" PetscInt_FMT " dual basis vectors != %" PetscInt_FMT "", s, sd, spdim);
  }
...
static PetscErrorCode PetscFECreateTabulation_Composite(PetscFE fem, PetscInt npoints, const PetscReal points[], PetscInt K, PetscTabulation T)
{
  ...
  for (p = 0; p < npoints; ++p) {
    ...
    if (PetscUnlikely(s >= cmp->numSubelements)) SETERRQ1(PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Point %" PetscInt_FMT " was not found in any subelement", p);
  }
