...
static PetscErrorCode testOrthogonality(PetscInt dim, PetscInt deg)
{
  ...
  for (i = 0; i < Npoly; i++) {
    for (j = i; j < Npoly; j++) {
      ...
      if (PetscAbsReal(integral - exact) > PETSC_SMALL) SETERRQ5(PETSC_COMM_SELF, PETSC_ERR_PLIB, "<P[%" PetscInt_FMT "], P[%" PetscInt_FMT "]> = %g != delta_{%" PetscInt_FMT ",%" PetscInt_FMT "}", i, j, (double) integral, i, j);
    }
  }
