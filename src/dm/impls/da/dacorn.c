...
PetscErrorCode  DMDASetFieldName(DM da,PetscInt nf,const char name[])
{
  ...
  if (nf < 0 || nf >= dd->w) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_ARG_OUTOFRANGE,"Invalid field number: %" PetscInt_FMT "",nf);
...
PetscErrorCode  DMDASetFieldNames(DM da,const char * const *names)
{
  ...
  if (!dd->fieldname) SETERRQ(PetscObjectComm((PetscObject)da),PETSC_ERR_ORDER,"You should call DMSetUp() first");
  while (names[nf++]) {};
  if (nf != dd->w+1) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_ARG_OUTOFRANGE,"Invalid number of fields %" PetscInt_FMT "",nf-1);
...
PetscErrorCode DMDASetCoordinateName(DM dm,PetscInt nf,const char name[])
{
  ...
  if (nf < 0 || nf >= dm->dim) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_ARG_OUTOFRANGE,"Invalid coordinate number: %" PetscInt_FMT "",nf);
