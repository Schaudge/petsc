...
PetscErrorCode  DMDAGetProcessorSubset(DM da,DMDirection dir,PetscInt gp,MPI_Comm *comm)
{
  ...
  ict  = 0;
  ierr = PetscInfo2(da,"DMDAGetProcessorSubset: dim=%" PetscInt_FMT ", direction=%d, procs: ",da->dim,(int)dir);CHKERRQ(ierr);
  for (i=0; i<size; i++) {
    if (owners[i]) {
      ranks[ict] = i; ict++;
      ierr       = PetscInfo1(da,"%" PetscInt_FMT " ",i);CHKERRQ(ierr);
    }
  }
  ierr = PetscInfo(da,"\n");CHKERRQ(ierr);
...
PetscErrorCode  DMDAGetProcessorSubsets(DM da, DMDirection dir, MPI_Comm *subcomm)
{
  ...
  for (p = 0; p < size; ++p) {
    if (firstPoints[p] == firstPoint) {
      ...
      ierr = PetscInfo1(da, "%" PetscInt_FMT " ", p);CHKERRQ(ierr);
    }
  }
  ierr = PetscInfo(da, "\n");CHKERRQ(ierr);
