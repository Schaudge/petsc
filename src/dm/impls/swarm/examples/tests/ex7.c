...
#if 0
static PetscErrorCode ConfigureTS(TS *ts, DM sw, PetscBool reconfigure, AppCtx user, PetscInt step){
  PetscInt          i;
  ...
  if(reconfigure){ ierr = TSReset(ts);CHKERRQ(ierr); }
  //else{ierr = TSCreate(comm,&ts);CHKERRQ(ierr);}
