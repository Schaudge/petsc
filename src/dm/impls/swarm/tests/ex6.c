...
static PetscErrorCode SetInitialCoordinates(DM dmSw)
{
  ...
  for (p = 0; p < Np; ++p) {
    ...
    if (cells[p].rank != rank) SETERRQ1(PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Point %" PetscInt_FMT " not found in the mesh", part);
