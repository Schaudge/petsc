...
int main(int argc,char **argv)
{
  ...
  PetscCall(PetscOptionsGetBool(NULL,NULL,"-dump_operators",&dump_operators,NULL));
  ...
  if (ctx->dim == 2) {
    PetscCall(DMStagCreateCompatibleDMStag(dm_stokes,2,0,1,0,&ctx->levels[ctx->n_levels-1]->dm_coefficients));
  } else if (ctx->dim == 3) {
    PetscCall(DMStagCreateCompatibleDMStag(dm_stokes,0,2,0,1,&ctx->levels[ctx->n_levels-1]->dm_coefficients));
  } else SETERRQ(PETSC_COMM_WORLD,PETSC_ERR_SUP,"Unsupported dimension: %" PetscInt_FMT,ctx->dim);
  ...
  if (dump_operators) {
    PetscCall(DumpOperator(A,"A"));
    for (PetscInt level=0; level<ctx->n_levels; ++level) {
      if (A_faces && A_faces[level]) {
        char filename[PETSC_MAX_PATH_LEN];

        PetscCall(PetscSNPrintf(filename,sizeof(filename),"A_faces_%" PetscInt_FMT,level));
        PetscCall(DumpOperator(A_faces[level],filename));
      }
    }
    if (build_auxiliary_operator) {
      PetscCall(DumpOperator(S_hat,"S_hat"));
    }
    if (custom_pc_mat) {
      PetscCall(DumpOperator(P,"P"));
    }
  }
...
static PetscErrorCode CreateSystem3d(SystemParameters parameters,Mat *pA,Vec *pRhs)
{
  ...
  if (parameters->include_inverse_visc) {
    PetscCheck(!parameters->faces_only,PetscObjectComm((PetscObject)dm_main),PETSC_ERR_SUP,"Does not make sense with faces only");
    PetscCall(OperatorInsertInverseViscosityPressureTerms(dm_main, dm_coefficients, ctx->levels[level]->coeff, 1.0, A));  // TODO not sure of scale
  }
...
static PetscErrorCode DumpOperator(Mat A, const char *name)
{
  ...
  PetscFunctionBeginUser;
  PetscCall(PetscSNPrintf(filename,PETSC_MAX_PATH_LEN,"%s.pbin",name));
