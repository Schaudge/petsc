...
int main(int argc,char **argv)
{
  ...
  if (dim == 1) {
    PetscCall(DMStagCreate1d(PETSC_COMM_WORLD,DM_BOUNDARY_NONE,4,1,0,DMSTAG_STENCIL_BOX,1,NULL,&dm));
  } else if (dim == 2) {
    PetscCall(DMStagCreate2d(PETSC_COMM_WORLD,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,4,4,PETSC_DECIDE,PETSC_DECIDE,0,1,0,DMSTAG_STENCIL_BOX,1,NULL,NULL,&dm));
  } else if (dim == 3) {
