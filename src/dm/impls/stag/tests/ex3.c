...
int main(int argc,char **argv)
{
  ...
  for (i=0; i<NDMS; ++i) {
    ...
    if (!set || !compatible) SETERRQ1(PetscObjectComm((PetscObject)dms[i]),PETSC_ERR_PLIB,"DM %" PetscInt_FMT " not determined compatible with itself",i);
  }
  ...
  for (i=1; i<=2; ++i) {
    ...
    if (!set || !compatible) SETERRQ2(PetscObjectComm((PetscObject)dms[i]),PETSC_ERR_PLIB,"DM %" PetscInt_FMT " not determined compatible with DM %d",i,0);
  }
  ...
  /* Check expected incompatibility */
  {
    ...
    if (!set || compatible) SETERRQ2(PetscObjectComm((PetscObject)dms[i]),PETSC_ERR_PLIB,"DM %" PetscInt_FMT " not determined incompatible with DM %d",i,0);
  }
