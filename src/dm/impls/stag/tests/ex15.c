...
int main(int argc,char **argv)
{
  ...
  if (dump) {
    PetscViewer viewer;
    PetscCall(PetscViewerBinaryOpen(PetscObjectComm((PetscObject)dm),"matA.pbin",FILE_MODE_WRITE,&viewer));
    PetscCall(MatView(A,viewer));
    PetscCall(PetscViewerDestroy(&viewer));
  }
  if (dump) {
    PetscViewer viewer;
    PetscCall(PetscViewerBinaryOpen(PetscObjectComm((PetscObject)dm),"vecb.pbin",FILE_MODE_WRITE,&viewer));
  ...
  if (dump) {
    PetscViewer viewer;
    PetscCall(PetscViewerBinaryOpen(PetscObjectComm((PetscObject)dm),"vecx.pbin",FILE_MODE_WRITE,&viewer));
