...
static PetscErrorCode DMCreateInterpolation_Stag(DM dmc, DM dmf, Mat *A,Vec *vec)
{
  ...
  for (d=0; d<dim; ++d) PetscCheck(nf[d] == 2*nc[d],PetscObjectComm((PetscObject)dmc),PETSC_ERR_SUP,"No support for fine to coarse ratio other than 2 (it is %" PetscInt_FMT " to %" PetscInt_FMT " in dimension %" PetscInt_FMT ")",nf[d],nc[d],d);
  PetscCall(DMStagGetDOF(dmc,&dofc[0],&dofc[1],&dofc[2],&dofc[3]));
  PetscCall(DMStagGetDOF(dmf,&doff[0],&doff[1],&doff[2],&doff[3]));
  for (d=0; d<dim+1; ++d) PetscCheck(dofc[d] == doff[d],PetscObjectComm((PetscObject)dmc),PETSC_ERR_SUP,"No support for different numbers of dof per stratum between coarse and fine DMStag objects: dof%" PetscInt_FMT " is %" PetscInt_FMT " (fine) but %" PetscInt_FMT "(coarse))",d,doff[d],dofc[d]);
...
static PetscErrorCode DMCreateMatrix_Stag(DM dm,Mat *mat)
{
  ...
  if (!is_aij) {
    PetscCall(PetscObjectBaseTypeCompare((PetscObject)*mat,MATSEQAIJ,&is_aij));
  }
  if (!is_aij) {
    PetscCall(PetscObjectBaseTypeCompare((PetscObject)*mat,MATMPIAIJ,&is_aij));
  }
