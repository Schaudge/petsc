...
PetscErrorCode  DMMoabVecGetArray(DM dm, Vec vec, void* array)
{
  ...
  if (vmoab->is_native_vec) {
    ...
    if (count != dmmoab->nloc + dmmoab->nghost) SETERRQ2(PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Mismatch between local vertices and tag partition for Vec. %" PetscInt_FMT " != %" PetscInt_FMT ".", count, dmmoab->nloc + dmmoab->nghost);
...
#endif
{
  ...
  ierr = PetscSNPrintf(*tag_name, PETSC_MAX_PATH_LEN - 1, "%s_%" PetscInt_FMT "", PVEC_PREFIX, global_n);CHKERRQ(ierr);
