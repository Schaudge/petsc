                                                  ...
                                                  PetscReal *jacobian, PetscReal *ijacobian, PetscReal *volume)
{
  ...
  } else SETERRQ1(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "The number of entity vertices are invalid. Currently only support EDGE2 and EDGE3 basis evaluations in 1-D : %" PetscInt_FMT "", nverts);
                                                  ...
                                                  PetscReal *jacobian, PetscReal *ijacobian, PetscReal *volume)
{
  ...
  } else SETERRQ1(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "The number of entity vertices are invalid. Currently only support QUAD4 and TRI3 basis evaluations in 2-D : %" PetscInt_FMT "", nverts);
  PetscFunctionReturn(0);
}
                                                  ...
                                                  PetscReal *jacobian, PetscReal *ijacobian, PetscReal *volume)
{
  ...
  } else SETERRQ1(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "The number of entity vertices are invalid. Currently only support HEX8 and TET4 basis evaluations in 3-D : %" PetscInt_FMT "", nverts);
  PetscFunctionReturn(0);
}
                                     ...
                                     PetscReal *fe_basis, PetscReal **fe_basis_derivatives)
{
  ...
  if (idim != dim) SETERRQ2(PETSC_COMM_WORLD, PETSC_ERR_ARG_WRONG, "Dimension mismatch: provided (%" PetscInt_FMT ") vs quadrature (%" PetscInt_FMT ")\n",idim,dim);
  ...
  case 3:
    ...
    break;
  default:
    SETERRQ1(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "Invalid dimension; should be in [1,3] : %" PetscInt_FMT "", dim);
  }
  PetscFunctionReturn(0);
}
...
PetscErrorCode DMMoabFEMCreateQuadratureDefault(const PetscInt dim, const PetscInt nverts, PetscQuadrature *quadrature)
{
  ...
  case 2:
    ...
    if (nverts == 3) {
      ...
      } else SETERRQ1(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "Triangle quadrature rules for points 3 and 6 supported; npoints : %" PetscInt_FMT "", npoints);
  ...
  case 3:
    ...
    if (nverts == 4) {
      ...
      if (npoints == 4) { /*  KEAST1, K1,  N=4, O=4 */
        ...
        order = 10;
      } else SETERRQ1(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "Tetrahedral quadrature rules for points 4 and 10 supported; npoints : %" PetscInt_FMT "", npoints);
      ierr = PetscQuadratureCreate(PETSC_COMM_SELF, quadrature);CHKERRQ(ierr);
      ierr = PetscQuadratureSetOrder(*quadrature, order);CHKERRQ(ierr);
      ierr = PetscQuadratureSetData(*quadrature, dim, nc, npoints, x, w);CHKERRQ(ierr);
      /* ierr = PetscDTStroudConicalQuadrature(dim, nc, nverts, 0.0, 1.0, quadrature);CHKERRQ(ierr); */
  ...
  PetscReal *jacobian, PetscReal *ijacobian, PetscReal* dvolume)
{
  ...
  if (dim == 1) {
    ...
    if (nverts == 2) { /* Linear Edge */
      ...
      SETERRQ1(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "The number of 1-D entity vertices are invalid. Currently only support EDGE2 and EDGE3 basis evaluations in 1-D : %" PetscInt_FMT "", nverts);
    }
    ...
    } else SETERRQ1(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "The number of 2-D entity vertices are invalid. Currently only support QUAD4 and TRI3 basis evaluations in 2-D : %" PetscInt_FMT "", nverts);
    ...
    } else SETERRQ1(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "The number of 3-D entity vertices are invalid. Currently only support HEX8 and TET4 basis evaluations in 3-D : %" PetscInt_FMT "", nverts);
