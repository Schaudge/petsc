...
int main(int argc,char **argv)
{
  ...
  ierr = PetscInitialize(&argc,&argv,"",help);if (ierr) return ierr;
  ierr = MPI_Comm_rank(PETSC_COMM_WORLD,&rank);CHKERRQ(ierr);
  ierr = MPI_Comm_size(PETSC_COMM_WORLD,&size);CHKERRQ(ierr);

  /* Register various stages for profiling */
  ierr = PetscLogStageRegister("Wash setup",&stages[0]);CHKERRQ(ierr);
  ierr = PetscLogStageRegister("TS step",&stages[1]);CHKERRQ(ierr);
  ierr = PetscLogStageRegister("Free space",&stages[2]);CHKERRQ(ierr);
  ...
  ierr = PetscCalloc3(nsubnet,&numEdges,nsubnet,&numVertices,nsubnet,&edgelists);CHKERRQ(ierr);

  ierr = DMNetworkCreate(PETSC_COMM_WORLD,&networkdm);CHKERRQ(ierr);
  ierr = PetscObjectSetOptionsPrefix((PetscObject)networkdm,"wash_");CHKERRQ(ierr);
  ierr = DMSetFromOptions(networkdm);CHKERRQ(ierr);
  ...
  ierr = DMNetworkRegisterComponent(networkdm,"riverstruct",sizeof(struct _p_River),&KeyRiver);CHKERRQ(ierr);
  ierr = DMNetworkRegisterComponent(networkdm,"pumpstruct",sizeof(struct _p_Pump),&KeyPump);CHKERRQ(ierr);

  /* Set the wash test case */
  washCase = 0; /* default case */
  ierr = PetscOptionsGetInt(NULL,NULL, "-case", &washCase, NULL);CHKERRQ(ierr);

  /* Read files from a screen (runtime) or from ../cases/master_.inp */
  parseflg = PETSC_FALSE;
  ierr = PetscOptionsGetString(NULL,NULL,"-f",filename0,PETSC_MAX_PATH_LEN,&parseflg);CHKERRQ(ierr);
  ...
  if (parseflg && fp) {
    ...
    ierr = PetscStrcpy(fsmall,"../cases/master_small.inp");CHKERRQ(ierr);
    ierr = PetscStrcpy(fmaster,"../cases/master.inp");CHKERRQ(ierr);
    ierr = PetscStrcpy(filename[0],filename0);CHKERRQ(ierr);

    /* All processes read filename[i], i=0,...,nsubnet-1 */
    if (strcmp(filename[0],fmaster)==0){
      ierr = WashReadInputFile(nsubnet,filename);CHKERRQ(ierr);
  ...
  /* Create wash */
  ierr = WashCreate(PETSC_COMM_WORLD,nsubnet,0,&wash);CHKERRQ(ierr);
  wash->dm = networkdm;
  ...
  for (i=0; i<nsubnet; i++) {
    if (size < nsubnet) {
      ...
      ierr = WashAddSubnet(i,washCase,filename[i],0,wash);CHKERRQ(ierr);
    } else {
      /* proc[i] reads i-th subnetwork input */
      ierr = WashAddSubnet(i,washCase,filename[i],i,wash);CHKERRQ(ierr);
    }
  ...
  for (i=0; i<nsubnet; i++) {
    PetscInt netNum = -1;
    ierr = DMNetworkAddSubnetwork(networkdm,NULL,numEdges[i],edgelists[i],&netNum);CHKERRQ(ierr);
  }
  if (size < 10) {
    ierr = PetscSynchronizedPrintf(PETSC_COMM_WORLD, "[%d] nV %d, nE %d, nsubnet %d\n",rank,numVertices[0],numEdges[0],nsubnet);CHKERRQ(ierr);
    ierr = PetscSynchronizedFlush(PETSC_COMM_WORLD,PETSC_STDOUT);CHKERRQ(ierr);
  }

  /* Set up the network layout */
  ierr = DMNetworkLayoutSetUp(networkdm);CHKERRQ(ierr);
  if (!rank) {ierr = PetscPrintf(PETSC_COMM_SELF,"...DMNetworkLayoutSetUp() is done\n");CHKERRQ(ierr);}
  ...
  PetscOptionsBegin(PETSC_COMM_WORLD,NULL,"Wash options","");
    if (size == 1) { /* Monitor selected rivers graphically */
      ierr = PetscMalloc2(ne,&riv,ne,&riv_tmp);CHKERRQ(ierr);
      nriv = ne;
      ierr = PetscOptionsGetIntArray(NULL,NULL,"-river_monitor",riv_tmp,&nriv,&moni);CHKERRQ(ierr);
      if (moni) {
        moni_q=PETSC_TRUE; moni_h=PETSC_TRUE;
        ...
        ierr = PetscOptionsGetBool(NULL,NULL,"-river_monitor_q",&moni_q,&parseflg);CHKERRQ(ierr);
        if (parseflg && moni_q) moni_h = PETSC_FALSE;
        ierr = PetscOptionsGetBool(NULL,NULL,"-river_monitor_h",&moni_h,&parseflg);CHKERRQ(ierr);
  ...
  for (subnet=0; subnet<nsubnet; subnet++) {
    ...
    if (!rank) {
      if (ne != washsubnet->nriver) {
        printf("[%d] Warning: subnet[%d] ne %d != wash->nriver %d\n",rank,subnet,ne,washsubnet->nriver);
      }
    }

    for (i = 0; i<washsubnet->nriver; i++) {
      /* Add River component to edges, rivers=wash->river, and number of variables */
      ierr = DMNetworkAddComponent(networkdm,edges[i],KeyRiver,&rivers[i],2*rivers[i].ncells);CHKERRQ(ierr);

      if (size == 1 && moni && riv[k]) { /* Add monitor */
        PetscReal ymin,ymax;
        if (moni_q) {
          ymin = wash->QMin; ymax = wash->QMax;
          ierr = PetscOptionsGetReal(NULL,NULL,"-qmin",&ymin,NULL);CHKERRQ(ierr);
          ierr = PetscOptionsGetReal(NULL,NULL,"-qmax",&ymax,NULL);CHKERRQ(ierr);
          ierr = DMNetworkMonitorAdd(monitor,"River Q",edges[i],rivers[i].ncells,0,2,0.0,rivers[i].length,ymin,ymax,PETSC_TRUE);CHKERRQ(ierr);
        }
        if (moni_h) {
          ymin = wash->HMin; ymax = wash->HMax;
          ierr = PetscOptionsGetReal(NULL,NULL,"-hmin",&ymin,NULL);CHKERRQ(ierr);
          ierr = PetscOptionsGetReal(NULL,NULL,"-hmax",&ymax,NULL);CHKERRQ(ierr);
          ierr = DMNetworkMonitorAdd(monitor,"River H",edges[i],rivers[i].ncells,1,2,0.0,rivers[i].length,ymin,ymax,PETSC_TRUE);CHKERRQ(ierr);
        }
      }
      k++;
    }
    ...
    while (i<ne) { /* Add Pump component to the remaining edges ??? */
      ierr = DMNetworkAddComponent(networkdm,edges[i],KeyPump,&pumps[e++],0);CHKERRQ(ierr);
      i++;
    }

    /* Add Junction component to all vertices */
    for (i=0; i<nv; i++) {
      ierr = DMNetworkAddComponent(networkdm,vtx[i],KeyJunction,&junctions[i],2);CHKERRQ(ierr);
    }
  }
  if (size == 1) {
    ierr = PetscFree2(riv,riv_tmp);CHKERRQ(ierr);
  }

  /* Set up DM for use */
  ierr = DMSetUp(networkdm);CHKERRQ(ierr);
  wash->dm = networkdm;
  ierr = WashCleanUp(wash,edgelists);CHKERRQ(ierr);
  ierr = PetscFree3(numEdges,numVertices,edgelists);CHKERRQ(ierr);

  /* Network partitioning and distribution of data */
  ierr = DMNetworkDistribute(&networkdm,0);CHKERRQ(ierr);
  wash->dm = networkdm;
  ierr = PetscOptionsGetBool(NULL,NULL, "-junction_view", &view,NULL);CHKERRQ(ierr);
  if (view) {
    /* Display JunctionType */
    ierr = WashJunctionView(wash);CHKERRQ(ierr);
  }

  if (!rank) {ierr = PetscPrintf(PETSC_COMM_SELF,"...DMNetworkDistribute() is done by nproc %d\n",size);CHKERRQ(ierr);}
  ...
  for (subnet=0; subnet<nsubnet; subnet++) {
    ...
    for (i=0; i<ne; i++) {
      ...
      ierr = RiverSetParameters(river,e);CHKERRQ(ierr);
      ierr = RiverSetUp(river);CHKERRQ(ierr);
      ...
      ierr = DMNetworkGetConnectedVertices(networkdm,e,&cone);CHKERRQ(ierr);  /* get vertexes index */
      vfrom = cone[0];
      vto   = cone[1];

      ierr = DMNetworkGetComponent(networkdm,vfrom,0,&vkey,(void**)&junction,NULL);CHKERRQ(ierr); /* get upsteam elevation */
      elevUs = junction->elev;
      ierr = DMNetworkGetComponent(networkdm,vto,0,&vkey,(void**)&junction,NULL);CHKERRQ(ierr); /* get downstream elevation */
      elevDs = junction->elev;
      ierr = RiverSetElevation(river,elevUs,elevDs);CHKERRQ(ierr); /* set elevation at each cell */
    }
  }
  if (!rank && wash->test_mscale) {ierr = PetscPrintf(PETSC_COMM_SELF,"...dx_min/max: %g, %g\n",dx_min,dx_max);CHKERRQ(ierr);}

  /* Create vectors */
  ierr = WashCreateVecs(wash);CHKERRQ(ierr);
  X = wash->X;
  ierr = VecGetSize(X,&i);CHKERRQ(ierr);
  if (!rank) {ierr = PetscPrintf(PETSC_COMM_SELF,"Total variables %d\n",i);CHKERRQ(ierr);}

  /* Set initial solution */
  ierr = WashSetInitialSolution(networkdm,wash);CHKERRQ(ierr);

  /* Setup solver ts                                       */
  /*-------------------------------------------------------*/
  ierr = TSCreate(PETSC_COMM_WORLD,&ts);CHKERRQ(ierr);
  ierr = WashTSSetUp(wash,ts);CHKERRQ(ierr);
  if (size == 1 && moni) {
  ...
  ierr = TSGetStepNumber(ts,&ntmaxi);CHKERRQ(ierr);
  ierr = TSGetConvergedReason(ts,&reason);CHKERRQ(ierr);
  ierr = PetscPrintf(PETSC_COMM_WORLD,"%s at time %g after %d steps\n",TSConvergedReasons[reason],(double)ftime,ntmaxi);CHKERRQ(ierr);

  /* View solution vector X */
  view = PETSC_FALSE;
  ierr = PetscOptionsGetBool(NULL,NULL, "-X_view", &view,NULL);CHKERRQ(ierr);
  if (view) {
    if (!rank) printf("ts X:\n");
  ...
  /* ------------------------- */
  view = PETSC_FALSE;
  ierr = PetscOptionsGetBool(NULL,NULL, "-wash_view", &view,NULL);CHKERRQ(ierr);
  if (view) {
    ierr = WashVecView(wash,X);CHKERRQ(ierr);
  }
  ierr = PetscLogStagePop();CHKERRQ(ierr);
  ...
  ierr = WashDestroyVecs(wash);CHKERRQ(ierr);
