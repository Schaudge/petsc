...
static inline PetscErrorCode hllRiemannSolution(PetscInt cells,RiverField *xL,RiverField *xR,RiverField *flux)
{
  ...
  ierr = hll(xL,xR,&SL,&SR);CHKERRQ(ierr);

  /* compute left and right fluxes */
  ierr = ComputeFlux(xL,0.0,&fluxL);CHKERRQ(ierr);
  ierr = ComputeFlux(xR,0.0,&fluxR);CHKERRQ(ierr);
...
static inline PetscErrorCode Computebexv(RiverField *x,RiverField *islopeL,RiverField *islopeR,RiverField *bexvL,RiverField *bexvR)
{
  ...
  ierr  = SuperbeeLimiter(r,omega,&delta);CHKERRQ(ierr);
  delta *= 0.5;
  bexvL->h = x->h - delta;
  bexvR->h = x->h + delta;
...
static inline PetscErrorCode MusclHancokScheme(RiverField *xl1,RiverField *x,RiverField *xr1,RiverField *xr2,PetscReal dx,PetscReal dt,RiverField *xL,RiverField *xR)
{
  ...
  ierr = CorrectSlope(&islopeL,tol);CHKERRQ(ierr);
  ierr = CorrectSlope(&islopeR,tol);CHKERRQ(ierr);

  ierr = Computebexv(x,&islopeL,&islopeR,&bexvL,&bexvR);CHKERRQ(ierr);

  ierr = ComputeFlux(&bexvL,tol,&fluxL);CHKERRQ(ierr);
  ierr = ComputeFlux(&bexvR,tol,&fluxR);CHKERRQ(ierr);
  ...
  ierr = Computebexv(xr1,&islopeL,&islopeR,&bexvL,&bexvR);CHKERRQ(ierr);
...
PetscErrorCode RiverSetInitialSolution(PetscInt riverCase,PetscInt riverSubCase,River river,RiverField *x,PetscReal q0,PetscReal h0)
{
  ...
  if (riverCase == -1) { /* intial q and h overwritten from a file */
    for (i=0; i<ncells; i++){ x[i].q = q0; x[i].h = h0;}
...
PetscErrorCode RiverSetUp(River river)
{
  ...
  ierr = DMDACreate1d(river->comm,DM_BOUNDARY_GHOSTED,river->ncells,2,1,NULL,&river->da);CHKERRQ(ierr);
...
PetscErrorCode RiverRHSFunctionLocal(River river,RiverField *x,RiverField *f)
{
  ...
  for (i=0; i<ncells-1; i++){
    ...
    if (i == 0) {
      /* mirror: x[i-1] = x[i+1] */
      ierr = MusclHancokScheme(&x[i+1],&x[i],&x[i+1],&x[i+2],dx,dt,&xL,&xR);CHKERRQ(ierr);
    } else if (i == ncells - 2) {
      /* mirror: x[i+2] = x[i] */
      ierr = MusclHancokScheme(&x[i-1],&x[i],&x[i+1],&x[i],dx,dt,&xL,&xR);CHKERRQ(ierr);
    } else {
      ierr = MusclHancokScheme(&x[i-1],&x[i],&x[i+1],&x[i+2],dx,dt,&xL,&xR);CHKERRQ(ierr);
    }
    ...
    ierr = hllRiemannSolution(i,&xL,&xR,&flux);CHKERRQ(ierr);
...
static inline PetscErrorCode RiverDestroyJacobian(River river)
{
  ...
  if (Jriver) {
    for (i=0; i<3; i++) {
      ierr = MatDestroy(&Jriver[i]);CHKERRQ(ierr);
    }
  }
  ierr = PetscFree(Jriver);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode RiverCreateJacobian(River river,Mat *Jin,Mat *J[])
{
  ...
  ierr = PetscMalloc1(3,&Jriver);CHKERRQ(ierr);
  ...
  ierr = MatCreate(PETSC_COMM_SELF,&J0);CHKERRQ(ierr);
  ierr = MatSetSizes(J0,PETSC_DECIDE,PETSC_DECIDE,M,M);CHKERRQ(ierr);
  ierr = MatSetBlockSize(J0,2);CHKERRQ(ierr);
  ierr = MatSetFromOptions(J0);CHKERRQ(ierr);
  ierr = MatSeqAIJSetPreallocation(J0,2,NULL);CHKERRQ(ierr);
  for (i=0; i<river->ncells; i++) { /* set diagonal entries */
    rows[0] = 2*i; rows[1] = 2*i+1;
    ierr = MatSetValues(J0,2,rows,2,rows,aa,INSERT_VALUES);CHKERRQ(ierr);
  }
  ierr = MatAssemblyBegin(J0,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
  ierr = MatAssemblyEnd(J0,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
  Jriver[0] = J0;

  /* Jacobian for upstream vertex */
  ierr = MatCreate(PETSC_COMM_SELF,&Jriver[1]);CHKERRQ(ierr);
  ierr = MatSetSizes(Jriver[1],PETSC_DECIDE,PETSC_DECIDE,M,2);CHKERRQ(ierr);
  ierr = MatSetFromOptions(Jriver[1]);CHKERRQ(ierr);
  //ierr = MatSetOption(Jriver[1],MAT_STRUCTURE_ONLY,PETSC_TRUE);CHKERRQ(ierr);
  nz[0] = 2; nz[1] = 2;
  rows[0] = 0; rows[1] = 1;
  ierr = MatSeqAIJSetPreallocation(Jriver[1],0,nz);CHKERRQ(ierr);
  ierr = MatSetValues(Jriver[1],2,rows,2,rows,aa,INSERT_VALUES);CHKERRQ(ierr);
  ierr = MatAssemblyBegin(Jriver[1],MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
  ierr = MatAssemblyEnd(Jriver[1],MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);

  /* Jacobian for downstream vertex */
  ierr = MatCreate(PETSC_COMM_SELF,&Jriver[2]);CHKERRQ(ierr);
  ierr = MatSetSizes(Jriver[2],PETSC_DECIDE,PETSC_DECIDE,M,2);CHKERRQ(ierr);
  ierr = MatSetFromOptions(Jriver[2]);CHKERRQ(ierr);
  ...
  cols[0] = 0; cols[1] = 1;
  ierr = MatSeqAIJSetPreallocation(Jriver[2],0,nz);CHKERRQ(ierr);
  ierr = MatSetValues(Jriver[2],2,rows,2,cols,aa,INSERT_VALUES);CHKERRQ(ierr);
  ierr = MatAssemblyBegin(Jriver[2],MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
  ierr = MatAssemblyEnd(Jriver[2],MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
...
PetscErrorCode RiverCleanup(River river)
{
  ...
  ierr = RiverDestroyJacobian(river);CHKERRQ(ierr);
  ierr = PetscFree2(river->z,river->zMax);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode RiverSetElevation(River river,PetscReal zUs,PetscReal zDs)
{
  ...
  PetscFunctionBegin;
  ierr = PetscMalloc2(ncells,&river->z,ncells+1,&river->zMax);CHKERRQ(ierr); /* allocate memory */
...
PetscErrorCode RiverView(Vec X,DM networkdm,Wash wash)
{
  ...
  j1 = 0;
  for (i = Start; i < End; i++) {
    ierr = DMNetworkGetComponent(networkdm,i,0,&key,(void**)&river,NULL);CHKERRQ(ierr);
    nnodes = river->ncells;
  ...
  if (!rank) {
    ...
    printf("    i       Q           H        U \n");
    ierr = VecGetArray(Xq,&q_arr);CHKERRQ(ierr);
    ierr = VecGetArray(Xh,&h_arr);CHKERRQ(ierr);
    for (i=0; i<nx; i++) {
      Q = q_arr[i];
      H = h_arr[i];
      if (H ==0.0){
    ...
    ierr = VecRestoreArray(Xq,&q_arr);CHKERRQ(ierr);
    ierr = VecRestoreArray(Xh,&h_arr);CHKERRQ(ierr);
  }
