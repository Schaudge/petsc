...
PetscErrorCode VecView_Network_Seq_Wash(Wash wash,DM networkdm,Vec X)
{
  ...
  for (e=Start; e<End; e++) {
    ...
    ierr = VecArrayPrint_private_Wash(2*river->ncells,xv+offset);CHKERRQ(ierr);
  }
  ...
  for (v=Start; v<End; v++) {
    ...
    ierr = VecArrayPrint_private_Wash(2,xv+offset);CHKERRQ(ierr);
  }
...
PetscErrorCode WashVecView(Wash wash,Vec v)
{
  ...
  if (size == 1) {
    ierr = VecView_Network_Seq_Wash(wash,dm,v);CHKERRQ(ierr);
  } else {
...
PetscErrorCode WashJunctionView(Wash wash)
{
  ...
  for (subnet=0; subnet<nsubnet; subnet++) {
    ierr = DMNetworkGetSubnetwork(dm,subnet,&nv,&ne,&vtx,&edges);CHKERRQ(ierr);
    for (i=0; i<nv; i++) {
      ierr = DMNetworkGetComponent(dm,vtx[i],0,NULL,(void**)&junction,NULL);CHKERRQ(ierr);
      ierr = DMNetworkGetGlobalVertexIndex(dm,vtx[i],&gv);CHKERRQ(ierr);
