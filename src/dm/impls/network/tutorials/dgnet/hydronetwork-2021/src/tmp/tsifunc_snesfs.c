...
PetscErrorCode WashPostSNESSetUpFieldsplit_River(SNES snes)
{
  ...
  ierr = SNESGetApplicationContext(snes,&wash);CHKERRQ(ierr);
  ...
  ierr = VecGetLocalSize(wash->X,&localXsize);CHKERRQ(ierr);
  
  ierr = PCGetType(pc,&pctype);CHKERRQ(ierr);
  ierr = PetscObjectTypeCompare((PetscObject)pc,PCFIELDSPLIT,&snes_fieldsplit);CHKERRQ(ierr);

  if (snes_fieldsplit) {
    numEdges_river = 0;
    for (e=eStart; e<eEnd; e++) {
      ierr = DMNetworkGetComponent(networkdm,e,0,&type,(void**)&river);CHKERRQ(ierr);
    ...
    ierr = PetscMalloc1(nvar_junc+1,&junc_idx);CHKERRQ(ierr);
    ...
    for (e=eStart; e<eEnd; e++) { /* each edge has only one component, river */
      ...
      ierr = DMNetworkGetVariableGlobalOffset(networkdm,e,&varoffset);CHKERRQ(ierr);   
    ...
    for (v=vStart; v<vEnd; v++) { 
      ierr = DMNetworkIsGhostVertex(networkdm,v,&ghost);CHKERRQ(ierr);
      if (ghost) continue;
      ierr = DMNetworkGetVariableGlobalOffset(networkdm,v,&varoffset);CHKERRQ(ierr);   
      junc_idx[kjunc++] = varoffset;
      junc_idx[kjunc++] = varoffset + 1;      
    }

    ierr = ISCreateGeneral(comm,nvar_junc,junc_idx,PETSC_COPY_VALUES,&juncIs);CHKERRQ(ierr);
    ierr = ISCreateStride(comm,localXsize,junc_idx[0],1,&riverIs);CHKERRQ(ierr);
    ierr = ISSort(juncIs);CHKERRQ(ierr);
    ierr = ISSort(riverIs);CHKERRQ(ierr);

    ierr = PCFieldSplitSetBlockSize(pc,2);CHKERRQ(ierr);
    ierr = PCFieldSplitSetIS(pc, "river", riverIs);CHKERRQ(ierr);
    ierr = PCFieldSplitSetIS(pc, "junction", juncIs);CHKERRQ(ierr);

    ierr = ISDestroy(&juncIs);CHKERRQ(ierr);
    ierr = ISDestroy(&riverIs);CHKERRQ(ierr);
    ierr = PetscFree(junc_idx);CHKERRQ(ierr);
  } 
...
PetscErrorCode WashJuncSNESFuncFieldsplit(SNES snes,Vec X,Vec F,void *ctx)
{
  ...
  ierr = DMGetLocalVector(networkdm,&localXold);CHKERRQ(ierr);
  ierr = DMGlobalToLocalBegin(networkdm,Xold,INSERT_VALUES,localXold);CHKERRQ(ierr);
  ierr = DMGlobalToLocalEnd(networkdm,Xold,INSERT_VALUES,localXold);CHKERRQ(ierr);
  ...
  ierr = DMRestoreLocalVector(networkdm,&localXold);CHKERRQ(ierr);
...
PetscErrorCode WashIFunction(TS ts,PetscReal t,Vec X,Vec Xdot,Vec F,void* ctx)
{
  ...
  for (e=eStart; e<eEnd; e++) {
    ...
    ierr = RiverIFunctionLocal(river,riverxold,riverx,riverxdot,riverf);CHKERRQ(ierr);
...
PetscErrorCode WashJuncSNESFunc(SNES snes,Vec Xjunc,Vec Fjunc,void *ctx)
{
  ...
  ierr = VecDuplicate(Xold,&X);CHKERRQ(ierr);
  ierr = VecDuplicate(Xold,&F);CHKERRQ(ierr);
  
  /* (1) Scatter Xjunc to X */
  ierr = VecSet(X,0.0);CHKERRQ(ierr);
  ierr = VecSet(F,0.0);CHKERRQ(ierr);
  ierr = VecScatterBegin(vscat,Xjunc,X,INSERT_VALUES,SCATTER_REVERSE);CHKERRQ(ierr);
  ierr = VecScatterEnd(vscat,Xjunc,X,INSERT_VALUES,SCATTER_REVERSE);CHKERRQ(ierr);
  ...
  ierr = VecScatterBegin(vscat,F,Fjunc,INSERT_VALUES,SCATTER_FORWARD);CHKERRQ(ierr);
  ierr = VecScatterEnd(vscat,F,Fjunc,INSERT_VALUES,SCATTER_FORWARD);CHKERRQ(ierr);
...
PetscErrorCode WashPostSNESSetUp_River(SNES snes)
{
  ...
  ierr = VecDuplicate(wash->X,&Xtmp);CHKERRQ(ierr); 
  ierr = VecSet(Xtmp,-1.0);CHKERRQ(ierr);

  Xjunc = wash->Xjunc;
  ierr = VecGetOwnershipRange(Xjunc,&rstart_junc,NULL);CHKERRQ(ierr);
  ierr = VecGetOwnershipRange(wash->X,&rstart,NULL);CHKERRQ(ierr);
  
  /* Create Jacobian data structure */
  ierr = WashGetJuncLocalSize(wash,&nvar_junc);CHKERRQ(ierr);
  ierr = MatCreate(comm,&Jac);CHKERRQ(ierr);
  ierr = MatSetSizes(Jac,nvar_junc,nvar_junc,PETSC_DECIDE,PETSC_DECIDE);CHKERRQ(ierr);
  ...
  ierr = PetscCalloc2(nvar_junc,&idx1,4,&zeros);CHKERRQ(ierr);
  ierr = VecGetArray(Xtmp,&xtmp_arr);CHKERRQ(ierr);
  ...
  for (e=eStart; e<eEnd; e++) {
    ...
    rows[0] = kjunc+rstart_junc; rows[1] = kjunc+1+rstart_junc;
    ierr = MatSetValues(Jac,2,rows,2,rows,zeros,INSERT_VALUES);CHKERRQ(ierr);
  ...
  ierr = VecRestoreArray(Xtmp,&xtmp_arr);CHKERRQ(ierr);

  /* localXtmp: localXtmp[loffset] = global index of Xjunc */
  ierr = DMGetLocalVector(networkdm,&localXtmp);CHKERRQ(ierr);
  ...
  ierr = VecGetArray(localXtmp,&xtmp_arr);CHKERRQ(ierr);
  for (e=eStart; e<eEnd; e++) {
    ierr = DMNetworkGetComponent(networkdm,e,0,&type,(void**)&river);CHKERRQ(ierr);
    if (type != wash->keyRiver) continue;
    ierr = DMNetworkGetVariableOffset(networkdm,e,&varoffset);CHKERRQ(ierr);
    ...
    rows[0] = xtmp_arr[varoffset];  rows[1] = xtmp_arr[varoffset] + 1; 
    cols[0] = xtmp_arr[offsetfrom]; cols[1] = xtmp_arr[offsetfrom] + 1; 
    ierr = MatSetValues(Jac,2,rows,2,cols,zeros,INSERT_VALUES);CHKERRQ(ierr);
    ierr = MatSetValues(Jac,2,cols,2,rows,zeros,INSERT_VALUES);CHKERRQ(ierr);

    /* Down stream edge couples with vto: */
    rows[0] = xtmp_arr[varoffset+2*river->ncells-2]; rows[1] = xtmp_arr[varoffset+2*river->ncells-2] + 1;
    cols[0] = xtmp_arr[offsetto]; cols[1] = xtmp_arr[offsetto] + 1; 
    ierr = MatSetValues(Jac,2,rows,2,cols,zeros,INSERT_VALUES);CHKERRQ(ierr);
    ierr = MatSetValues(Jac,2,cols,2,rows,zeros,INSERT_VALUES);CHKERRQ(ierr);
  }
  ierr = VecRestoreArray(localXtmp,&xtmp_arr);CHKERRQ(ierr);
  ierr = MatAssemblyBegin(Jac,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
  ierr = MatAssemblyEnd(Jac,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);

  /* Create IS for vscat */
  ierr = ISCreateGeneral(comm,nvar_junc,idx1,PETSC_COPY_VALUES,&is1);CHKERRQ(ierr);
  ierr = ISCreateStride(comm,nvar_junc,rstart_junc,1,&is2);CHKERRQ(ierr);

  ierr = VecScatterCreate(wash->X,is1,Xjunc,is2,&wash->vscat_junc);CHKERRQ(ierr);
  ierr = ISDestroy(&is1);CHKERRQ(ierr);
  ierr = ISDestroy(&is2);CHKERRQ(ierr);
  ierr = PetscFree2(idx1,zeros);CHKERRQ(ierr);

  /* Create fdcoloring for Jac */
  ierr = MatColoringCreate(Jac,&coloring);CHKERRQ(ierr);
  ierr = MatColoringSetType(coloring,MATCOLORINGSL);CHKERRQ(ierr);
  ierr = MatColoringSetFromOptions(coloring);CHKERRQ(ierr);
  ierr = MatColoringApply(coloring,&iscoloring);CHKERRQ(ierr);
  ierr = MatColoringDestroy(&coloring);CHKERRQ(ierr);

  ierr = MatFDColoringCreate(Jac,iscoloring,&fdcoloring);CHKERRQ(ierr);
  ierr = MatFDColoringSetFunction(fdcoloring,(PetscErrorCode (*)(void))WashJuncSNESFunc,snes);CHKERRQ(ierr);
  ierr = MatFDColoringSetFromOptions(fdcoloring);CHKERRQ(ierr);
  ierr = MatFDColoringSetUp(Jac,iscoloring,fdcoloring);CHKERRQ(ierr);
  ierr = SNESSetJacobian(snes,Jac,Jac,SNESComputeJacobianDefaultColor,fdcoloring);CHKERRQ(ierr);

  ierr = ISColoringDestroy(&iscoloring);CHKERRQ(ierr);
  ierr = MatFDColoringDestroy(&fdcoloring);CHKERRQ(ierr);
  ierr = MatDestroy(&Jac);CHKERRQ(ierr);
  ierr = VecDestroy(&Xtmp);CHKERRQ(ierr);
  ierr = DMRestoreLocalVector(networkdm,&localXtmp);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode TSWashPostStage(TS ts,PetscReal time,PetscInt stageindex,Vec *Y)
{
  ...
  ierr = VecGetArrayRead(Xold,&xarr);CHKERRQ(ierr);
  ierr = DMNetworkGetEdgeRange(networkdm,&eStart,&eEnd);CHKERRQ(ierr);
  for (e=eStart; e<eEnd; e++) {
    ierr = DMNetworkGetComponent(networkdm,e,0,&type,(void**)&river);CHKERRQ(ierr);
    if (type != wash->keyRiver) continue;
    ...
    ierr = DMNetworkGetComponent(networkdm,vfrom,0,&type,(void**)&junction);CHKERRQ(ierr);
    ierr = DMNetworkGetVariableOffset(networkdm,vfrom,&varoffset);CHKERRQ(ierr);
    juncx = (RiverField*)(xarr + varoffset);
    ...
    /* Average down stream */
    ierr = DMNetworkGetComponent(networkdm,vto,0,&type,(void**)&junction);CHKERRQ(ierr);
    ierr = DMNetworkGetVariableOffset(networkdm,vto,&varoffset);CHKERRQ(ierr);
    juncx = (RiverField*)(xarr + varoffset);
    riverx[ncells-1].q = 0.5*(xR.q + juncx[0].q);
    riverx[ncells-1].h = 0.5*(xR.h + juncx[0].h);
  }
  ierr = VecRestoreArrayRead(Xold,&xarr);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
