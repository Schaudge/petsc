...
int main(int argc,char *argv[])
{
  ...
  ctx.comm = comm;
  ctx.cfl  = 0.9; ctx.bctype = FVBC_OUTFLOW; //FVBC_PERIODIC; the "mc" limiter provides smooth result with FVBC_OUTFLOW 
  ctx.xmin = 0; ctx.xmax = 500; //ctx.xmin = -1; ctx.xmax = 1;
  ctx.initial = 0;
  ...
  ierr = DMDACreate1d(comm,DM_BOUNDARY_PERIODIC,100,ctx.physics.dof,2,NULL,&da);CHKERRQ(ierr);
  ...
  ierr = TSSetIJacobian(ts,B,B,TSComputeIJacobianDefaultColor,NULL);CHKERRQ(ierr);
  ierr = TSSetType(ts,TSSSP);CHKERRQ(ierr);
  ierr = TSSetMaxTime(ts,60);CHKERRQ(ierr);
