...
int main(int argc, char **argv)
{
  ...
  ierr = PetscOptionsGetInt(NULL, NULL, "-nstencilpts", &user.nstencilpts, NULL);
  CHKERRQ(ierr);
  ierr = PetscOptionsGetInt(NULL, NULL, "-boundary", &user.boundary, NULL);
  CHKERRQ(ierr);
  ierr = PetscOptionsHasName(NULL, NULL, "-viewJacobian", &user.viewJacobian);
  CHKERRQ(ierr);
  ...
  if (user.nstencilpts == 5) {
    ierr = DMDACreate1d(PETSC_COMM_WORLD, DM_BOUNDARY_NONE, 11, 1, 1, NULL, &da);
    CHKERRQ(ierr);
  }
  ...
  ierr = TSSetDM(ts, da);
  CHKERRQ(ierr);
  ierr = TSSetIFunction(ts, r, FormIFunction, &user);
  CHKERRQ(ierr);
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ierr = FormInitialSolution(u, &user);
  CHKERRQ(ierr);
  ...
  CHKERRQ(ierr);
  Jtype = 0;
  ierr  = PetscOptionsGetInt(NULL, NULL, "-Jtype", &Jtype, NULL);
  CHKERRQ(ierr);
  if (Jtype == 0) { /* use user provided Jacobian evaluation routine */
    ...
    if (Jtype == 1) { /* slow finite difference J; */
      ierr = SNESSetJacobian(snes, Jmf, J, SNESComputeJacobianDefault, NULL);
      CHKERRQ(ierr);
    } else if (Jtype == 2) { /* Use coloring to compute  finite difference J efficiently */
      ierr = SNESSetJacobian(snes, Jmf, J, SNESComputeJacobianDefaultColor, 0);
      CHKERRQ(ierr);
