...
int main(int argc,char **argv)
{
  ...
  ierr = DMDACreate2d(PETSC_COMM_WORLD, DM_BOUNDARY_NONE, DM_BOUNDARY_NONE,DMDA_STENCIL_STAR,4,4,PETSC_DECIDE,PETSC_DECIDE,1,1,0,0,&da);CHKERRQ(ierr);
  ierr = DMSetFromOptions(da);CHKERRQ(ierr);
  ierr = DMSetUp(da);CHKERRQ(ierr);
  ierr = DMDASetUniformCoordinates(da,0,12,0,12,0,0);CHKERRQ(ierr);
  ...
  flg = PETSC_FALSE;
  ierr = PetscOptionsGetBool(NULL,NULL,"-view_x",&flg,NULL);CHKERRQ(ierr);
  if (flg) {
