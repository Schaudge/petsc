...
PetscErrorCode RiverIFunction(TS ts, PetscReal t, Vec X, Vec Xdot, Vec F, void *ctx)
{
  ...
  CHKERRQ(ierr); /* Note: we use Xold, thus an explicit scheme! */

  ierr = DMDAVecGetArrayRead(da, Xold, &xoldarr);
  CHKERRQ(ierr);
  ierr = DMDAVecGetArrayRead(da, X, &xarr);
  CHKERRQ(ierr);
  ierr = DMDAVecGetArray(da, Xdot, &xdotarr);
  CHKERRQ(ierr);
  ierr = DMDAVecGetArray(da, F, &farr);
  CHKERRQ(ierr);
  ...
  x    = (RiverField *)xarr;
  f    = (RiverField *)farr;
  ierr = RiverIFunctionLocal(river, xold, x, (RiverField *)xdotarr, f);
  CHKERRQ(ierr);
  ...
  ierr = DMDAVecRestoreArrayRead(da, Xold, &xoldarr);
  CHKERRQ(ierr);
  ierr = DMDAVecRestoreArrayRead(da, X, &xarr);
  CHKERRQ(ierr);
  ierr = DMDAVecRestoreArray(da, Xdot, &xdotarr);
  CHKERRQ(ierr);
  ierr = DMDAVecRestoreArray(da, F, &farr);
  CHKERRQ(ierr);
  //printf("\n t=%g, F:\n",t);
...
static PetscErrorCode RiverSetUp_tsMtlb(River river)
{
  ...
  ierr = DMDACreate1d(river->comm, DM_BOUNDARY_GHOSTED, river->ncells, 2, s, NULL, &river->da);
  CHKERRQ(ierr);
...
int main(int argc, char *argv[])
{
  ...
  CHKERRQ(ierr);
  ierr = PetscOptionsInt("-ts_steps", "Num of time steps", "", t, &ntmaxi, NULL);
  CHKERRQ(ierr);
  ierr = PetscOptionsEnd();
  CHKERRQ(ierr);

  /* Create river */
  ierr = PetscCalloc1(1, &river);
  CHKERRQ(ierr);
  river->comm   = comm;
  river->length = length;
  river->ncells = Mx;
  ...
  /* SetUp river */
  ierr = RiverSetParameters(river, id, gravity);
  CHKERRQ(ierr);
  ierr = RiverSetUp_tsMtlb(river);
  CHKERRQ(ierr);
  ...
  ierr = RiverSetInitialSolution_tsMtlb(river, X);
  CHKERRQ(ierr);
  if (draw & 0x1) {
  ...
  CHKERRQ(ierr);
  ierr = TSSetIFunction(ts, NULL, RiverIFunction, river);
  CHKERRQ(ierr);
  ierr = DMCreateMatrix(da, &J);
  CHKERRQ(ierr);
  ierr = TSSetIJacobian(ts, J, J, TSComputeIJacobianDefaultColor, NULL);
  CHKERRQ(ierr);
  ierr = TSSetExactFinalTime(ts, TS_EXACTFINALTIME_STEPOVER);
  CHKERRQ(ierr);
  ierr = TSSetMaxSteps(ts, ntmaxi);
  CHKERRQ(ierr);
  ...
  for (t = 1; t <= ntmaxi; t++) {
    ...
    CHKERRQ(ierr);
    ierr = RiverGetTimeStep(river, (RiverField *)xarr, &dt);
    CHKERRQ(ierr);
