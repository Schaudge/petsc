...
PetscErrorCode TSDGNetworkCompare(TS ts)
{
  ...
  if (dgnet_nest->numsimulations != 2) {
    SETERRQ1(comm, PETSC_ERR_SUP, "Only valid for 2 simulations. \n \
  There were %i simulations",
             dgnet_nest->numsimulations);
  }
...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  if (size == 1 && dgnet->view) { PetscCall(TSMonitorSet(ts, TSDGNetworkMonitor_Nest, dgnet_nest, NULL)); }
  if (limit) {
