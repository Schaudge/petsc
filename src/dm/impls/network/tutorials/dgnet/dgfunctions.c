...
static PetscReal evalboundary_internal(DGNetwork dgnet, PetscInt field, PetscInt qpoint, PetscReal *comp)
{
  ...
  for (deg = 0; deg <= ndegree; deg++) { eval += comp[deg] * dgnet->LegEvaL_bdry[tab][qpoint * (ndegree + 1) + deg]; }
  return eval;
}
static PetscReal evalquad_internal(DGNetwork dgnet, PetscInt field, PetscInt qpoint, PetscReal *comp)
{
  ...
  for (deg = 0; deg <= ndegree; deg++) { eval += comp[deg] * dgnet->LegEval[tab][qpoint * (ndegree + 1) + deg]; }
  return eval;
}
  ...
  for (e = eStart; e < eEnd; e++) {
    ...
    for (c_fine = cStart_fine; c_fine < cEnd_fine; c_fine++) {
      ...
      for (q = 0; q < quadsize; q++) {
        ...
        for (i = 0; i < dgnet_coarse->tabordersize; i++) { PetscCall(PetscDTLegendreEval(1, &q_coarse, dgnet_coarse->taborder[i] + 1, degs_coarse[i], q_coarse_Eval[i], NULL, NULL)); }
        ...
        for (field = 0; field < dof; field++) {
          ...
          qeval        = 0.0;
          for (deg_coarse = 0; deg_coarse < ndeg_coarse; deg_coarse++) { qeval += q_coarse_Eval[tab_coarse][deg_coarse] * coeff_coarse[deg_coarse]; }
...
PetscErrorCode DGNetworkNormL2(DGNetwork dgnet, Vec X, PetscReal *norm)
{
  ...
  PetscCall(DMNetworkGetEdgeRange(dgnet->network, &eStart, &eEnd));
  for (field = 0; field < dof; field++) { norm[field] = 0.0; }
  ...
  for (e = eStart; e < eEnd; e++) {
    ...
    for (c = cStart; c < cEnd; c++) {
      ...
      for (field = 0; field < dof; field++) { norm[field] += detJ * cellint[field]; }
    }
  }
...
{
  return (PetscSign(a) == PetscSign(b) && PetscSign(b) == PetscSign(c)) ? PetscSign(a) * PetscMin(PetscMin(PetscAbs(a), PetscAbs(b)), PetscAbs(c)) : 0;
}
  ...
  if (limiteractivated) {
    ...
    for (field = 0; field < dof; field++) {
      ...
      for (deg = 0; deg < fielddeg; deg++) {
        for (k = 0; k < dof; k++) { dgnet->charcoeff[k * (dgnet->physics.maxorder + 1) + deg] += dgnet->Rinv[k + field * dof] * uCoeff[fieldoff + deg]; }
      }
    }
    ...
    for (field = 0; field < dof; field++) {
      ...
      for (deg = 0; deg < fielddeg; deg++) {
        ...
        for (k = 0; k < dof; k++) { uCoeff[fieldoff + deg] += dgnet->R[field + k * dof] * dgnet->charcoeff[k * (dgnet->physics.maxorder + 1) + deg]; }
      }
    }
  }
...
PetscErrorCode Limit_1D_onesided(DGNetwork dgnet, const PetscScalar *uL, const PetscScalar *uM, PetscReal *uCoeff, PetscSection sec, PetscInt c, PetscReal jumptol)
{
  ...
  if (limiteractivated) {
    ...
    for (field = 0; field < dof; field++) {
      for (k = 0; k < dof; k++) { dgnet->cuAvg[k] += dgnet->Rinv[k + field * dof] * uM[field]; }
    }
...
PetscErrorCode DGNetlimiter_Nested(TS ts, PetscReal stagetime, PetscInt stageindex, Vec *Y)
{
  ...
  if (nestsize < numsim) {
    SETERRQ(comm, PETSC_ERR_ARG_WRONG, "Vec X must have at least 1 vector per dgnet simulation.\n  \
  Num Simulation: %i \n Num Vectors %i \n ",
            numsim, nestsize);
  }
    ...
    for (c = cStart; c < cEnd; c++) {
      ...
      for (field = 0; field < dof; field++) {
        ...
        for (deg = 0; deg < ndeg; deg++) {
          ...
          for (q = 0; q < quadsize; q++) { *coeff += qweight[q] * dgnet->fluxeval[q * dof + field] * dgnet->LegEvalD[tab][ndeg * q + deg]; }
        }
      }
    }
    /* Flux Time !!! :) */
