...
int main(int argc, char *argv[])
{
  ...
  PetscCall(PetscCalloc1(numvalues, &u));
  for (i = 0; i < numvalues; i++) {
    PetscCall(PetscMalloc1(dgnet->physics.dof, &u[i]));
    for (field = 0; field < dgnet->physics.dof; field++) { PetscCall(PetscRandomGetValueReal(random[field], &u[i][field])); }
  }
  PetscCall(PetscViewerCreate(PETSC_COMM_SELF, &viewer));
  PetscCall(PetscViewerSetType(viewer, PETSCVIEWERASCII));
  PetscCall(PetscViewerSetFromOptions(viewer));
  PetscCall(RiemannSolverTestEigDecomposition(rs, numvalues, (const PetscReal **)u, tol, NULL, NULL, viewer));
  for (i = 0; i < numvalues; i++) { PetscCall(PetscFree(u[i])); }
