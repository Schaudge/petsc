...
PetscErrorCode DMPlexCheckPointSF(DM dm)
{
  ...
  PetscInt       *refRoots;
  PetscInt        cellHeight, cStart, cEnd, nroots, nleaves, nRefRoots, l;
  PetscErrorCode  ierr;

  PetscFunctionBegin;
  PetscValidHeaderSpecific(dm, DM_CLASSID, 1);
  ierr = DMGetPointSF(dm, &sf);CHKERRQ(ierr);
  ierr = PetscSFGetGraph(sf, &nroots, &nleaves, &locals, NULL);CHKERRQ(ierr);
  ...
  for (l = 0; l < nleaves; ++l) {
    ...
    ierr = DMPlexGetCone(dm, point, &cone);CHKERRQ(ierr);
    for (c = 0; c < coneSize; ++c) {
      ierr = PetscFindInt(cone[c], nleaves, locals, &idx);CHKERRQ(ierr); /* try to find cone[c] in leaves */
      if (idx < 0) {
        ierr = PetscFindInt(cone[c], nRefRoots, refRoots, &idx);CHKERRQ(ierr); /* try to find cone[c] in referenced roots */
