...
PetscErrorCode DMPlexGetFieldType_Internal(DM dm, PetscSection section, PetscInt field, PetscInt *sStart, PetscInt *sEnd, PetscViewerVTKFieldType *ft)
{
  ...
  *ft  = PETSC_VTK_INVALID;
  ...
  if (field >= 0) {
    if ((vStart >= pStart) && (vStart < pEnd)) {ierr = PetscSectionGetFieldCount(section, vStart, field, &vcdof[0]);CHKERRQ(ierr);}
    if ((cStart >= pStart) && (cStart < pEnd)) {ierr = PetscSectionGetFieldCount(section, cStart, field, &vcdof[1]);CHKERRQ(ierr);}
  } else {
    if ((vStart >= pStart) && (vStart < pEnd)) {ierr = PetscSectionGetCount(section, vStart, &vcdof[0]);CHKERRQ(ierr);}
    if ((cStart >= pStart) && (cStart < pEnd)) {ierr = PetscSectionGetCount(section, cStart, &vcdof[1]);CHKERRQ(ierr);}
  }
...
PETSC_UNUSED static PetscErrorCode DMPlexView_Ascii_Geometry(DM dm, PetscViewer viewer)
{
  ...
  for (c = cStart; c < cEnd; ++c) {
    ...
    for (cl = 0; cl < closureSize*2; cl += 2) {
      ...
      ierr = PetscSectionGetCount(coordSection, point, &dof);CHKERRQ(ierr);
      if (!dof) continue;
...
static PetscErrorCode DMPlexView_Ascii(DM dm, PetscViewer viewer)
{
  ...
  if (format == PETSC_VIEWER_ASCII_INFO_DETAIL) {
    ...
    for (p = pStart; p < pEnd; ++p) {
      ...
      ierr = PetscSectionGetCount(mesh->coneSection, p, &dof);CHKERRQ(ierr);
    ...
    if (coordSection && coordinates) {
      ...
      for (p = pStart; p < pEnd; ++p) {
        ...
        ierr = PetscViewerASCIISynchronizedPrintf(viewer, "  (%4D) count %2D offset %3D", p, dof, off);CHKERRQ(ierr);
...
\\begin{document}\n");CHKERRQ(ierr);
    ...
    if (plotEdges) {
      ...
      for (e = eStart; e < eEnd; ++e) {
        ...
        ierr = PetscSectionGetCount(coordSection, cone[0], &dof);CHKERRQ(ierr);
...
PetscErrorCode DMCreateMatrix_Plex(DM dm, Mat *J)
{
  ...
  if (!isShell) {
    ...
    for (p = pStart, lsize = 0; p < pEnd; ++p) {
      ...
      ierr = PetscSectionGetCount(sectionGlobal, p, &dof);CHKERRQ(ierr);
      ierr = PetscSectionGetConstraintCount(sectionGlobal, p, &cdof);CHKERRQ(ierr);
      dof  = dof < 0 ? -(dof+1) : dof;
...
PetscErrorCode DMPlexGetConeSize(DM dm, PetscInt p, PetscInt *size)
{
  ...
  ierr = PetscSectionGetCount(mesh->coneSection, p, size);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode DMPlexSetConeSize(DM dm, PetscInt p, PetscInt size)
{
  ...
  PetscFunctionBegin;
  PetscValidHeaderSpecific(dm, DM_CLASSID, 1);
  ierr = PetscSectionSetCount(mesh->coneSection, p, size);CHKERRQ(ierr);
...
PetscErrorCode DMPlexAddConeSize(DM dm, PetscInt p, PetscInt size)
{
  ...
  PetscFunctionBegin;
  PetscValidHeaderSpecific(dm, DM_CLASSID, 1);
  ierr = PetscSectionAddCount(mesh->coneSection, p, size);CHKERRQ(ierr);
  ierr = PetscSectionGetCount(mesh->coneSection, p, &csize);CHKERRQ(ierr);
  ...
  for (d=depth_-1; d>=0; d--) {
    ...
    for (i=0; i<n; i++) {
      ...
      if (arr[i] >= start && arr[i] < end) {
        ...
        ierr = PetscSectionSetCount(sections_[d], i, cn);CHKERRQ(ierr);
      } else {
        ierr = PetscSectionSetCount(sections_[d], i, 1);CHKERRQ(ierr);
      }
    }
    ...
    for (i=0; i<n; i++) {
      ierr = PetscSectionGetCount(sections_[d], i, &cn);CHKERRQ(ierr);
...
PetscErrorCode DMPlexSetConeOrientation(DM dm, PetscInt p, const PetscInt coneOrientation[])
{
  ...
  for (c = 0; c < dof; ++c) {
    ...
    ierr = PetscSectionGetCount(mesh->coneSection, mesh->cones[off+c], &cdof);CHKERRQ(ierr);
...
PetscErrorCode DMPlexGetSupportSize(DM dm, PetscInt p, PetscInt *size)
{
  ...
  PetscValidHeaderSpecific(dm, DM_CLASSID, 1);
  PetscValidPointer(size, 3);
  ierr = PetscSectionGetCount(mesh->supportSection, p, size);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode DMPlexSetSupportSize(DM dm, PetscInt p, PetscInt size)
{
  ...
  PetscFunctionBegin;
  PetscValidHeaderSpecific(dm, DM_CLASSID, 1);
  ierr = PetscSectionSetCount(mesh->supportSection, p, size);CHKERRQ(ierr);
...
PetscErrorCode DMPlexSymmetrize(DM dm)
{
  ...
  for (p = pStart; p < pEnd; ++p) {
    ...
    ierr = PetscSectionGetOffset(mesh->coneSection, p, &off);CHKERRQ(ierr);
    for (c = off; c < off+dof; ++c) {
      ierr = PetscSectionAddCount(mesh->supportSection, mesh->cones[c], 1);CHKERRQ(ierr);
    }
  }
  for (p = pStart; p < pEnd; ++p) {
    PetscInt dof;
...
PetscErrorCode DMPlexGetJoin(DM dm, PetscInt numPoints, const PetscInt points[], PetscInt *numCoveredPoints, const PetscInt **coveredPoints)
{
  ...
  ierr = PetscSectionGetCount(mesh->supportSection, points[0], &dof);CHKERRQ(ierr);
  ...
  for (p = 1; p < numPoints; ++p) {
    ...
    ierr = PetscSectionGetCount(mesh->supportSection, points[p], &dof);CHKERRQ(ierr);
...
PetscErrorCode DMPlexGetMeet(DM dm, PetscInt numPoints, const PetscInt points[], PetscInt *numCoveringPoints, const PetscInt **coveringPoints)
{
  ...
  ierr = PetscSectionGetCount(mesh->coneSection, points[0], &dof);CHKERRQ(ierr);
  ...
  for (p = 1; p < numPoints; ++p) {
    ...
    ierr = PetscSectionGetCount(mesh->coneSection, points[p], &dof);CHKERRQ(ierr);
...
static PetscErrorCode PetscSectionFieldGetTensorDegree_Private(PetscSection section,PetscInt field,PetscInt line,PetscBool vertexchart,PetscInt *Nc,PetscInt *k)
{
  ...
  if (line < 0) {
    ...
    *k = 1;
    ...
    ierr = PetscSectionGetFieldCount(section, line, field, k);CHKERRQ(ierr);
    *k = *k / *Nc + 1;
  }
  PetscFunctionReturn(0);
}
...
PETSC_STATIC_INLINE PetscErrorCode DMPlexVecGetClosure_Depth1_Static(DM dm, PetscSection section, Vec v, PetscInt point, PetscInt *csize, PetscScalar *values[])
{
  ...
  if (!values || !*values) {
    ...
    for (p = 0; p < numPoints; ++p) {
      ...
      ierr = PetscSectionGetCount(section, cp, &dof);CHKERRQ(ierr);
      size += dof;
    }
    if (!values) {
      if (csize) *csize = size;
...
PetscErrorCode DMPlexGetCompressedClosure(DM dm, PetscSection section, PetscInt point, PetscInt *numPoints, PetscInt **points, PetscSection *clSec, IS *clPoints, const PetscInt **clp)
{
  ...
  if (*clPoints) {
    ...
    ierr = PetscSectionGetCount(*clSec, point, &dof);CHKERRQ(ierr);
  ...
  *size = 0;
  for (f = 0; f < numFields; ++f) {
    ...
    for (p = 0; p < numPoints; p++) {
      ...
      ierr = PetscSectionGetFieldCount(section, point, f, &fdof);CHKERRQ(ierr);
...
PetscErrorCode DMPlexVecGetClosureAtDepth_Internal(DM dm, PetscSection section, Vec v, PetscInt point, PetscInt depth, PetscInt *csize, PetscScalar *values[])
{
  ...
  for (clsize=0,p=0; p<Np; p++) {
    PetscInt dof;
    ierr = PetscSectionGetCount(section, points[2*p], &dof);CHKERRQ(ierr);
    clsize += dof;
  }
...
PETSC_STATIC_INLINE PetscErrorCode updatePoint_private(PetscSection section, PetscInt point, PetscInt dof, void (*fuse)(PetscScalar*, PetscScalar), PetscBool setBC, const PetscInt perm[], const PetscScalar flip[], const PetscInt clperm[], const PetscScalar values[], PetscInt offset, PetscScalar array[])
{
  ...
  PetscFunctionBegin;
  ierr = PetscSectionGetConstraintCount(section, point, &cdof);CHKERRQ(ierr);
  ierr = PetscSectionGetOffset(section, point, &off);CHKERRQ(ierr);
  a    = &array[off];
  if (!cdof || setBC) {
    if (clperm) {
...
PETSC_STATIC_INLINE PetscErrorCode updatePointFields_private(PetscSection section, PetscInt point, const PetscInt *perm, const PetscScalar *flip, PetscInt f, void (*fuse)(PetscScalar*, PetscScalar), PetscBool setBC, const PetscInt clperm[], const PetscScalar values[], PetscInt *offset, PetscScalar array[])
{
  ...
  PetscFunctionBegin;
  ierr = PetscSectionGetFieldCount(section, point, f, &fdof);CHKERRQ(ierr);
  ierr = PetscSectionGetFieldConstraintCount(section, point, f, &fcdof);CHKERRQ(ierr);
  ierr = PetscSectionGetFieldOffset(section, point, f, &foff);CHKERRQ(ierr);
  a    = &array[foff];
  if (!fcdof || setBC) {
    if (clperm) {
...
PETSC_STATIC_INLINE PetscErrorCode DMPlexVecSetClosure_Depth1_Static(DM dm, PetscSection section, Vec v, PetscInt point, const PetscScalar values[], InsertMode mode)
{
  ...
  for (p = 0, off = 0; p <= numPoints; ++p, off += dof) {
    ...
    /* ADD_VALUES */
    {
      ...
      ierr = PetscSectionGetConstraintCount(section, cp, &cdof);CHKERRQ(ierr);
...
PetscErrorCode DMPlexVecSetFieldClosure_Internal(DM dm, PetscSection section, Vec v, PetscBool fieldActive[], PetscInt point, PetscInt Ncc, const PetscInt comps[], DMLabel label, PetscInt labelId, const PetscScalar values[], InsertMode mode)
{
  ...
  for (f = 0; f < numFields; ++f) {
    ...
    if (!fieldActive[f]) {
      for (p = 0; p < numPoints*2; p += 2) {
        PetscInt fdof;
        ierr = PetscSectionGetFieldCount(section, points[p], f, &fdof);CHKERRQ(ierr);
        offset += fdof;
      }
      continue;
    }
...
PetscErrorCode DMPlexGetIndicesPointFields_Internal(PetscSection section, PetscBool islocal, PetscInt point, PetscInt off, PetscInt foffs[], PetscBool setBC, const PetscInt ***perms, PetscInt permsoff, const PetscInt indperm[], PetscInt indices[])
{
  ...
  for (f = 0, foff = 0; f < numFields; ++f) {
    ...
    ierr = PetscSectionGetFieldCount(section, point, f, &fdof);CHKERRQ(ierr);
    ierr = PetscSectionGetFieldConstraintCount(section, point, f, &cfdof);CHKERRQ(ierr);
    if (!cfdof || setBC) {
      for (b = 0; b < fdof; ++b) {
...
PetscErrorCode DMPlexAnchorsModifyMat(DM dm, PetscSection section, PetscInt numPoints, PetscInt numIndices, const PetscInt points[], const PetscInt ***perms, const PetscScalar values[], PetscInt *outNumPoints, PetscInt *outNumIndices, PetscInt *outPoints[], PetscScalar *outValues[], PetscInt offsets[], PetscBool multiplyLeft)
{
  ...
  if (aSec) {
    ...
    for (p = 0; p < 2*numPoints; p+=2) {
      ...
      ierr = PetscSectionGetCount(section,b,&bSecDof);CHKERRQ(ierr);
      if (!bSecDof) {
        continue;
      }
      if (b >= aStart && b < aEnd) {
        ierr = PetscSectionGetCount(aSec,b,&bDof);CHKERRQ(ierr);
      }
      if (bDof) {
        ...
        for (q = 0; q < bDof; q++) {
          ...
          for (f = 0; f < numFields; ++f) {
            ...
            ierr             = PetscSectionGetFieldCount(section, a, f, &fDof);CHKERRQ(ierr);
            newOffsets[f+1] += fDof;
          }
        }
      }
        ...
        for (f = 0; f < numFields; ++f) {
          ...
          ierr = PetscSectionGetFieldCount(section, b, f, &fDof);CHKERRQ(ierr);
          newOffsets[f+1] += fDof;
        }
      }
    }
      ...
      if (bDof) {
        for (f = 0; f < numFields; f++) {
          ...
          for (q = 0; q < bDof; q++) {
            ...
            ierr     = PetscSectionGetFieldCount(section, a, f, &aFDof);CHKERRQ(ierr);
            allFDof += aFDof;
          }
  ...
  ierr = PetscSectionGetMaxCount(aSec, &maxAnchor);CHKERRQ(ierr);
    ...
    for (p = 0, newP = 0; p < numPoints; p++) {
      ...
      if (bDof) {
        ...
        for (f = 0; f < numFields; f++) {
          ...
          ierr        = PetscSectionGetFieldCount(cSec, b, f, &fDof);CHKERRQ(ierr);
    ...
    if (numFields) {
      for (f = 0; f < numFields; f++) {
        ...
        for (p = 0; p < numPoints; p++) {
          ...
          ierr = PetscSectionGetFieldCount(section,b,f,&dof);CHKERRQ(ierr);
          if (!dof) {
            continue;
          }
      ...
      for (p = 0; p < numPoints; p++) {
        ...
        ierr = PetscSectionGetCount(section,b,&dof);CHKERRQ(ierr);
        if (!dof) {
          continue;
        }
  ...
  for (f = 0; f < PetscMax(1, Nf); ++f) {
    ...
    if (values && flips[f]) {
      ...
      for (p = 0; p < Ncl; ++p) {
        ...
        if (!Nf) {ierr = PetscSectionGetCount(section, pnt, &fdof);CHKERRQ(ierr);}
        else     {ierr = PetscSectionGetFieldCount(section, pnt, f, &fdof);CHKERRQ(ierr);}
        if (flip) {
          PetscInt i, j, k;
...
PetscErrorCode DMPlexMatSetClosureRefined(DM dmf, PetscSection fsection, PetscSection globalFSection, DM dmc, PetscSection csection, PetscSection globalCSection, Mat A, PetscInt point, const PetscScalar values[], InsertMode mode)
{
  ...
  for (p = 0, numCIndices = 0; p < numCPoints*2; p += 2) {
    ...
    ierr = PetscSectionGetCount(csection, cpoints[p], &dof);CHKERRQ(ierr);
    if (!dof) continue;
    for (f = 0; f < numFields; ++f) {
      ierr           = PetscSectionGetFieldCount(csection, cpoints[p], f, &fdof);CHKERRQ(ierr);
      coffsets[f+1] += fdof;
    }
    numCIndices += dof;
  }
  ...
  for (r = 0, q = 0; r < numSubcells; ++r) {
    ...
    for (p = 0; p < numFPoints*2; p += 2) {
      if ((fpoints[p] >= pStart) && (fpoints[p] < pEnd)) {
        ierr = PetscSectionGetCount(fsection, fpoints[p], &dof);CHKERRQ(ierr);
        if (!dof) continue;
  ...
  for (p = 0, numFIndices = 0; p < numFPoints*2; p += 2) {
    ...
    ierr = PetscSectionGetCount(fsection, ftotpoints[p], &dof);CHKERRQ(ierr);
    if (!dof) continue;
    for (f = 0; f < numFields; ++f) {
      ierr           = PetscSectionGetFieldCount(fsection, ftotpoints[p], f, &fdof);CHKERRQ(ierr);
      foffsets[f+1] += fdof;
    }
    numFIndices += dof;
  }
...
PetscErrorCode DMPlexCreateNumbering_Plex(DM dm, PetscInt pStart, PetscInt pEnd, PetscInt shift, PetscInt *globalSize, PetscSF sf, IS *numbering)
{
  ...
  for (p = pStart; p < pEnd; ++p) {
    ierr = PetscSectionSetCount(section, p, 1);CHKERRQ(ierr);
  }
  ...
  if (PetscUnlikelyDebug(anchorIS && anchorSection)) {
    ...
    for (a = 0; a < size; a++) {
      ...
      if (p >= pStart && p < pEnd) {
        ...
        ierr = PetscSectionGetCount(anchorSection,p,&dof);CHKERRQ(ierr);
        if (dof) {
          PetscErrorCode ierr2;
...
static PetscErrorCode DMPlexCreateConstraintSection_Anchors(DM dm, PetscSection section, PetscSection *cSec)
{
  ...
  for (p = pStart; p < pEnd; p++) {
    ...
    if (dof) {
      ...
      ierr = PetscSectionSetCount(*cSec,p,dof);CHKERRQ(ierr);
      for (f = 0; f < numFields; f++) {
        ierr = PetscSectionGetFieldCount(section,p,f,&dof);CHKERRQ(ierr);
        ierr = PetscSectionSetFieldCount(*cSec,p,f,dof);CHKERRQ(ierr);
      }
    }
  }
...
static PetscErrorCode DMPlexCreateConstraintMatrix_Anchors(DM dm, PetscSection section, PetscSection cSec, Mat *cMat)
{
  ...
  for (p = pStart; p < pEnd; p++) {
    ...
    ierr = PetscSectionGetCount(aSec,p,&rDof);CHKERRQ(ierr);
    if (!rDof) continue;
    ...
    if (numFields) {
      for (f = 0; f < numFields; f++) {
        ...
        for (r = 0; r < rDof; r++) {
          ...
          ierr = PetscSectionGetFieldCount(section,a,f,&aDof);CHKERRQ(ierr);
          annz += aDof;
        }
        ierr = PetscSectionGetFieldCount(cSec,p,f,&dof);CHKERRQ(ierr);
...
PetscErrorCode DMCreateSubDomainDM_Plex(DM dm, DMLabel label, PetscInt value, IS *is, DM *subdm)
{
  ...
  if (is) {
    ...
    for (p = pStart; p < pEnd; ++p) {
      ...
      if (gdof > 0) {
        for (f = 0; f < Nf; ++f) {
          ...
          ierr     = PetscSectionGetFieldCount(subsection, p, f, &fdof);CHKERRQ(ierr);
          ierr     = PetscSectionGetFieldConstraintCount(subsection, p, f, &fcdof);CHKERRQ(ierr);
          pSubSize += fdof-fcdof;
        }
        subSize += pSubSize;
        if (pSubSize) {
    ...
    for (p = pStart; p < pEnd; ++p) {
      ...
      ierr = PetscSectionGetCount(subsectionGlobal, p, &gdof);CHKERRQ(ierr);
      if (gdof > 0) {
