...
PetscErrorCode DMPlexCreateExodus(MPI_Comm comm, PetscInt exoid, PetscBool interpolate, DM *dm)
{
  ...
  if (rank == 0 && interpolate && (num_fs > 0)) {
    ...
    for (fs = 0; fs < num_fs; ++fs) {
      ...
      if (checkReserved) PetscCheck(fs_id[fs] != 1 && fs_id[fs] != 2, comm, PETSC_ERR_ARG_WRONG, "Side set %s marker cannot be %d since this is reserved by ExodusII", fs_name, fs_id[fs]);
