...
PetscErrorCode DMPlexCreatePartitionerGraph_ViaMat(DM dm, PetscInt height, PetscInt *numVertices, PetscInt **offsets, PetscInt **adjacency, IS *globalNumbering)
{
  Mat            conn, CSR;
  IS             fis, cis, cis_own;
  PetscSF        sfPoint;
...
PetscErrorCode PetscPartitionerPartition(PetscPartitioner part, DM dm, PetscSection partSection, IS *partition)
{
  ...
  if (part->height == 0) {
    ...
    if (usemat) {
      ierr = DMPlexCreatePartitionerGraph_ViaMat(dm, 0, &numVertices, &start, &adjacency, &globalNumbering);CHKERRQ(ierr);
    } else {
