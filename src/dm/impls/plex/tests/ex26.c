...
int main(int argc, char **argv) {
  ...
  if ((order > 2) || (order < 1)) SETERRQ1(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "Unsupported polynomial order %" PetscInt_FMT " not in [1, 2]", order);
  ...
  /* Create the exodus result file */
  {
    ...
    */
    ...
    case 3:
      ...
      break;
    default: SETERRQ1(PetscObjectComm((PetscObject) dm), PETSC_ERR_ARG_OUTOFRANGE, "No layout for dimension %" PetscInt_FMT "", sdim);
    }
  ...
  for (set = 0; set < numCS; set++) {
    ...
    if (numCells > 0) {
      ...
      switch (closureSize) {
        ...
        break;
        default: SETERRQ1(PETSC_COMM_SELF, PETSC_ERR_ARG_INCOMP, "Unknown element with closure size %" PetscInt_FMT "\n", closureSize);
      }
