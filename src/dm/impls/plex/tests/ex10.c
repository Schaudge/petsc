...
PetscErrorCode TestReordering(DM dm, AppCtx *user)
{
  ...
  if (pbw > bw) {
    ierr = PetscPrintf(PetscObjectComm((PetscObject) dm), "Ordering method %s increased bandwidth from %" PetscInt_FMT " to %" PetscInt_FMT "\n", order, bw, pbw);CHKERRQ(ierr);
  } else {
    ierr = PetscPrintf(PetscObjectComm((PetscObject) dm), "Ordering method %s reduced bandwidth from %" PetscInt_FMT " to %" PetscInt_FMT "\n", order, bw, pbw);CHKERRQ(ierr);
  }
  PetscFunctionReturn(0);
}
...
PetscErrorCode CreateGroupLabel(DM dm, PetscInt numGroups, DMLabel *label, AppCtx *options)
{
  ...
  if (numGroups != 2) SETERRQ1(PETSC_COMM_SELF, PETSC_ERR_ARG_OUTOFRANGE, "Test only coded for 2 groups, not %" PetscInt_FMT "", numGroups);
...
int main(int argc, char **argv)
{
  ...
  ierr = DMGetDimension(dm, &dim);CHKERRQ(ierr);
  {
    ...
    if (flg && (len != (dim+1) * PetscMax(1, user.numFields))) SETERRQ2(PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Length of dof array is %" PetscInt_FMT " should be %" PetscInt_FMT "", len, (dim+1) * PetscMax(1, user.numFields));
    ierr = PetscOptionsEnd();CHKERRQ(ierr);
  }
  if (user.numGroups < 1) {
