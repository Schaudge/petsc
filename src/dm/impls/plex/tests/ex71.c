static char help[] = "Test for Disjoint Geometric Union of DMPlexes \n\n";

#include <petscdmplex.h>
#include <petscds.h>


/* geometric transformations needed for testing */

/* constant shift of all coordinates by constant(s) stored in the geometric DS */
static void f0_shift(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                     ...
                     PetscReal t, const PetscReal x[], PetscInt numConstants, const PetscScalar constants[], PetscScalar coords[])
{
  ...
  for (c = 0; c < Nc; ++c) {
    coords[c] = u[c] + constants[c];
  }
}

/* Copied from ex33.c. Used to generate some higher order meshes to test from */
...
PetscErrorCode DMPlexShiftGeometry(DM dm, PetscReal shift[]) {
  DM             cdm;
  PetscDS        cds;
  PetscObject    obj;
  PetscClassId   id;
  ...
  if (id != PETSCFE_CLASSID) {
    ...
    for (v = vStart; v < vEnd; ++v) {
      ...
      for (c = 0; c < dE; ++c) coords[off+c] += shift[c];
    }
    PetscCall(VecRestoreArray(lCoords, &coords));
  } else {
    PetscCall(PetscDSSetConstants(cds, dE, shift));
    PetscCall(DMPlexRemapGeometry(dm, 0.0, f0_shift));
  }
  PetscFunctionReturn(0);
}

PetscErrorCode UnionTwoRefTrianglesShifted()
{
  DM           dm[2],dmunion;
  PetscReal     shift[2] = {0,3};
  ...
  PetscCall(DMPlexShiftGeometry(dm[1],shift));
...
/* checks 3D geometry */
PetscErrorCode UnionTwoBallsShifted()
{
  DM           dm[2],dmunion;
  PetscReal     shift[2] = {0,3};
  PetscSection unionmapping; 
  ...
  PetscFunctionBegin; 
  PetscCall(DMPlexCreateBallMesh(PETSC_COMM_SELF,3,1,&dm[0]));
  PetscCall(DMPlexCreateBallMesh(PETSC_COMM_SELF,3,1,&dm[1]));
...
/* checks union of different topological dimensions */
PetscErrorCode UnionTwoTopDims()
{
  DM           dm[2],dmunion;
  PetscReal    shift[2] = {0,3};
  PetscSection unionmapping; 
  ...
  PetscFunctionBegin; 
  PetscCall(DMPlexCreateEmbeddedLineMesh(PETSC_COMM_SELF, 2, 1,NULL, NULL, &dm[0]));
  PetscCall(DMPlexCreateEmbeddedLineMesh(PETSC_COMM_SELF, 2, 1,NULL, NULL, &dm[1]));
  PetscCall(DMPlexShiftGeometry(dm[1],shift));
  PetscCall(DMPlexDisjointUnion_Geometric_Section(dm,2,&dmunion,&unionmapping));
...
PetscErrorCode UnionTwoHigherOrderShifted()
{
  DM           dm[2],dmunion;
  PetscReal    shift[2] = {0,4};
  PetscSection unionmapping; 
  AppCtx         user;

  PetscFunctionBegin; 
  PetscCall(ProcessOptions(PETSC_COMM_WORLD, &user));
  PetscCall(CreateMesh(PETSC_COMM_WORLD, &user, &dm[0]));
  PetscCall(CreateMesh(PETSC_COMM_WORLD, &user, &dm[1]));
