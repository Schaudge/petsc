...
static PetscErrorCode AdaptMesh(DM *dm, AppCtx *ctx)
{
  ...
  while (adapt) {
    ...
    for (c = cStart; c < cEnd; ++c) {
      ...
      if (vidx < 0) SETERRQ2(PETSC_COMM_SELF, PETSC_ERR_ARG_OUTOFRANGE, "Value %" PetscInt_FMT " for cell %" PetscInt_FMT " does not exist in label", value, c);
    ...
    if (nAdapt[0]) {
      ierr = PetscInfo2(dmCur, "Adapted mesh, marking %" PetscInt_FMT " cells for refinement, and %" PetscInt_FMT " cells for coarsening\n", nAdapt[0], nAdapt[1]);CHKERRQ(ierr);
