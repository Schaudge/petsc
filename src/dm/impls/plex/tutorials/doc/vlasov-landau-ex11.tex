...
\begin{algorithm}[h!]
\begin{algorithmic}[100]
\STATE{$ N_q = blockDim.x/n\_sub\_blocks$} 
\STATE{$ my\_elem \leftarrow blockIdx.x $} 
\STATE{$ my\_qi  \leftarrow  threadIdx.x\%N_q$} 
\STATE{$ my\_sub\_blk =  threadIdx.x/N_q$}
...
\FOR[fuse inner element and quadrature loop]{$j=ip\_start:ip\_end$}
...
\ENDFOR
\ENDIF
\IF[On master thread in each block]{$threadIdx.x = 0$}
...
\STATE{$\mathbf{A}\left[my\_elem\right] \leftarrow  \mathbf{A}\left[my\_elem\right]  +  \mathcal{D^T}\left(q_i\right) \mathbf{G3}\left[ i \right]  \mathcal{D}\left(q_i\right)  +  \mathcal{D^T}\left(q_i\right) \mathbf{G2}\left[ i \right]  \mathcal{B}\left(q_i\right)$}
\ENDFOR
\ENDIF
...
%\ENDFOR
\end{algorithmic} 
\caption{CUDA kernel; input: $r$, $z$, $w$, $f$, $df$ and $n\_sub\_blocks$;
