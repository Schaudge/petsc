  ...
  if (rank == 0 && faces) {
    ...
    for (f = 0; f < numFaces; f++) {
      ...
      if (cl > 0 && cr > 0) {
        ...
        if (PetscUnlikely(meetSize != 1)) SETERRQ1(PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Could not determine Plex facet for Fluent face %" PetscInt_FMT, f);
        ...
        if (PetscUnlikely(joinSize != 1)) SETERRQ1(PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Could not determine Plex facet for Fluent face %" PetscInt_FMT, f);
