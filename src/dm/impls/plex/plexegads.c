...
PetscErrorCode DMPlex_EGADS_FACE_XYZtoUV_Internal(const PetscScalar coords[], ego obj, const PetscScalar range[], const PetscInt v, const PetscInt dE, PetscScalar paramsV[]) {
  ...
  while (tolr >= target) {
    ...
    PetscCheck(denom != 0.0, PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Singular transform matrix");
    ...
    if (PetscSqrtReal(delta[0] * delta[0] + delta[1] * delta[1]) < target) {
      tolr = obj_old;
      break;
    }
...
PetscErrorCode DMPlexSnapToGeomModel_EGADS_Internal(DM dm, PetscInt p, ego model, PetscInt bodyID, PetscInt faceID, PetscInt edgeID, const PetscScalar mcoords[], PetscScalar gcoords[])
{
  ...
  PetscCheck((params[0] + pTolr >= range[0]) && (params[0] - pTolr <= range[1]), PETSC_COMM_SELF, PETSC_ERR_ARG_OUTOFRANGE, "Point %" PetscInt_FMT " had bad interpolation on t or u", p);
  PetscCheck(Np <= 1 || ((params[1] + pTolr >= range[2]) && (params[1] + pTolr <= range[3])), PETSC_COMM_SELF, PETSC_ERR_ARG_OUTOFRANGE, "Point %" PetscInt_FMT " had bad interpolation on v", p);
...
PetscErrorCode DMPlexEGADSPrintModel_Internal(ego model)
{
  ...
  for (b = 0; b < Nb; ++b) {
    ...
    /* List Topology of Bodies */
    PetscCall(PetscPrintf(PETSC_COMM_SELF, "\n"));
    PetscCall(PetscPrintf(PETSC_COMM_SELF, "      TOPOLOGY DETAILS for body %d\n", b));
...
static PetscErrorCode DMPlexCreateEGADS_Internal(MPI_Comm comm, ego context, ego model, DM *newdm)
{
  ...
  PetscCall(DMPlexAttachEGADS(dm, model, context));
  /* Label points */
...
PetscErrorCode DMPlexCreateEGADSFromFile(MPI_Comm comm, const char filename[], DM *dm)
{
...
#endif
  PetscBool printModel = PETSC_FALSE, tessModel = PETSC_FALSE, newModel = PETSC_FALSE, shapeOpt = PETSC_FALSE;
  ...
  if (rank == 0) {
    ...
    if (shapeOpt) {
      PetscCall(ConvertEGADSModelToAllBSplines(model, &newmodel));
      PetscCall(EG_deleteObject(model));
      model = newmodel;
    }
...
PetscErrorCode DMPlexEGADSModifyGeomModel(DM dm, PetscScalar newCP[], PetscScalar newW[], PetscBool autoInflate, PetscBool saveGeom, const char stpName[]) {
  ...
  PetscFunctionBegin;
#if defined(PETSC_HAVE_EGADS)
  PetscCall(PetscPrintf(PETSC_COMM_SELF, "\n MODIFY_EGADS -- START OF FUNCTION -- \n"));
  ...
  // Initialize vector to store equivalent NODE indices between the 2 geometrys
  ...
  PetscCheck(newNb == 1, PETSC_COMM_SELF, PETSC_ERR_PLIB, "  ERROR :: newNb > 1 || newNb = %d \n", newNb);
  ...
  PetscCall(ISCreateGeneral(PetscObjectComm((PetscObject)dm), totalNumNode + 1, nodeIDEquiv, PETSC_COPY_VALUES, &isNodeID));
  PetscCall(ISCreateGeneral(PetscObjectComm((PetscObject)dm), totalNumEdge + 1, edgeIDEquiv, PETSC_COPY_VALUES, &isEdgeID));
...
. dm - The `DM` object representing the mesh with `PetscContainer` containing an EGADS geometry model
  ...
  Output Parameter:
. dm - The DM object representing the mesh with PetscContainers containing the associated [t] and [u, v] parameter data for associated `DM` points.
...
PetscErrorCode DMPlexGetEGADSGeomModel_tuv(DM dm) {
  ...
  PetscCall(PetscMalloc3(vEnd - vStart, &t_point, vEnd - vStart, &u_point, vEnd - vStart, &v_point));
...
. dm - The DM object representing the mesh with PetscContainer containing an EGADS geometry model
  ...
  Output Parameter:
. dm - The updated DM object inflated to the associated underlying geometry. This updates the [x, y, z] coordinates of DM points associated with geometry.
...
/*@C
  DMPlexComputeSurfaceGradient - Calculate the gradient of mesh coordinates with respect the associated bspline geometry control points and weights.
...
.seealso: `DMPlexCreateEGADS()``
@*/
PetscErrorCode DMPlexComputeSurfaceGradient(DM dm)
{
  ego            model, geom, *bodies, *fobjs;
  PetscContainer modelObj;
  int            oclass, mtype, *senses;
  ...
  for (int ii = 0; ii < Nf; ++ii) {
    ...
    maxNumCPs = PetscMax(maxNumCPs_temp, maxNumCPs);
  }

  PetscInt     cpCoordDataLength = 3 * totalNumCPs;
  PetscInt     wDataLength       = totalNumCPs;
  PetscScalar *cntrlPtCoords;
  PetscScalar *cntrlPtWeights;
