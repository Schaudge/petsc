...
PetscErrorCode DMPlexSnapToGeomModel(DM dm, PetscInt p, PetscInt dE, const PetscScalar mcoords[], PetscScalar gcoords[])
{
...
#ifdef PETSC_HAVE_EGADS
  {
    ...
    PetscCall(PetscOptionsGetBool(NULL, NULL, "-dm_plex_refine_without_snap_to_geom", &dontSnapToGeom, NULL));
...
static PetscErrorCode DMPlexCreateGeom(MPI_Comm comm, ego context, ego model, DM *newdm, PetscBool islite)
{
  ...
  // Embed EGADS model in DM
  {
    ...
    PetscCall(PetscContainerCreate(PETSC_COMM_SELF, &modelObj));
    PetscCall(PetscContainerSetPointer(modelObj, model));
    if (islite) {PetscCall(PetscObjectCompose((PetscObject)dm, "EGADSlite Model", (PetscObject)modelObj));}
    else        {PetscCall(PetscObjectCompose((PetscObject)dm, "EGADS Model", (PetscObject)modelObj));}
    PetscCall(PetscContainerDestroy(&modelObj));
...
PetscErrorCode ConvertGeomModelToAllBSplines(ego model, PetscBool islite)
{
  ...
  for (int ii = 0; ii < Nf; ++ii) {
    ...
    PetscCall(EG_convertToBSpline(face, &bspline));   // Does not have an EGlite_ version
    ...
    context = NULL;
    PetscCall(EG_getContext(face, &context));   // Does not have an EGlite_ version
    ...
    ego newgeom;
    PetscCall(EG_makeGeometry(context, SURFACE, BSPLINE, NULL, bpinfo, bprv, &newgeom));  // Does not have an EGlite_ version
    ...
    ego newface;
    PetscCall(EG_makeFace(newgeom, SFORWARD, data, &newface));  // Does not have an EGlite_ version
  ...
  ego newmodel;
  PetscCall(EG_sewFaces(Nf, newFaces, 0.0, 0, &newmodel));    // Does not have an EGlite_ version
  *model = *newmodel;
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
/*@C
  DMPlexGetGeomModelBodies - Returns an array of PetscGeom BODY objects attached to the referenced geomtric model entity as
