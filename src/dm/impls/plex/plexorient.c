...
PetscErrorCode DMPlexCompareOrientations(PetscInt coneSize, const PetscInt cone[], PetscInt masterConeSize, const PetscInt masterCone[], PetscInt *start, PetscBool *reverse)
{
  PetscInt        i, start_;
  PetscBool       reverse_;
...
PetscErrorCode DMPlexOrientCell(DM dm, PetscInt p, PetscInt masterConeSize, const PetscInt masterCone[])
{
  ...
  if (masterConeSize > coneSize) SETERRQ3(PETSC_COMM_SELF, PETSC_ERR_ARG_OUTOFRANGE, "point p=%D: masterConeSize = %D > %D = coneSize(p)", p, masterConeSize, coneSize);
  if (!coneSize) PetscFunctionReturn(0); /* do nothing for points with no cone */
  if (!masterConeSize) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_ARG_OUTOFRANGE, "masterConeSize cannot be 0 if cone size > 0");
  ierr = DMPlexGetCone(dm, p, &cone);CHKERRQ(ierr);
  ierr = DMPlexCompareOrientations(coneSize, cone, masterConeSize, masterCone, &start1, &reverse1);CHKERRQ(ierr);
