  ...
  for (bc = 0; bc < numBC; ++bc) {
    ...
    for (i = 0; i < n; ++i) {
      ...
      if (cNc > 0) {
        ...
        if (Nf) {
          if (numConst % Nc) SETERRQ3(PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Point %" PetscInt_FMT " has %" PetscInt_FMT " dof which is not divisible by %" PetscInt_FMT " field components", p, numConst, Nc);
