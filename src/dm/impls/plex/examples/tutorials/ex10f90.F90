...
#include <petsc/finclude/petscdmplex.h>
  ...
  ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  dim = 2
  numSpecies = 1 ! default to electrons only
  call DMPlexFPCreateVelocitySpace(PETSC_COMM_SELF, dim, numSpecies, '', X, dm, ierr); CHKERRQ(ierr)
...
!    suffix: 0
!    requires: p4est
!    args: -petscspace_degree 4 -mass_petscspace_degree 4 -petscspace_poly_tensor 1 -mass_petscspace_poly_tensor 1 -dm_type p4est -info -info_exclude null,vec,mat,pc,ksp,snes,p4est,vecscatter,is -ion_masses 2,40 -ion_charges 1,18 -thermal_temps 5,5,.005 -n 1.00018,1,1e-5 -n_0 1e20 -fp_ts_monitor -fp_snes_rtol 1.e-8 -fp_snes_monitor -fp_snes_converged_reason -fp_ts_type arkimex -fp_ts_arkimex_type 1bee -fp_ts_max_snes_failures -1 -fp_ts_rtol 1e-4 -fp_ts_dt 1.e-8 -fp_ts_max_time 1 -fp_ts_adapt_clip .5,1.25 -fp_ts_adapt_scale_solve_failed -fp_ts_adapt_time_step_increase_delay 5 -fp_ts_max_steps 1 -fp_pc_type lu -fp_ksp_type preonly -amr_levels_max 18 -domain_radius 5 -amr_re_levels 0 -re_radius 1
!
!TEST*/
