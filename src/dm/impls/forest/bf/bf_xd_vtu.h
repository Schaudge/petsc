...
static PetscErrorCode DMBFGetVTKVertexCoordinates(DM dm, PetscVTUReal *point_data, PetscInt nPoints)
{
              ...
              for (yi = 0; yi < 2; ++yi) {
                ...
                for (xi = 0; xi < 2; ++xi) {
                  ...
                  if ( !(0 <= l && l < P4EST_CHILDREN) ) {
                    SETERRQ(PETSC_COMM_SELF,PETSC_ERR_ARG_WRONGSTATE,"Index out of bounds: %i, bounds=[0,%i)",(int)l,P4EST_CHILDREN);
                  }
#endif
  ...
  if ((P4EST_CHILDREN*quad_count) != nPoints) {
    SETERRQ(PETSC_COMM_SELF,PETSC_ERR_ARG_WRONGSTATE,"Counts mismatch: %i != %i (nPoints)",(int)(P4EST_CHILDREN*quad_count),(int)nPoints);
  }
...
PetscErrorCode DMBF_XD_VTKWriteAll(PetscObject odm,PetscViewer viewer)
{
  ...
  if(!rank) {
    ...
    ierr = PetscStrncpy(noext, vtk->filename, n + 1);CHKERRQ(ierr);
    ierr = PetscSNPrintf(gfname, sizeof(gfname), "%s.pvtu", noext);CHKERRQ(ierr);
