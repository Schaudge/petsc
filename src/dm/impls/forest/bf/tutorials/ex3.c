...
static PetscErrorCode fill_unknowns_guard_layer_facefn(DM dm, DM_BF_Face *face, void *ctx)
{
  ...
  if (isBoundary) {
    ...
    switch(face->dir) {
      ...
      default: SETERRQ(PETSC_COMM_SELF,PETSC_ERR_ARG_WRONGSTATE,"Unknown face direction %i",face->dir);
    }
    switch(face->dir) {
      case DM_BF_FACEDIR_XNEG:
      case DM_BF_FACEDIR_XPOS:
...
int main(int argc, char **argv)
{
  ...
  // set time step and number of steps
  {
    ...
    ierr = PetscOptionsGetRealArray(PETSC_NULLPTR,PETSC_NULLPTR,"-dm_p4est_brick_bounds",B,&n,&isset);CHKERRQ(ierr);
    ...
    ierr = PetscOptionsGetReal(PETSC_NULLPTR,PETSC_NULLPTR,"-max_time",&max_time,&isset);CHKERRQ(ierr);
