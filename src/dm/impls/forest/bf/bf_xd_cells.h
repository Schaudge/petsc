...
PetscErrorCode DMBF_XD_CellsDestroy(DM dm, DM_BF_XD_Cells *cells)
{
  ...
  PetscFunctionBegin;
  if (cells->mesh)  { ierr = DMBF_XD_P4estMeshDestroy(cells->mesh);CHKERRQ(ierr); }
  if (cells->ghost) { ierr = DMBF_XD_GhostDestroy(cells->ghost);CHKERRQ(ierr); }
                                  ...
                                  /*IN/OUT*/ DM clonedDm)
{
  ...
  PetscFunctionBegin;
  ierr = PetscNew(clonedCells);CHKERRQ(ierr);
                                    ...
                                    /*IN    */ PetscInt minLevel)
{
  ...
  PetscFunctionBegin;
  ierr = PetscNew(coarseCells);CHKERRQ(ierr);
                                   ...
                                   /*IN    */ PetscInt maxLevel)
{
  ...
  PetscFunctionBegin;
  ierr = PetscNew(fineCells);CHKERRQ(ierr);
                                                ...
                                                const DM_BF_Shape *cellMemoryShape)
{
  ...
  PetscFunctionBegin;
  ierr = PetscNew(adapCells);CHKERRQ(ierr);
...
PetscErrorCode DMBF_XD_CellsGetGhost(DM_BF_XD_Cells *cells, void *ghost)
{
  ...
  PetscFunctionBegin;
  if (!cells->ghost) { ierr = DMBF_XD_GhostCreate(cells->p4est,&cells->ghost);CHKERRQ(ierr); }
