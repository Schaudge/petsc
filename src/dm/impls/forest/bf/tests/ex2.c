...
int main(int argc, char **argv)
{
  ...
  // check cell data shapes
  {
    ...
    ierr = DMBFGetCellDataShape(dm,&shapeElements,&n,&d);CHKERRQ(ierr);
    PetscPrintf(PETSC_COMM_WORLD,"[%s] Shape elements dim=(%d,%d)\n",_name,n,d);
    for (i=0; i<n; i++) {
      PetscPrintf(PETSC_COMM_WORLD,"[%s]   %d: ",_name, i);
      for (j=0; j<d; j++) {
        PetscPrintf(PETSC_COMM_WORLD,"%d ",shapeElements[i*d+j]);
      }
      PetscPrintf(PETSC_COMM_WORLD,"\n");
    }
    if (shapeElements) {
      ierr = PetscFree(shapeElements);CHKERRQ(ierr);
    }
  }
  ...
  // print info
  {
    ...
    ierr = DMBFGetInfo(dm,&dim,&n,&N,&ng);CHKERRQ(ierr);
