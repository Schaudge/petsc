...
static PetscErrorCode DMBF_CellsCreate(DM dm)
{
  ...
  if (_p_nCells(dm)) {
    ...
    ierr = DMBFGetInfo(dm,&dim,&n,PETSC_NULLPTR,&ng);CHKERRQ(ierr);
    n_cells  = (size_t)n;
    ng_cells = (size_t)ng;
...
static PetscErrorCode DMBF_CellsSetUpGhost(DM dm)
{
  ...
  ierr = DMBFGetInfo(dm,&dim,&offset_cells,PETSC_NULLPTR,&ng_cells);CHKERRQ(ierr);
...
static PetscErrorCode DMCreateGlobalVector_BF(DM dm, Vec *vec)
{
  ...
  PetscAssertPointer(vec,2);
  /* get number of entries */
  ierr = DMBFGetInfo(dm,&dim,&n,&N,PETSC_NULLPTR);CHKERRQ(ierr);
  ierr = DMBFGetBlockSize(dm,blockSize);CHKERRQ(ierr);
  for (i=0; i<dim; i++) {
    cellDof *= blockSize[i];
  }
...
PetscErrorCode DMBFGetLocalSize(DM dm, PetscInt *nLocal)
{
  ...
  PetscFunctionBegin;
  ierr = DMBFGetInfo(dm,&dim,nLocal,PETSC_NULLPTR,PETSC_NULLPTR);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode DMBFGetGlobalSize(DM dm, PetscInt *nGlobal)
{
  ...
  PetscFunctionBegin;
  ierr = DMBFGetInfo(dm,&dim,PETSC_NULLPTR,nGlobal,PETSC_NULLPTR);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode DMBFGetGhostSize(DM dm, PetscInt *nGhost)
{
  ...
  PetscFunctionBegin;
  ierr = DMBFGetInfo(dm,&dim,PETSC_NULLPTR,PETSC_NULLPTR,nGhost);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode DMBFAMRFlag(DM dm)
{
  ...
  /* set data pointers of all ghost cells */
  ierr = DMBFGetInfo(dm,&dim,&n_cells,PETSC_NULLPTR,PETSC_NULLPTR);CHKERRQ(ierr);
  for (i=0; i<n_cells; i++) {
    cell = _p_getCellPtrFromIndex(bf,i);
