...
static PetscErrorCode DMBF_2D_ConnectivityCreate(DM dm, p4est_connectivity_t **connectivity)
{
  ...
  if (isBrick && dm->setfromoptionscalled) { /* if brick topology with given uptions */
    ...
    PetscCheck(!flgB || nretB == 2 * P4EST_DIM, PetscObjectComm((PetscObject)dm), PETSC_ERR_ARG_SIZ, "Need to give %d bounds in -dm_p4est_brick_bounds, gave %" PetscInt_FMT, 2 * P4EST_DIM, nretB);
    ...
    /* create connectivity */
    PetscCallP4estReturn(*connectivity, p4est_connectivity_new_brick, ((int)N[0], (int)N[1], (P[0] == DM_BOUNDARY_PERIODIC), (P[1] == DM_BOUNDARY_PERIODIC)));
    ...
    /* create connectivity */
    PetscCallP4estReturn(*connectivity, p4est_connectivity_new_byname, ((const char *)topologyName));
  }
