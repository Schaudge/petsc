...
PetscErrorCode DMBFShapeClear(DM_BF_Shape *shape)
{
  ...
  if (shape->list && shape->list[0]) {
    ...
    CHKERRQ(ierr);
    ierr = PetscFree(shape->list);
    CHKERRQ(ierr);
  }
  if (shape->pad) {
    ierr = PetscFree(shape->pad);
    CHKERRQ(ierr);
  }
...
PetscErrorCode DMBFShapeCompare(const DM_BF_Shape *refShape, const DM_BF_Shape *chkShape, PetscBool *similar)
{
  ...
  ierr = DMBFShapeIsSetUp(refShape, &refSetUp);
  CHKERRQ(ierr);
...
PetscErrorCode DMBFShapeCopy(DM_BF_Shape *trgShape, const DM_BF_Shape *srcShape)
{
  ...
  CHKERRQ(ierr);
  ierr = PetscArraycpy(trgShape->pad, srcShape->pad, srcShape->n);
  CHKERRQ(ierr);
...
PetscErrorCode DMBFShapeSet(DM_BF_Shape *shape, const size_t *elements, const size_t *pad)
{
  ...
  CHKERRQ(ierr);
  /* set */
  ierr = PetscArraycpy(shape->list[0], elements, _p_DMBFShapeNElements(shape));
  CHKERRQ(ierr);
  ierr = PetscArraycpy(shape->pad, pad, shape->n);
  CHKERRQ(ierr);
...
PetscErrorCode DMBFShapeGet(const DM_BF_Shape *shape, size_t **elements, size_t **pad, size_t *n, size_t *dim)
{
  ...
  CHKERRQ(ierr);
  /* get */
  ierr = PetscMalloc1(_p_DMBFShapeNElements(shape), elements);
  CHKERRQ(ierr);
  ierr = PetscArraycpy(*elements, shape->list[0], _p_DMBFShapeNElements(shape));
  CHKERRQ(ierr);
  ierr = PetscMalloc1(shape->n, pad);
  CHKERRQ(ierr);
  ierr = PetscArraycpy(*pad, shape->pad, shape->n);
  CHKERRQ(ierr);
  *n   = shape->n;
  *dim = shape->dim;
  PetscFunctionReturn(PETSC_SUCCESS);
