...
PETSC_STATIC_INLINE size_t _p_DMBFShapeNElements(const DM_BF_Shape *shape)
{
  return shape->n*shape->dim;
}

PETSC_STATIC_INLINE size_t _p_DMBFShapeSize(const DM_BF_Shape *shape)
{
  size_t        i, j, s, size=0;
...
PetscErrorCode DMBFShapeIsValid(const DM_BF_Shape *shape, PetscBool *isValid)
{
  PetscFunctionBegin;
  PetscValidPointer(shape,1);
  PetscValidBoolPointer(isValid,2);
  *isValid = PETSC_TRUE;
...
PetscErrorCode DMBFShapeCompare(const DM_BF_Shape *refShape, const DM_BF_Shape *chkShape, PetscBool *similar)
{
  ...
  PetscFunctionBegin;
  PetscValidPointer(refShape,1);
  PetscValidPointer(chkShape,2);
...
PetscErrorCode DMBFShapeSetFromInt(DM_BF_Shape *shape, const PetscInt *elements)
{
  ...
  /* check */
  PetscValidPointer(shape,1);
  PetscValidIntPointer(elements,2);
  ierr = DMBFShapeCheckSetUp(shape);CHKERRQ(ierr);
  /* set */
