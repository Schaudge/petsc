...
static int
_p_coarsen_via_flag (p4est_t *p4est, p4est_topidx_t tree, p4est_quadrant_t *quadrants[])
{
  DM_BF_AmrCtx  *amrCtx = p4est->user_pointer;
  int           k;
...
static int
_p_refine_via_flag (p4est_t *p4est, p4est_topidx_t tree, p4est_quadrant_t *quadrant)
{
  DM_BF_AmrCtx *amrCtx = p4est->user_pointer;
...
PetscErrorCode DMBF_XD_AmrAdaptData(p4est_t *orig_p4est, p4est_t *adap_p4est, DM dm, DM_BF_AmrOps *amrOps)
{
  ...
  PetscFunctionBegin;
  /* check input */
  if (orig_p4est->data_size != adap_p4est->data_size) SETERRQ2(orig_p4est->mpicomm,PETSC_ERR_ARG_SIZ,"p4est data size mismatch: original %d, adapted %d",(int)orig_p4est->data_size,(int)adap_p4est->data_size);
  ...
  while (orig_quadid < orig_n_quads) {
    ...
    if (adap_n_quads <= adap_quadid) SETERRQ2(orig_p4est->mpicomm,PETSC_ERR_PLIB,"Quadrant id %d is larger than the number of quadrants %d",adap_quadid,adap_n_quads);
#endif
    PetscStackCallP4estReturn(orig_quad,p4est_mesh_quadrant_cumulative,(orig_p4est,orig_quadid,NULL,NULL));
    PetscStackCallP4estReturn(adap_quad,p4est_mesh_quadrant_cumulative,(adap_p4est,adap_quadid,NULL,NULL));
  ...
  if (orig_quadid != orig_n_quads) SETERRQ2(orig_p4est->mpicomm,PETSC_ERR_PLIB,"Original quadrant id %d did not reach number of quadrants %d",(int)orig_quadid,(int)orig_n_quads);
  if (adap_quadid != adap_n_quads) SETERRQ2(orig_p4est->mpicomm,PETSC_ERR_PLIB,"Adapted quadrant id %d did not reach number of quadrants %d",(int)adap_quadid,(int)adap_n_quads);
  PetscFunctionReturn(0);
}
