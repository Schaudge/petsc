...
#else
  PetscStackCallP4est(p4est_iterate,(p4est,ghost,&iterCtx,_p_iterCopy,NULL,NULL));
#endif
  PetscFunctionReturn(0);
}
                                          ...
                                          Vec *vecRead, Vec *vecReadWrite)
{
  ...
  ierr = DMBFGetGhost(dm,&ghost);CHKERRQ(ierr);
#if defined(P4_TO_P8)
  PetscStackCallP4est(p4est_iterate,(p4est,ghost,&iterCtx,_p_iterGetCellData,NULL,NULL,NULL));
#else
  PetscStackCallP4est(p4est_iterate,(p4est,ghost,&iterCtx,_p_iterGetCellData,NULL,NULL));
#endif
  /* clear iterator context */
  if (vecRead) {
    for (i=0; i<nValsPerElemRead; i++) {
...
PetscErrorCode DMBF_XD_IterateFVMatAssembly(DM dm, DM_BF_Cell *cells, size_t cellSize, Mat M,
                                            PetscErrorCode (*iterFace)(DM,DM_BF_Face*,PetscReal*,void*), void *userIterCtx)
{
  ...
  ierr = DMBFGetGhost(dm,&ghost);CHKERRQ(ierr);
#if defined(P4_TO_P8)
  PetscStackCallP4est(p4est_iterate,(p4est,ghost,&iterCtx,NULL,_p_iterFVMatAssembly,NULL,NULL));
#else
