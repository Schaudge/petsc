...
struct _IRKTableau{
  PetscScalar *A,*A_inv,*A_inv_rowsum,*b;
  PetscReal   *c,*I_s;
...
PetscErrorCode TSIRKTableauCreate(TS ts,PetscInt nstages,const PetscScalar *A,const PetscScalar *b,const PetscReal *c,const PetscReal *binterp,const PetscScalar *A_inv,const PetscScalar *A_inv_rowsum,const PetscScalar *I_s)
{
...
{
  PetscInt       nstages;
  PetscScalar    *gauss_A,*gauss_b,*gauss_A_inv,*gauss_A_inv_rowsum;
  PetscReal      *b,*gauss_c,*I_s;
  PetscScalar    *G0,*G1;
  PetscInt       i,j;
  ...
  ierr = TSIRKTableauCreate(ts,nstages,gauss_A,gauss_b,gauss_c,NULL,gauss_A_inv,gauss_A_inv_rowsum,I_s);CHKERRQ(ierr);
  ierr = PetscFree3(gauss_A,gauss_b,gauss_c);CHKERRQ(ierr);
  ierr = PetscFree3(gauss_A_inv,gauss_A_inv_rowsum,I_s);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
static PetscErrorCode TSStep_IRK(TS ts)
{
  ...
  */
  ...
  while (!ts->reason && irk->status != TS_STEP_COMPLETE) {
    ...
    for (i=0; i<nstages; i++) {
      ierr = VecStrideGather(irk->Z,i*bs,irk->Y[i],INSERT_VALUES);CHKERRQ(ierr);
    }
    for (i=0; i<nstages; i++) {
      ierr = VecZeroEntries(irk->YdotI[i]);CHKERRQ(ierr);;
      for (j=0; j<nstages; j++) {
        ierr = VecAXPY(irk->YdotI[i],A_inv[j*nstages+i]/ts->time_step,irk->Y[j]);CHKERRQ(ierr);
      }
...
static PetscErrorCode TSInterpolate_IRK(TS ts,PetscReal itime,Vec U)
{
  ...
  ierr = PetscMalloc(nstages,&bt);CHKERRQ(ierr);
...
static PetscErrorCode SNESTSFormFunction_IRK(SNES snes,Vec ZC,Vec FC,TS ts)
{
  ...
  IRKTableau      tab  = irk->tableau;
  const PetscInt  nstages = irk->nstages;
  const PetscReal *c = tab->c,*A_inv = tab->A_inv,*A_inv_rowsum = tab->A_inv_rowsum;
  DM              dm,dmsave;
  ...
  for (i=0; i<nstages; i++) {
    ierr = VecStrideGather(ZC,i*bs,Y[i],INSERT_VALUES);CHKERRQ(ierr);
  }
  dmsave = ts->dm;
  ts->dm = dm;
  for (i=0; i<nstages; i++) {
    ...
    ierr = VecStrideScatter(YdotI[i],i*bs,FC,INSERT_VALUES);CHKERRQ(ierr);
  }
  ts->dm = dmsave;
...
static PetscErrorCode SNESTSFormJacobian_IRK(SNES snes,Vec ZC,Mat JC,Mat JCpre,TS ts)
{
  ...
  } else { /* ToDo: need the mass matrix for DEA  */
  }
  ts->dm = dmsave;
  PetscFunctionReturn(0);
}
...
static PetscErrorCode TSSetFromOptions_IRK(PetscOptionItems *PetscOptionsObject,TS ts)
{
  ...
  ierr = PetscOptionsHead(PetscOptionsObject,"IRK ODE solver options");CHKERRQ(ierr);
  {
    ...
    ierr = PetscOptionsInt("-ts_irk_nstages","Stages of the IRK method","TSIRKSetStages",irk->nstages,&irk->nstages,NULL);CHKERRQ(ierr);
...
static PetscErrorCode TSView_IRK(TS ts,PetscViewer viewer)
{
  ...
  if (iascii) {
    ...
    ierr = PetscFormatRealArray(buf,sizeof(buf),"% 8.6f",PetscSqr(irk->nstages),tab->A_inv);CHKERRQ(ierr);
    ierr = PetscViewerASCIIPrintf(viewer,"  A inverse       A_inv = %s\n",buf);CHKERRQ(ierr);
  }
  PetscFunctionReturn(0);
}
...
static PetscErrorCode TSIRKSetType_IRK(TS ts,TSIRKType irktype)
{
  ...
  PetscFunctionBegin;
  if (irk->method_name) {
    ierr = PetscStrcmp(irk->method_name,irktype,&same);CHKERRQ(ierr);
