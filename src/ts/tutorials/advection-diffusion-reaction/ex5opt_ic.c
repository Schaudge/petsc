...
extern PetscErrorCode RHSJacobianShell(TS,PetscReal,Vec,Mat,Mat,void*);
...
extern PetscErrorCode IJacobianShell(TS,PetscReal,Vec,Vec,PetscReal,Mat,Mat,void*);
...
int main(int argc,char **argv)
{
  ...
  AppCtx         appctx;
  MCtx           mctx;
  PetscBool      forwardonly = PETSC_FALSE,implicitform = PETSC_FALSE,mf = PETSC_FALSE;
  ...
  if (mf) {
    ...
    mctx.ts = appctx.ts;
    mctx.appctx = &appctx;
    ierr = VecDuplicate(appctx.U,&mctx.U);CHKERRQ(ierr);
    /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    if (!implicitform) { /* for explicit methods only */
      ierr = TSSetRHSJacobian(appctx.ts,appctx.A,appctx.A,RHSJacobianShell,&appctx);CHKERRQ(ierr);
    } else {
      ...
      ierr = TSSetIJacobian(appctx.ts,appctx.A,appctx.A,IJacobianShell,&appctx);CHKERRQ(ierr);
    }
  }
