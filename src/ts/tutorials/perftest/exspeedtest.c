...
PetscErrorCode ThrowItInTheBin(MPI_Comm comm, PetscScalar local, PetscInt root, const char header[])
{
  ...
  if (!rank) {ierr = PetscMalloc1(size, &rootArray);CHKERRQ(ierr);}
...
static PetscErrorCode ProcessOpts(MPI_Comm comm, AppCtx *options)
{
  ...
  options->gpuAwareMPI             = PETSC_TRUE;
  ...
  ierr = PetscOptionsBegin(comm, NULL, "Speedtest Options", "");CHKERRQ(ierr); {
    ...
    ierr = PetscOptionsString("-filename", "Filename of mesh to read", "DMPlexCreateFromFile()", options->filename, options->filename, PETSC_MAX_PATH_LEN, &options->fileflg); CHKERRQ(ierr);

    ierr = PetscOptionsInt("-dim", "Dimension of mesh", "DMGetDimension()", options->dim, &options->dim, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsInt("-num_fields", "Number of PetscFE fields", "PetscFECreateDefault()", options->numFields, &options->numFields, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsInt("-overlap", "DMPlex overlap", "DMPlexCreate()", options->overlap, &options->overlap, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsInt("-qorder", "PetscFE order", "PetscFECreateDeafult()", options->qorder, &options->qorder, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsInt("-refine_dm_level", "Number of times to refine the DM", "DMRefine()", options->level, &options->level, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsScalar("-refine_limit", "Refinement limit", "", options->refinementLimit, &options->refinementLimit, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsInt("-max_com", "Number of communication loops", "", options->commax, &options->commax, NULL);CHKERRQ(ierr);

    ierr = PetscOptionsInt("-vec_size_local", "Size of local vector with which to clear the cache", "VecCreate()", options->VSL, &options->VSL, NULL);CHKERRQ(ierr);
    ierr = PetscOptionsInt("-vec_size_global", "Global size of cache vectors", "VecCreate()", options->VSG, &options->VSG, NULL);CHKERRQ(ierr);
...
int main(int argc, char **argv)
{
  ...
  HookInfo              sinfo, rinfo;
  PetscSF               sf;

  ierr = PetscOptionsSetValue(NULL, "-use_gpu_aware_mpi", "true");CHKERRQ(ierr);
  ierr = PetscInitialize(&argc, &argv,(char *) 0, help);if(ierr){ return ierr;}
  comm = PETSC_COMM_WORLD;
  ierr = PetscMPIHookGetStats(&sinfo, PETSC_TRUE);CHKERRQ(ierr);
  ierr = PetscMPIHookGetStats(&rinfo, PETSC_FALSE);CHKERRQ(ierr);
  ierr = MPI_Comm_size(comm, &size);CHKERRQ(ierr);
  ierr = MPI_Comm_rank(comm, &rank);CHKERRQ(ierr);
  ...
  ierr = PetscMPIHookZeroCounters(sinfo);CHKERRQ(ierr);
  ierr = PetscMPIHookZeroCounters(rinfo);CHKERRQ(ierr);

  // We do not want to communicate local to local; we are timing communication for ghost points
  ierr = DMGetSectionSF(dm,&sf);CHKERRQ(ierr);
  PetscSF_Basic *bas = (PetscSF_Basic*)sf->data;
  bas->rootbuflen[PETSCSF_LOCAL] = PETSC_FALSE;
  ...
  ierr = PetscLogStagePop();CHKERRQ(ierr);
  ierr = PetscMPIHookViewFromOptions(comm, sinfo, "-mpi_send_view");CHKERRQ(ierr);
  ierr = PetscMPIHookViewFromOptions(comm, rinfo, "-mpi_recv_view");CHKERRQ(ierr);
  ...
  if (user.distribute) {
    ...
    const PetscInt        *leafOffset, *rootOffset;
    ...
    ierr = PetscSFGetRootRanks(sectionSF, &nRootRanks, &RootRanks, &rootOffset, NULL, NULL);CHKERRQ(ierr);
    ierr = PetscSFGetLeafRanks(sectionSF, &nLeafRanks, &LeafRanks, &leafOffset, NULL);CHKERRQ(ierr);
    nRanks = nRootRanks + nLeafRanks;
    ierr = PetscCalloc4(nRootRanks, &rDiffs, nLeafRanks, &iDiffs, nRootRanks, &RootRanks64, nLeafRanks, &LeafRanks64);CHKERRQ(ierr);
    for (i = 0; i < nRootRanks; ++i) {
      rDiffs[i] = rootOffset[i+1] - rootOffset[i];
      rMin = PetscMin(rMin, rDiffs[i]);
      rMax = PetscMax(rMax, rDiffs[i]);
      RootRanks64[i] = (PetscInt) RootRanks[i];
    }
    if (rMin == PETSC_MAX_INT) rMin = 0;
    for (i = 0; i < nLeafRanks; ++i) {
      iDiffs[i] = leafOffset[i+1] - leafOffset[i];
      iMin = PetscMin(iMin, iDiffs[i]);
      iMax = PetscMax(iMax, iDiffs[i]);
      LeafRanks64[i] = (PetscInt) LeafRanks[i];
    }
    if (iMin == PETSC_MAX_INT) iMin = 0;
    ...
    ierr = PetscPrintf(comm, "\n");CHKERRQ(ierr);
    ierr = ThrowItInTheBin(comm,(PetscScalar) iMin, 0, "MIN LEAFOFFSET NUM MESSAGES");CHKERRQ(ierr);
    ierr = ThrowItInTheBin(comm,(PetscScalar) iMax, 0, "MAX LEAFOFFSET NUM MESSAGES");CHKERRQ(ierr);
    ierr = PetscPrintf(comm, "\n");CHKERRQ(ierr);
    ierr = ThrowItInTheBin(comm,(PetscScalar) rMin, 0, "MIN ROOTOFFSET NUM MESSAGES");CHKERRQ(ierr);
    ierr = ThrowItInTheBin(comm,(PetscScalar) rMax, 0, "MAX ROOTOFFSET NUM MESSAGES");CHKERRQ(ierr);
    ierr = PetscFree4(rDiffs, iDiffs, RootRanks64, LeafRanks64);CHKERRQ(ierr);
  ...
  ierr = DMGetSectionSF(dm,&sf);CHKERRQ(ierr);
  ierr = PetscSFView(sf,PETSC_VIEWER_BINARY_WORLD);CHKERRQ(ierr);
  ierr = DMDestroy(&dm);CHKERRQ(ierr);
