...
int main(int argc,char *argv[])
{
  ...
  /* Set up Riemann Solver */
    ...
    ierr = PetscCalloc3(numvalues,&u,numvalues,&isequal,numvalues,&norms);CHKERRQ(ierr);
    ...
    ierr = RiemannSolverTestEigDecomposition(rs,numvalues,u,tol,isequal,norms);CHKERRQ(ierr);
    /* Print the Failures to the Screen */
    ierr = VecCreateSeq(PETSC_COMM_SELF,dgnet->physics.dof,&viewvec);CHKERRQ(ierr);
    ierr = PetscViewerCreate(PETSC_COMM_WORLD,&viewer);CHKERRQ(ierr);
    ierr = PetscViewerSetType(viewer,PETSCVIEWERASCII);CHKERRQ(ierr);
    ierr = PetscViewerSetFromOptions(viewer);CHKERRQ(ierr);
    for (i=0; i<numvalues; i++) {
        if (isequal[i] == PETSC_FALSE) {
        ierr = VecPlaceArray(viewvec,u[i]);CHKERRQ(ierr);
        ierr = PetscPrintf(PETSC_COMM_WORLD,"\nThe following point had an incorrect Eigen Decomposition\nWith Norm Differnce: %e\n",norms[i]);CHKERRQ(ierr);
        ierr = VecView(viewvec,viewer);CHKERRQ(ierr);
        ierr = VecResetArray(viewvec);CHKERRQ(ierr);
        }
    }
    for (i=0; i<numvalues; i++) {
    ...
    ierr = VecDestroy(&viewvec);CHKERRQ(ierr);
    ierr = PetscFree3(u,isequal,norms);CHKERRQ(ierr);
