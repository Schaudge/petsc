...
/*
Here I am attempting to rewrite and clean up the FV solvers. Generalize them and allow for arbitrary splitting, and have a single RHS function that 
paramaterized by the indices that should be evaluated. 
*/
...
/* Compute the flux at the ith cellboundary in the global indexing of the DM. Flux will be stored in the ctx->flux.*/
PetscErrorCode ComputeFlux_2WaySplit(FVCtx *ctx,PetscScalar *slope, PetscScalar *x, PetscReal hxs,PetscReal hxf,PetscInt xs,PetscInt xm,PetscInt i){
PetscReal   maxspeed;
    PetscScalar *uL,*uR;
    PetscInt dof = ctx->physics2.dof; 
    uL = &ctx->uLR[0];
    uR = &ctx->uLR[dof];
    if (i < ctx->sf) { /* slow region */
      for (j=0; j<dof; j++) {
...
PetscErrorCode FVRHSFunction_2WaySplit_test(TS ts,PetscReal time,Vec X,Vec F,void *vctx)
{
  RHSCtx         *rhsctx = (RHSCtx*)vctx;
  FVCtx          *ctx = rhsctx->ctx; 
  PetscErrorCode ierr;
  PetscInt       i,j,k,Mx,dof,xs,xm,sf = ctx->sf,fs = ctx->fs;
  PetscReal      hxf,hxs,cfl_idt = 0;
  PetscScalar    *x,*f,*slope;
  ...
  for (k=0; k<rhsctx->indexlength; k++) {
    i = rhsctx->index[k];
    PetscReal   maxspeed;
    PetscScalar *uL,*uR;
    uL = &ctx->uLR[0];
    ...
    if (i < sf) { /* slow region */
      ...
      ierr    = (*ctx->physics2.riemann2)(ctx->physics2.user,dof,uL,uR,ctx->flux,&maxspeed);CHKERRQ(ierr);
      if (i > xs) {
        for (j=0; j<dof; j++) f[(k-1)*dof+j] -= ctx->flux[j]/hxs;
      }
      if (i < xs+xm) {
        for (j=0; j<dof; j++) f[k*dof+j] += ctx->flux[j]/hxs;
      }
    } else if (i == sf) { /* interface between the slow region and the fast region */
      for (j=0; j<dof; j++) {
        uL[j] = x[(i-1)*dof+j]+slope[(i-1)*dof+j]*hxs/2;
      ...
      if (i < xs+xm) {
        for (j=0; j<dof; j++) f[k*dof+j] += ctx->flux[j]/hxf;
      }
    } else if (i > sf && i < fs) { /* fast region */
      for (j=0; j<dof; j++) {
        uL[j] = x[(i-1)*dof+j]+slope[(i-1)*dof+j]*hxf/2;
      ...
      ierr    = (*ctx->physics2.riemann2)(ctx->physics2.user,dof,uL,uR,ctx->flux,&maxspeed);CHKERRQ(ierr);
      if (i > xs) {
        for (j=0; j<dof; j++) f[(k-1)*dof+j] -= ctx->flux[j]/hxf;
      }
      if (i < xs+xm) {
        for (j=0; j<dof; j++) f[k*dof+j] += ctx->flux[j]/hxf;
      }
...
int main(int argc,char *argv[])
{
  ...
  PetscReal         ptime;
  PetscErrorCode    ierr;
  RHSCtx            slowctx,fastctx,bufferctx,totalctx;
  ...
  slowctx.ctx = &ctx; 
  slowctx.index = index_slow; 
  slowctx.indexlength = islow;

  fastctx.ctx = &ctx; 
  fastctx.index = index_fast; 
  fastctx.indexlength = ifast; 

  bufferctx.ctx = &ctx; 
  bufferctx.index = index_slowbuffer; 
  bufferctx.indexlength = islowbuffer; 
  ...
  ierr = TSRHSSplitSetRHSFunction(ts,"fast",NULL,FVRHSFunction_2WaySplit_test,&fastctx);CHKERRQ(ierr);
