...
{
  PetscErrorCode ierr;
  PetscInt       id,e,eStart,eEnd;;
  PetscInt       *slow_edges,*fast_edges;
  PetscInt       slow_edges_size = 0,fast_edges_size = 0;
  PetscInt       slow_edges_count = 0,fast_edges_count = 0;
  FVEdge         fvedge;
...
PetscErrorCode FVNetworkFinalizePartition(FVNetwork fvnet) 
{
  ...
  PetscInt       *buf_vert,*fast_vert,*slow_vert;
  PetscInt       *vert_sizes,*vert_count;
  ...
  for(i=0; i<ne; i++) {
    ...
    vto   = cone[1];
    ierr = DMNetworkGetComponentVariableOffset(fvnet->network,vfrom,FLUX,&offsetf);CHKERRQ(ierr);
    f[offsetf+fvedge->vfrom_recon_offset] = SLOW;
    ierr = DMNetworkGetComponentVariableOffset(fvnet->network,vto,FLUX,&offsetf);CHKERRQ(ierr);
    f[offsetf+fvedge->vto_recon_offset] = SLOW; 
  }
  ...
  for(i=0; i<ne; i++) {
    ...
    vto   = cone[1];
    ierr = DMNetworkGetComponentVariableOffset(fvnet->network,vfrom,FLUX,&offsetf);CHKERRQ(ierr);
    f[offsetf+fvedge->vfrom_recon_offset] = FAST;
    ierr = DMNetworkGetComponentVariableOffset(fvnet->network,vto,FLUX,&offsetf);CHKERRQ(ierr);
    f[offsetf+fvedge->vto_recon_offset] = FAST; 
  }
  ...
  for(v=vStart; v<vEnd; v++) {
    ...
    if(hasmarkededge[0] && hasmarkededge[1]) {
      ...
      for(i=0; i<ne; i++) { /* Mark the connected slow edges as buffer edges */
        ...
        vto   = cone[1];
        if (v == vfrom) {
          if (f[offsetf+fvedge->vfrom_recon_offset] == SLOW) fvedge->frombufferlvl = 1; 
        } else if (v == vto) {
          if (f[offsetf+fvedge->vto_recon_offset] == SLOW) fvedge->tobufferlvl = 1; 
        }
      }
    }
  }
