...
PetscErrorCode CharacteristicSolve(Characteristic c, PetscReal dt, Vec solution)
{
  ...
  ierr = PetscInfo1(NULL, "Calculating %" PetscInt_FMT " remote velocities at t_{n - 1/2}\n", c->queueRemoteSize);CHKERRQ(ierr);
  ...
  ierr = PetscInfo1(NULL, "Calculating %" PetscInt_FMT " remote field points at t_{n}\n", c->queueRemoteSize);CHKERRQ(ierr);
  for (n = 0; n < c->queueRemoteSize; n++) {
    ...
    if (1) { /* hacked bounds test...let's do better */
      ...
      if (PetscUnlikely((im < (PetscScalar) is - 1.) || (im > (PetscScalar) ie) || (jm < (PetscScalar)  js - 1.) || (jm > (PetscScalar) je))) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_LIB, "Nonlocal point: (%g,%g)", (double)PetscRealPart(im), (double)PetscRealPart(jm));
    }
...
{
  PetscFunctionBegin;
  if (c->queueSize >= c->queueMax) SETERRQ1(PETSC_COMM_SELF,PETSC_ERR_ARG_OUTOFRANGE, "Exceeded maximum queue size %" PetscInt_FMT, c->queueMax);
...
int CharacteristicSendCoordinatesBegin(Characteristic c)
{
  ...
  for (n = 1; n < c->numNeighbors; n++) {
    ierr = PetscInfo2(NULL, "Receiving %" PetscInt_FMT " requests for values from proc %d\n", c->fillCount[n], c->neighbors[n]);CHKERRQ(ierr);
  ...
  for (n = 1; n < c->numNeighbors; n++) {
    ierr = PetscInfo2(NULL, "Sending %" PetscInt_FMT " requests for values from proc %d\n", c->needCount[n], c->neighbors[n]);CHKERRQ(ierr);
...
/*---------------------------------------------------------------------*/
{
  ...
  if (0) { /* Check the order of the queue before sorting */
    ...
    for (n=0; n<size; n++) {
      ierr = PetscInfo2(NULL,"%" PetscInt_FMT " %d\n",n,queue[n].proc);CHKERRQ(ierr);
    }
  }
