...
PetscErrorCode DMPlexViewAsciiConcise(DM dmin, PetscViewer viewer)
{
	...
	PetscInt		i, locdepth, depth, localVertexSize = 0, globalVertexSize = 0, globalEdgeSize = 0, globalFaceSize = 0, globalCellSize = 0, dim = 0, localSupportSize = 0, locmaxSupportSize, maxSupportSize, locminMultiplicity = 10000, locmaxMultiplicity = 0, minMultiplicity = 10000, maxMultiplicity = 0,  numBinnedVertexProcesses, numBinnedEdgeProcesses, numBinnedFaceProcesses, numBinnedCellProcesses;
	PetscInt		*binnedVertices = NULL, *binnedEdges = NULL, *binnedFaces = NULL, *binnedCells = NULL;
	PetscScalar		*locmultiplicities, *verticesPerProcess = NULL, *edgesPerProcess = NULL, *facesPerProcess = NULL, *cellsPerProcess = NULL;
	...
	if (size > 1) {
		...
		if (sf) {
			//ierr = DMPlexDistributeOverlap(dmin, 1, NULL,
			//&dm);CHKERRQ(ierr);
			dm = dmin;
	...
	for (i = 0; i <= depth; i++) {
		...
		switch (i)
			{
			case 0:
				ierr = DMPlexGetVertexNumbering(dmin, &vertexIS);CHKERRQ(ierr);
				...
				ierr = DMPlexGetXXXPerProcess(dmin, i, &numBinnedVertexProcesses, &verticesPerProcess, &binnedVertices);CHKERRQ(ierr);
				break;
			case 1:
				ierr = DMPlexGetEdgeNumbering(dmin, &edgeIS);CHKERRQ(ierr);
				...
				ierr = DMPlexGetXXXPerProcess(dmin, i, &numBinnedEdgeProcesses, &edgesPerProcess, &binnedEdges);CHKERRQ(ierr);
				break;
			case 2:
				ierr = DMPlexGetFaceNumbering(dmin, &faceIS);CHKERRQ(ierr);
				...
				ierr = DMPlexGetXXXPerProcess(dmin, i, &numBinnedFaceProcesses, &facesPerProcess, &binnedFaces);CHKERRQ(ierr);
                                break;
			case 3:
				ierr = DMPlexGetCellNumbering(dmin, &cellIS);CHKERRQ(ierr);
				...
				ierr = DMPlexGetXXXPerProcess(dmin, i, &numBinnedCellProcesses, &cellsPerProcess, &binnedCells);CHKERRQ(ierr);
                                break;
			default:
	...
	*/}
	...
	ierr = DMPlexCheckFaces(dmin, 0);CHKERRQ(ierr); if (!ierr) { facesOK = PETSC_TRUE;}
	ierr = DMPlexCheckSymmetry(dmin);CHKERRQ(ierr); if (!ierr) { symmetryOK = PETSC_TRUE;}
	ierr = DMPlexCheckSkeleton(dmin, 0);CHKERRQ(ierr); if (!ierr) { skeletonOK = PETSC_TRUE;}
	ierr = DMPlexCheckPointSF(dmin);CHKERRQ(ierr); if(!ierr) { pointSFOK = PETSC_TRUE;}
	ierr = DMPlexCheckGeometry(dmin);CHKERRQ(ierr); if (!ierr) { geometryOK = PETSC_TRUE;}
	ierr = DMPlexCheckConesConformOnInterfaces(dmin);CHKERRQ(ierr); if(!ierr) { coneConformOnInterfacesOK = PETSC_TRUE;}
...
int main(int argc, char **argv)
{
	...
	ierr = PetscViewerPushFormat(genViewer, PETSC_VIEWER_ASCII_INFO_CONCISE);CHKERRQ(ierr);
	//ierr = DMPlexViewAsciiConcise(dm, genViewer);CHKERRQ(ierr);
	ierr = DMView(dm, genViewer);CHKERRQ(ierr);
