...
/*
   du = D (u_xx + u_yy) dt + sigma * dW^Q
   G = { (x,y) |0 < x < 1, 0 < y < 1}
   At t=0:  u(x,y) = b*exp(-c*r*r), if r=PetscSqrtReal((x-.5)*(x-.5) + (y-.5)*(y-.5)) < rad
            u(x,y) = 0.0          , if r >= rad
   At bdry: u(x,y) = 0.0
   sigma * dW^Q(t,x,y) = mu + sigma \sum_{j>=1} sqrt{q_j} \phi_j(x,y) * dw_j(t)
   dw_j(t) are Gaussian white noise
...
PetscBool useMatlabPlot = PETSC_TRUE;

PetscErrorCode SetParams(ParamInfo*,GridInfo*,TsInfo*);
PetscErrorCode FormInitialSolution(AppCtx*,Vec);
PetscErrorCode BuildA_EU(AppCtx*);
...
int main(int argc,char **argv)
{
    ...
    /* Euler scheme */
//      ierr = BuildA_EU(user);CHKERRQ(ierr);
...
/* ----------------------------------------------------------------------------------------------------------------------- */
PetscErrorCode SetParams(ParamInfo *param, GridInfo *grid, TsInfo *ts)
{
    PetscErrorCode ierr;
    
    PetscFunctionBeginUser;
    /* physical parameters */
    param->M        = 1;     /* MC sampling number*/
    param->b        = 5.0;   /* the height of initial profile (Gaussian) */
    param->c        = 30.0;  /* the decay rate of initial profile (Gaussian) */
    param->rad      = 0.5;   /* the radius of initial profile (indicator function) */
    param->D        = 0.02;  /* the diffusion coefficient of heat equation */
    param->mu       = 0.0;   /* the mean of KL random field */
    param->sigma    = 1.0;   /* sigma here stands for strength of KL random field */
    ...
    ierr = PetscOptionsGetInt(NULL,NULL,"-M",&(param->M),NULL);CHKERRQ(ierr);
    ...
    ierr = PetscOptionsGetReal(NULL,NULL,"-D",&(param->D),NULL);CHKERRQ(ierr);
...
/* ----------------------------------------------------------------------------------------------------------------------- */
PetscErrorCode BuildA_EU(AppCtx *user)
{
    DMDALocalInfo  info;
    ParamInfo     *param = user->param;
    PetscScalar    D     = param->D;
    PetscInt       i,j;
    PetscReal      hx,hy,mux,muy;
    PetscErrorCode ierr;
    
    PetscFunctionBeginUser;
    ierr = DMDAGetLocalInfo(user->da,&info);CHKERRQ(ierr);
    hx   = 1.0/(PetscReal)(info.mx-1); mux = D/(hx*hx);
    hy   = 1.0/(PetscReal)(info.my-1); muy = D/(hy*hy);
    ...
    for (j=info.ys; j<info.ys+info.ym; j++) {
        for (i=info.xs; i<info.xs+info.xm; i++) {
            ...
            if (i == 0 || j == 0 || i == info.mx-1 || j == info.my-1) {
                ...
                col[nc].i = i-1; col[nc].j = j;   val[nc++] = mux;
                col[nc].i = i+1; col[nc].j = j;   val[nc++] = mux;
                col[nc].i = i;   col[nc].j = j-1; val[nc++] = muy;
                col[nc].i = i;   col[nc].j = j+1; val[nc++] = muy;
                col[nc].i = i;   col[nc].j = j;   val[nc++] = -2*mux - 2*muy;
            }
    ...
    for (j=info.ys; j<info.ys+info.ym; j++) {
        for (i=info.xs; i<info.xs+info.xm; i++) {
            ...
            if (i == 0 || j == 0 || i == info.mx-1 || j == info.my-1) {
                ...
                col[nc].i = i-1; col[nc].j = j;   val[nc++] = -.5*mux*dt;
                col[nc].i = i+1; col[nc].j = j;   val[nc++] = -.5*mux*dt;
                col[nc].i = i;   col[nc].j = j-1; val[nc++] = -.5*muy*dt;
                col[nc].i = i;   col[nc].j = j+1; val[nc++] = -.5*muy*dt;
                col[nc].i = i;   col[nc].j = j;   val[nc++] = 1 + mux*dt + muy*dt;
            }
            ierr = MatSetValuesStencil(user->A,1,&row,nc,col,val,INSERT_VALUES);CHKERRQ(ierr);
        }
    }
    ...
    for (j=info.ys; j<info.ys+info.ym; j++) {
        for (i=info.xs; i<info.xs+info.xm; i++) {
            if (i == 0 || j == 0 || i == info.mx-1 || j == info.my-1) {
                ...
                rhs[info.mx*j+i] = u[info.mx*j+i]                            * (1-mux*dt-muy*dt)
                               + ( u[info.mx*j+(i+1)] + u[info.mx*j+(i-1)] ) * .5*mux*dt
                               + ( u[info.mx*(j+1)+i] + u[info.mx*(j-1)+i] ) * .5*muy*dt;
            }
...
PetscErrorCode myTS(AppCtx *user, Vec u)
{
    ...
    Time stepping
    - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    for ( (ts->i) = 0; (ts->i) < (ts->tsteps) ; (ts->i)++){
        ierr = PetscOptionsGetBool(NULL,NULL,"-use-matlab-plot",&useMatlabPlot,NULL);CHKERRQ(ierr);
        if (useMatlabPlot) {
            if ((ts->i) == 0){
            /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
//        ierr = VecAXPY(unew,1.0,uold);CHKERRQ(ierr);
    ...
    mQ   = (PetscInt)PetscRoundReal((grid->Nx)*0.5-1);
...
PetscErrorCode Plot(AppCtx* user, Vec u, Vec r, char* output)
{
    ...
    if ( (ts->i) == 0 || ((ts->i) + 1) % (ts->tout) == 0){
        ...
        if ((ts->i) == 0){
            ierr = PetscMatlabEngineEvaluate(PETSC_MATLAB_ENGINE_(PETSC_COMM_WORLD),"h=figure(1);axis tight manual;filename = 'plot.gif';frame = getframe(h);im = frame2im(frame);[imind,cm] =rgb2ind(im,256);imwrite(imind,cm,filename,'gif', 'Loopcount',inf,'DelayTime',1/5);");CHKERRQ(ierr);
        }
        else{
            ierr = PetscMatlabEngineEvaluate(PETSC_MATLAB_ENGINE_(PETSC_COMM_WORLD),"h=figure(1);axis tight manual;filename = 'plot.gif';frame = getframe(h);im = frame2im(frame);[imind,cm]=rgb2ind(im,256);imwrite(imind,cm,filename,'gif','WriteMode','append','DelayTime',1/5);");CHKERRQ(ierr);
            ierr = PetscSynchronizedPrintf(PETSC_COMM_WORLD,"%s",output);CHKERRQ(ierr);
        }
    }
    PetscFunctionReturn(0);
