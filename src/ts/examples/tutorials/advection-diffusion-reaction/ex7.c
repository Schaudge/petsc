...
static char help[] = "Population modeling using Reaction-Diffusion Equations.\n";

/*F
   This model is based on notes from Erin Ellefsen and Nancy Rodriguez.
\begin{eqnarray*}
        u_t = \nabla\cdot\Big(u \nabla \big(2\eta u - K*(u-v) \big) \Big)           \\
        v_t = \nabla\cdot\Big(v \nabla \big(2\eta v - K*(v-u) \big) \Big)           \\
\end{eqnarray*}
   where $\eta$ is a diffusivity and $K$ is a radial kernel.  In serial, we can
   implement the convolution using direct summation.  An alternative formulation
   replaces (for suitable kernels K) the convolution with a potential p, leading
   to the differential algebraic equation
\begin{eqnarray*}
        u_t - \nabla\cdot\Big(u \nabla \big(2\eta u - p \big) \Big) = 0  \\
        v_t - \nabla\cdot\Big(v \nabla \big(2\eta v + p \big) \Big) = 0  \\
        -\nabla\cdot(\nabla p) = u - v \\
\end{eqnarray*}
   where the population densities and potential have periodic boundary conditions.
F*/
...
/*  Simple C struct that allows us to access the population densities and potentials easily in the code */
typedef struct {
  PetscScalar u,v;  /* population density */
...
int main(int argc,char **argv)
{
  ...
  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ierr = DMDACreate2d(PETSC_COMM_WORLD,DM_BOUNDARY_PERIODIC,DM_BOUNDARY_PERIODIC,DMDA_STENCIL_STAR,65,65,PETSC_DECIDE,PETSC_DECIDE,3,1,NULL,NULL,&da);CHKERRQ(ierr);
  ierr = DMSetFromOptions(da);CHKERRQ(ierr);
  ierr = DMSetUp(da);CHKERRQ(ierr);
  ierr = DMDASetFieldName(da,0,"u");CHKERRQ(ierr);
  ierr = DMDASetFieldName(da,1,"v");CHKERRQ(ierr);
  ierr = DMDASetFieldName(da,2,"p");CHKERRQ(ierr);
  ...
  ierr = TSSetProblemType(ts,TS_EQ_DAE_IMPLICIT_INDEX1);CHKERRQ(ierr);
...
PetscErrorCode InitialConditions(DM da,Vec U)
{
  ...
  for (j=ys; j<ys+ym; j++) {
    ...
    for (i=xs; i<xs+xm; i++) {
      x = i*hx;
      u[j][i].u = PetscExpScalar(-(PetscSqr(x-.2) + PetscSqr(y))/PetscSqr(4));
      u[j][i].v = PetscExpScalar(-(PetscSqr(x-.7) + PetscSqr(y))/PetscSqr(4));
      u[j][i].p = 0;
    }
  }
...
/*
   IFunction - Evaluates nonlinear function that defines the DAE.

   Input Parameters:
.  ts - the TS context
.  time - current time
.  U - input state vector
.  Udot - input state derivative vector
.  ptr - optional user-defined context, as set by TSSetRHSFunction()

   Output Parameter:
.  F - function vector
 */
PetscErrorCode IFunction(TS ts,PetscReal time,Vec U,Vec Udot,Vec F,void *ptr)
{
  ...
  hx = 1./(PetscReal)(Mx-1); sx = 1./(hx*hx);
  hy = 1./(PetscReal)(My-1); sy = 1./(hy*hy);

  ierr = DMGlobalToLocal(da,U,INSERT_VALUES,localU);CHKERRQ(ierr);
  ...
  */
  for (j=ys; j<ys+ym; j++) {
    for (i=xs; i<xs+xm; i++) {
      PetscScalar uc,vc,pc,uxx,uyy,vxx,vyy,pxx,pyy;
      uc        = u[j][i].u;
      ...
      pc        = u[j][i].p;
      pxx       = (-2.0*pc + u[j][i-1].p + u[j][i+1].p)*sx;
      pyy       = (-2.0*pc + u[j-1][i].p + u[j+1][i].p)*sy;
...
/*
   IJacobian - Evaluates the Jacobian of the right hand side
     function of the ODE.
...
.  time - current time
.  U - input vector
.  Udot - input time derivative
.  shift - transient shift
 ...
 */
PetscErrorCode IJacobian(TS ts,PetscReal t,Vec U,Vec Udot,PetscReal shift,Mat A,Mat BB,void *ctx)
{
...
/*TEST
   ...
   test:
      args: -ts_view  -ts_monitor -ts_max_time 100
      requires: double
      timeoutfactor: 3
