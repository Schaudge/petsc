...
int main(int argc, char **argv)
{
  ...
  */
  ...
  CHKERRQ(ierr);
  ierr = TSSetDuration(appctx.ts, appctx.param.steps, appctx.param.Tend);
  CHKERRQ(ierr);
...
PetscErrorCode RHSFunction(TS ts, PetscReal t, Vec globalin, Vec globalout, void *ctx)
{
  ...
  CHKERRQ(ierr);
  ierr = PetscGLLElementMassCreate(&appctx->SEMop.gll, &mass);
  CHKERRQ(ierr);
  ierr = PetscGLLElementAdvectionCreate(&appctx->SEMop.gll, &grad);
  CHKERRQ(ierr);
  ...
  */
  ...
  CHKERRQ(ierr);
  ierr = PetscGLLElementAdvectionDestroy(&appctx->SEMop.gll, &grad);
  CHKERRQ(ierr);
  ierr = PetscGLLElementMassDestroy(&appctx->SEMop.gll, &mass);
  CHKERRQ(ierr);
