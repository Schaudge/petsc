...
int main(int argc, char **argv)
{
  ...
  /*	Read in or create mesh	*/
  {
    ...
    if (fileflg) {
      ...
      ierr = DMPlexCreateFromFile(comm, filenameAlt, dmInterped, &dm);CHKERRQ(ierr);
  ...
  if (VTKdisp) {
    ...
    if (fileflg) {
      ...
      ierr = PetscStrlen(filename, &lenTotal);CHKERRQ(ierr);
      ierr = PetscStrrchr(filename, '.', &fileEnding);CHKERRQ(ierr);
      ierr = PetscStrlen(fileEnding, &lenEnding);CHKERRQ(ierr);
      if (lenTotal > lenEnding) {
        ierr = PetscMalloc1(lenTotal, &fixedFile);CHKERRQ(ierr);
        ierr = PetscStrncpy(fixedFile, filename, lenTotal-lenEnding);CHKERRQ(ierr);
      } else {
        ierr = PetscStrallocpy(filename, &fixedFile);CHKERRQ(ierr);
      }
