...
PetscErrorCode PetscAdolcComputeRHSJacobian(PetscInt tag,Mat A,PetscScalar *u_vec,void *ctx)
{
  ...
  if (adctx->Seed) {
    ...
    ierr = ConvertToCSR(J,m,(PetscInt**)adctx->ri,(PetscInt**)adctx->rj,c,NULL);CHKERRQ(ierr);
  } else
...
PetscErrorCode PetscAdolcComputeIJacobian(PetscInt tag1,PetscInt tag2,Mat A,PetscScalar *u_vec,PetscReal a,void *ctx)
{
  ...
  if (adctx->Seed) {
    ...
    ierr = ConvertToCSR(J,m,(PetscInt**)adctx->ri,(PetscInt**)adctx->rj,c,&a);CHKERRQ(ierr);
  } else
...
PetscErrorCode PetscAdolcComputeIJacobianAndDiagonalLocal(PetscInt tag1,PetscInt tag2,Vec diag,PetscScalar *u_vec,PetscReal a,void *ctx)
{
  ...
  if (adctx->Seed)
    ...
    jacobian(tag1,m,n,u_vec,J);
  if (adctx->sparse) {
    ierr = RecoverDiagonalLocal(diag,INSERT_VALUES,m,(PetscInt**)adctx->r,J,NULL);CHKERRQ(ierr);
  } else {
    for (i=0; i<m; i++) {
  ...
  if (adctx->Seed)
    ...
    jacobian(tag2,m,n,u_vec,J);
  if (adctx->sparse) {
    ierr = RecoverDiagonalLocal(diag,ADD_VALUES,m,(PetscInt**)adctx->r,J,NULL);CHKERRQ(ierr);
  } else {
    for (i=0; i<m; i++) {
      if (fabs(J[i][i]) > 1.e-16) {
        J[i][i] *= a;
