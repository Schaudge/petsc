...
PetscErrorCode TSClone(TS tsin, TS *tsout)
{
  ...
  PetscFunctionBegin;
  PetscValidHeaderSpecific(tsin, TS_CLASSID, 1);
  PetscValidPointer(tsout, 2);
  *tsout = NULL;

  PetscCall(TSCreate(PetscObjectComm((PetscObject)tsin), tsout));

  /* General TS description */
  (*tsout)->numbermonitors    = 0;
  (*tsout)->monitorFrequency  = 1;
  (*tsout)->setupcalled       = 0;
  (*tsout)->ksp_its           = 0;
  (*tsout)->snes_its          = 0;
  (*tsout)->nwork             = 0;
  (*tsout)->rhsjacobian.time  = PETSC_MIN_REAL;
  (*tsout)->rhsjacobian.scale = 1.;
  (*tsout)->ijacobian.shift   = 1.;
  (*tsout)->vec_sol           = NULL;
