...
PetscErrorCode  TSComputeRHSJacobian(TS ts,PetscReal t,Vec U,Mat A,Mat B)
{
  ...
  if (ts->rhsjacobian.shift) SETERRQ1(PetscObjectComm((PetscObject)ts),PETSC_ERR_USER,"Should not call TSComputeRHSJacobian() on a shifted matrix (shift=%lf). You can call TSRecoverRHSJacobian() to unshift the matrix.",ts->rhsjacobian.shift);
  if (rhsjacobianfunc) {
...
-  B - matrix from which the preconditioner is constructed; often the same as A
   ...
   Most users should not need to explicitly call this routine, as it
   is used internally within the TS solvers.

   Level: developer
.seealso:  TSComputeIJacobian(), TSComputeRHSJacobian()
@*/
PetscErrorCode TSRecoverRHSJacobian(TS ts,Mat A,Mat B)
{
  PetscErrorCode   ierr;
