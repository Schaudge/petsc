...
PetscErrorCode  TSSetFromOptions(TS ts)
{
  ...
  ierr = PetscOptionsString("-ts_monitor_python","Use Python function","TSMonitorSet",NULL,monfilename,PETSC_MAX_PATH_LEN,&flg);CHKERRQ(ierr);
  ...
  opt  = PETSC_FALSE;
  ierr = PetscOptionsString("-ts_monitor_solution_vtk","Save each time step to a binary file, use filename-%%03D.vts","TSMonitorSolutionVTK",NULL,monfilename,PETSC_MAX_PATH_LEN,&flg);CHKERRQ(ierr);
  if (flg) {
    const char *ptr,*ptr2;
    char       *filetemplate;
...
PetscErrorCode TSComputeIJacobian(TS ts,PetscReal t,Vec U,Vec Udot,PetscReal shift,Mat A,Mat B,PetscBool imex)
{
  ...
  if (imex) {
    ...
    if (Arhs == A) { /* No IJacobian matrix, so we only have the RHS matrix */
      ...
      ierr = PetscObjectStateGet((PetscObject)U,&Ustate);CHKERRQ(ierr);
      ierr = PetscObjectGetId((PetscObject)U,&Uid);CHKERRQ(ierr);
      if ((ts->rhsjacobian.time == t && (ts->problem_type == TS_LINEAR || (ts->rhsjacobian.Xid == Uid && ts->rhsjacobian.Xstate == Ustate)) && rhsfunction != TSComputeRHSFunctionLinear) && ts->rhsjacobian.scale == -1.) { /* No need to recompute RHSJacobian */
...
PetscErrorCode TSComputeRHSFunctionLinear(TS ts,PetscReal t,Vec U,Vec F,void *ctx)
{
  ...
  if (ts->rhsjacobian.shift) {
    ierr = MatShift(Arhs,-ts->rhsjacobian.shift);CHKERRQ(ierr);
   }
  if (ts->rhsjacobian.scale == -1.) {
    ierr = MatScale(Arhs,-1);CHKERRQ(ierr);
  }
  if (Arhs != Brhs) {
    if (ts->rhsjacobian.shift) {
      ierr = MatShift(Brhs,-ts->rhsjacobian.shift);CHKERRQ(ierr);
    }
    if (ts->rhsjacobian.scale == -1.) {
      ierr = MatScale(Brhs,-1);CHKERRQ(ierr);
    }
  }
