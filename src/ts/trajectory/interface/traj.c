...
PetscErrorCode TSTrajectorySet(TSTrajectory tj,TS ts,PetscInt stepnum,PetscReal time,Vec X)
{
  ...
  if (tj->monitor) {
    ierr = PetscViewerASCIIPrintf(tj->monitor,"TSTrajectorySet: stepnum %" PetscInt_FMT ", time %g (stages %" PetscInt_FMT ")\n",stepnum,(double)time,(PetscInt)!tj->solution_only);CHKERRQ(ierr);
  }
...
PetscErrorCode TSTrajectoryGet(TSTrajectory tj,TS ts,PetscInt stepnum,PetscReal *time)
{
  ...
  if (tj->monitor) {
    ierr = PetscViewerASCIIPrintf(tj->monitor,"TSTrajectoryGet: stepnum %" PetscInt_FMT ", stages %" PetscInt_FMT "\n",stepnum,(PetscInt)!tj->solution_only);CHKERRQ(ierr);
...
PetscErrorCode TSTrajectoryGetVecs(TSTrajectory tj,TS ts,PetscInt stepnum,PetscReal *time,Vec U,Vec Udot)
{
  ...
  if (tj->monitor) {
    ...
    pU    = U ? 1 : 0;
    pUdot = Udot ? 1 : 0;
    ierr  = PetscViewerASCIIPrintf(tj->monitor,"Requested by GetVecs %" PetscInt_FMT " %" PetscInt_FMT ": stepnum %" PetscInt_FMT ", time %g\n",pU,pUdot,stepnum,(double)*time);CHKERRQ(ierr);
    ierr = PetscViewerFlush(tj->monitor);CHKERRQ(ierr);
  }
  if (U && tj->lag.caching) {
    PetscObjectId    id;
...
PetscErrorCode  TSTrajectoryView(TSTrajectory tj,PetscViewer viewer)
{
  ...
  if (iascii) {
    ...
    ierr = PetscViewerASCIIPrintf(viewer,"  total number of recomputations for adjoint calculation = %" PetscInt_FMT "\n",tj->recomps);CHKERRQ(ierr);
    ierr = PetscViewerASCIIPrintf(viewer,"  disk checkpoint reads = %" PetscInt_FMT "\n",tj->diskreads);CHKERRQ(ierr);
    ierr = PetscViewerASCIIPrintf(viewer,"  disk checkpoint writes = %" PetscInt_FMT "\n",tj->diskwrites);CHKERRQ(ierr);
    if (tj->ops->view) {
...
PetscErrorCode  TSTrajectoryCreate(MPI_Comm comm,TSTrajectory *tj)
{
  ...
  *tj  = t;
  ierr = TSTrajectorySetFiletemplate(t,"TS-%06" PetscInt_FMT ".bin");CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode TSTrajectorySetDirname(TSTrajectory tj,const char dirname[])
{
  ...
  if (PetscUnlikely(!flg && tj->dirfiletemplate)) SETERRQ(PetscObjectComm((PetscObject)tj),PETSC_ERR_ARG_WRONGSTATE,"Cannot set directoryname after TSTrajectory has been setup");
  ierr = PetscFree(tj->dirname);CHKERRQ(ierr);
...
PetscErrorCode TSTrajectorySetFiletemplate(TSTrajectory tj,const char filetemplate[])
{
  ...
  if (PetscUnlikely(tj->dirfiletemplate)) SETERRQ(PetscObjectComm((PetscObject)tj),PETSC_ERR_ARG_WRONGSTATE,"Cannot set filetemplate after TSTrajectory has been setup");

  if (PetscUnlikely(!filetemplate[0])) SETERRQ(PetscObjectComm((PetscObject)tj),PETSC_ERR_USER,"-ts_trajectory_file_template requires a file name template, e.g. filename-%%06" PetscInt_FMT ".bin");
  /* Do some cursory validation of the input. */
  ierr = PetscStrstr(filetemplate,"%",(char**)&ptr);CHKERRQ(ierr);
  if (PetscUnlikely(!ptr)) SETERRQ1(PetscObjectComm((PetscObject)tj),PETSC_ERR_USER,"Invalid file template argument '%s' to -ts_trajectory_file_template, should look like filename-%%06" PetscInt_FMT ".bin",filetemplate);
  for (ptr++; ptr && *ptr; ptr++) {
    ierr = PetscStrchr(PetscInt_FMT "DdiouxX",*ptr,(char**)&ptr2);CHKERRQ(ierr);
    if (PetscUnlikely(!ptr2 && ((*ptr < '0') || ('9' < *ptr)))) SETERRQ1(PetscObjectComm((PetscObject)tj),PETSC_ERR_USER,"Invalid file template argument '%s' to -ts_trajectory_file_template, should look like filename-%%06" PetscInt_FMT ".bin",filetemplate);
    if (ptr2) break;
  }
  ierr = PetscFree(tj->filetemplate);CHKERRQ(ierr);
...
PetscErrorCode  TSTrajectorySetFromOptions(TSTrajectory tj,TS ts)
{
  ...
  ierr = PetscOptionsString("-ts_trajectory_file_template","Template for TSTrajectory file name, use filename-%06" PetscInt_FMT ".bin","TSTrajectorySetFiletemplate",NULL,filetemplate,sizeof(filetemplate),&set);CHKERRQ(ierr);
  if (set) {
