...
.  -ts_trajectory_monitor - print TSTrajectory information
.  -ts_trajectory_dirname - set directory name for TSTrajectory file
.  -ts_trajectory_file_template - set template for TSTrajectory file name, use filename-%06D.bin
.  -ts_trajectory_storage_viewer_type - set viewer type for trajectory files
-  -ts_trajectory_storage format[:[dirname/filetemplate]] - set viewer type and name template for TSTrajectory file. format defaults to binary, template defaults to filename-%06D.bin, format can be binary or hdf5
   Level: developer
...
{
  PetscBool                     set,flg;
  char                          dirname[PETSC_MAX_PATH_LEN],filetemplate[PETSC_MAX_PATH_LEN],storagestr[PETSC_MAX_PATH_LEN];
  ...
  ierr = PetscOptionsString("-ts_trajectory_storage","Set viewer type and name template for TSTrajectory file. Usage: format[:[dirname/filetemplate]]   format defaults to binary, template defaults to filename-%06D.bin, format can be binary or hdf5","TSTrajectorySetStorage",NULL,storagestr,sizeof(storagestr),&set);CHKERRQ(ierr);
  if (set) {
    char *loc0_vtype,*loc1_dirname = NULL,*loc2_filetemplate = NULL;

    ierr = PetscStrallocpy(storagestr,&loc0_vtype);CHKERRQ(ierr);
    ierr = PetscStrchr(loc0_vtype,':',&loc1_dirname);CHKERRQ(ierr);
    if (loc1_dirname) {
      *loc1_dirname++ = 0;
      ierr = PetscStrchr(loc1_dirname,'/',&loc2_filetemplate);CHKERRQ(ierr);
      if (loc2_filetemplate) *loc2_filetemplate++ = 0;
    }
    ierr = PetscEnumFind(TSTrajectoryStorageViewerTypes,loc0_vtype,(PetscEnum*)&storageviewertype,&flg);CHKERRQ(ierr);
    if (flg) {
      ierr = TSTrajectorySetStorageViewerType(tj,storageviewertype);CHKERRQ(ierr);
    } else SETERRQ1(PetscObjectComm((PetscObject)tj),PETSC_ERR_ARG_OUTOFRANGE,"Unknown storage viewer type: %s",loc0_vtype);
    if (loc1_dirname) {
      ierr = TSTrajectorySetDirname(tj,loc1_dirname);CHKERRQ(ierr);
    }
    if (loc2_filetemplate) {
      ierr = TSTrajectorySetFiletemplate(tj,loc2_filetemplate);CHKERRQ(ierr);
    }
  }
