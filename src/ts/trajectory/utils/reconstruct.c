...
PetscErrorCode TSTrajectoryReconstruct_Private(TSTrajectory tj,TS ts,PetscReal t,Vec U,Vec Udot)
{
  ...
  if (id == -1 || id == -tshn - 1) {
    ...
    SETERRQ4(PetscObjectComm((PetscObject)tj),PETSC_ERR_PLIB,"Requested time %g is outside the history interval [%g, %g] (%" PetscInt_FMT ")",(double)t,(double)t0,(double)tf,tshn);
  }
  if (tj->monitor) {
