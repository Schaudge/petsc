...
static PetscErrorCode LandauFormJacobian_Internal(Vec a_X, Mat JacP, const PetscInt dim, PetscReal shift, void *a_ctx)
{
  ...
  if (Nq >LANDAU_MAX_NQ) SETERRQ2(ctx->comm,PETSC_ERR_ARG_WRONG,"Order too high. Nq = %" PetscInt_FMT " > LANDAU_MAX_NQ (%d)",Nq,LANDAU_MAX_NQ);
  if (LANDAU_DIM != dim) SETERRQ2(ctx->comm, PETSC_ERR_PLIB, "dim %" PetscInt_FMT " != LANDAU_DIM %d",dim,LANDAU_DIM);
  /* setup each grid */
  nip_glb = 0;
  for (PetscInt grid=0;grid<ctx->num_grids;grid++) {
  ...
  if (!ctx->initialized) { /* create static point data, Jacobian called first */
    ...
    if (ctx->verbose > 0) {
      ierr = PetscPrintf(ctx->comm,"0) %s: %" PetscInt_FMT " IPs, %" PetscInt_FMT " cells[0], Nb=%" PetscInt_FMT ", Nq=%" PetscInt_FMT ", dim=%" PetscInt_FMT ", Tab: Nb=%" PetscInt_FMT " Nf=%" PetscInt_FMT " Np=%" PetscInt_FMT " cdim=%" PetscInt_FMT " N=%" PetscInt_FMT "\n",
                         "FormLandau",nip_glb,numCells[0], Nb, Nq, dim, Tf[0]->Nb, ctx->num_species, Tf[0]->Np, Tf[0]->cdim, N);CHKERRQ(ierr);
    }
  ...
  if (shift==0) { /* create dynamic point data: f_alpha for closure of each cell (cellClosure[ngrids,ncells[g],f[Nb,ns[g]]]) or xdata */
    ...
    if (!ctx->gpu_assembly || !container) {
      ...
      if (cellClosure_it-cellClosure != cellClosure_sz) SETERRQ2(PETSC_COMM_SELF, PETSC_ERR_PLIB, "iteration wrong %td != cellClosure_sz = %" PetscInt_FMT "",cellClosure_it-cellClosure,cellClosure_sz);
  ...
  if (ctx->deviceType == LANDAU_CUDA || ctx->deviceType == LANDAU_KOKKOS) {
    ...
    if (shift==0.0) { /* compute dynamic data f and df and init data for Jacobian */
      ...
      if (cellClosure && ((cellClosure_it-cellClosure) != IPf_sz)) SETERRQ2(PETSC_COMM_SELF, PETSC_ERR_PLIB, "iteration wrong %td != nip_loc*Nf = %" PetscInt_FMT "",cellClosure_it-cellClosure,IPf_sz);
      if (IPf_idx != IPf_sz) SETERRQ2(PETSC_COMM_SELF, PETSC_ERR_PLIB, "IPf_idx != IPf_sz %" PetscInt_FMT " %" PetscInt_FMT "",IPf_idx,IPf_sz);
    ...
    for (PetscInt grid = 0, jpidx = 0 ; grid < ctx->num_grids ; grid++) {
      ...
      for (PetscInt ei = 0; ei < numCells[grid]; ++ei, invJ += Nq*dim*dim) {
        ...
        if (ei==-1) {
          ...
          for (d = 0; d < totDim; ++d) {
            for (f = 0; f < totDim; ++f) {ierr2 = PetscPrintf(ctx->comm," %12.5e",  (double)PetscRealPart(elemMat[d*totDim + f]));CHKERRQ(ierr2);}
      ...
      if (!container) {   // move nest matrix to global JacP
        ...
        if (PetscUnlikely(nloc != ctx->mat_offset[grid+1] - moffset)) SETERRQ2(PetscObjectComm((PetscObject) B), PETSC_ERR_PLIB, "nloc %" PetscInt_FMT " != ctx->mat_offset[grid+1] - moffset = %" PetscInt_FMT "",nloc,ctx->mat_offset[grid+1] - moffset);
        for (int i=0 ; i<nloc ; i++) {
          ierr = MatGetRow(B,i,&nzl,&cols,&vals);CHKERRQ(ierr);
          if (PetscUnlikely(nzl>1024)) SETERRQ1(PetscObjectComm((PetscObject) B), PETSC_ERR_PLIB, "Row too big: %" PetscInt_FMT "",nzl);
...
#define MAP_BF_SIZE (64*LANDAU_DIM*LANDAU_DIM*LANDAU_MAX_Q_FACE*LANDAU_MAX_SPECIES)
  if (ctx->gpu_assembly && !container) {
    ...
    ierr = PetscInfo(ctx->plex[0], "Make GPU maps 1\n");CHKERRQ(ierr);
              ...
              if (PetscAbs(PetscRealPart(elMat[f*numindices + f])) > PETSC_MACHINE_EPSILON) {
                ...
                if (PetscAbs(PetscRealPart(elMat[f*numindices + f] - 1.)) < PETSC_MACHINE_EPSILON) {
                  ...
                  if (PetscAbs(sum-1.0) > 10*PETSC_MACHINE_EPSILON) { // debug
                    int       d,f;
                    PetscReal tmp = 0;
                    PetscPrintf(PETSC_COMM_SELF,"\t\t%" PetscInt_FMT ".%" PetscInt_FMT ".%" PetscInt_FMT ") ERROR total I = %22.16e (LANDAU_MAX_Q_FACE=%d, #face=%d)\n",eidx,q,fieldA,(double)sum,LANDAU_MAX_Q_FACE,maps[grid].num_face);
                    for (d = 0, tmp = 0; d < numindices; ++d) {
                      if (tmp!=0 && PetscAbs(tmp-1.0) > 10*PETSC_MACHINE_EPSILON) {ierr = PetscPrintf(PETSC_COMM_WORLD,"%3d) %3" PetscInt_FMT ": ",d,indices[d]);CHKERRQ(ierr);}
                      for (f = 0; f < numindices; ++f) {
                        tmp += PetscRealPart(elMat[d*numindices + f]);
                      }
                      if (tmp!=0) {ierr = PetscPrintf(ctx->comm," | %22.16e\n",(double)tmp);CHKERRQ(ierr);}
                    }
                  }
...
static PetscErrorCode LandauDMCreateVMeshes(MPI_Comm comm_self, const PetscInt dim, const char prefix[], LandauCtx *ctx, DM *pack)
{
  ...
  if (len) { // not used, need to loop over grids
    ...
    if (LANDAU_DIM != dim2) SETERRQ2(comm_self, PETSC_ERR_PLIB, "dim %" PetscInt_FMT " != LANDAU_DIM %d",dim2,LANDAU_DIM);
  } else { /* p4est, quads */
...
static PetscErrorCode adaptToleranceFEM(PetscFE fem, Vec sol, PetscInt type, PetscInt grid, LandauCtx *ctx, DM *newForest)
{
  ...
  if (type==4) {
    ...
    for (c = cStart; c < cEnd; c++) {
      ...
      for (qj = 0; qj < Nq; ++qj) {
        ...
        if (r < minRad - PETSC_SQRT_MACHINE_EPSILON*10.) {
          ...
          nr = 0;
          rCellIdx[nr++]= c;
          ierr = PetscInfo4(sol, "\t\tPhase: adaptToleranceFEM Found first inner r=%e, cell %" PetscInt_FMT ", qp %" PetscInt_FMT "/%" PetscInt_FMT "\n", (double)r, c, qj+1, Nq);CHKERRQ(ierr);
        } else if ((r-minRad) < PETSC_SQRT_MACHINE_EPSILON*100. && nr < nrmax) {
          for (k=0;k<nr;k++) if (c == rCellIdx[k]) break;
          if (k==nr) {
            rCellIdx[nr++]= c;
            ierr = PetscInfo5(sol, "\t\t\tPhase: adaptToleranceFEM Found another inner r=%e, cell %" PetscInt_FMT ", qp %" PetscInt_FMT "/%" PetscInt_FMT ", d=%e\n", (double)r, c, qj+1, Nq, (double)(r-minRad));CHKERRQ(ierr);
          }
        }
        if (ctx->sphere) {
          if ((tt=r-ctx->e_radius) > 0) {
            PetscInfo2(sol, "\t\t\t %" PetscInt_FMT " cell r=%g\n",c,(double)tt);
    ...
    if (ctx->sphere) {
      for (c = 0; c < eMaxIdx; c++) {
        ...
        ierr = PetscInfo3(sol, "\t\tPhase:%s: refine sphere e cell %" PetscInt_FMT " r=%g\n","adaptToleranceFEM",eCellIdx[c],(double)eMinRad);CHKERRQ(ierr);
      }
      for (c = 0; c < iMaxIdx; c++) {
        ierr = DMLabelSetValue(adaptLabel, iCellIdx[c], DM_ADAPT_REFINE);CHKERRQ(ierr);
        ierr = PetscInfo3(sol, "\t\tPhase:%s: refine sphere i cell %" PetscInt_FMT " r=%g\n","adaptToleranceFEM",iCellIdx[c],(double)iMinRad);CHKERRQ(ierr);
      }
    }
    ierr = PetscInfo4(sol, "Phase:%s: Adaptive refine origin cells %" PetscInt_FMT ",%" PetscInt_FMT " r=%g\n","adaptToleranceFEM",rCellIdx[0],rCellIdx[1],(double)minRad);CHKERRQ(ierr);
  ...
  *newForest = adaptedDM;
  if (adaptedDM) {
    ...
    ierr = DMPlexGetHeightStratum(plex,0,&cStart,&cEnd);CHKERRQ(ierr);
    ierr = PetscInfo2(sol, "\tPhase: adaptToleranceFEM: %" PetscInt_FMT " cells, %" PetscInt_FMT " total quadrature points\n",cEnd-cStart,Nq*(cEnd-cStart));CHKERRQ(ierr);
    ierr = DMDestroy(&plex);CHKERRQ(ierr);
  } else  *newForest = NULL;
  PetscFunctionReturn(0);
}
...
static PetscErrorCode ProcessOptions(LandauCtx *ctx, const char prefix[])
{
  ...
  /* get num species with temperature*/
  {
    ...
    if (flg && nt > LANDAU_MAX_SPECIES) SETERRQ2(ctx->comm,PETSC_ERR_ARG_WRONG,"-thermal_temps ,t1,t2,.. number of species %" PetscInt_FMT " > MAX %d",nt,LANDAU_MAX_SPECIES);
  }
  ...
  if (flg) {
    PetscInfo1(dummy, "num_species set to number of thermal temps provided (%" PetscInt_FMT ")\n",nt);
    ctx->num_species = nt;
  ...
  if (flg && nm != ctx->num_species-1) {
    SETERRQ2(ctx->comm,PETSC_ERR_ARG_WRONG,"num ion masses %" PetscInt_FMT " != num species %" PetscInt_FMT "",nm,ctx->num_species-1);
  }
  ...
  if (flg && nm != ctx->num_species) SETERRQ2(ctx->comm,PETSC_ERR_ARG_WRONG,"wrong num n: %" PetscInt_FMT " != num species %" PetscInt_FMT "",nm,ctx->num_species);
  ...
  if (flg && nc != ctx->num_species-1) SETERRQ2(ctx->comm,PETSC_ERR_ARG_WRONG,"num charges %" PetscInt_FMT " != num species %" PetscInt_FMT "",nc,ctx->num_species-1);
  ...
  if (flg) {
    ...
    if (ctx->num_species != nt) SETERRQ4(ctx->comm,PETSC_ERR_ARG_WRONG,"-dm_landau_num_species_grid: sum %" PetscInt_FMT " != num_species = %" PetscInt_FMT ". %" PetscInt_FMT " grids (check that number of grids <= LANDAU_MAX_GRIDS = %d)",nt,ctx->num_species,ctx->num_grids,LANDAU_MAX_GRIDS);
  } else {
  ...
  if (ctx->species_offset[ctx->num_grids] != ctx->num_species) SETERRQ2(ctx->comm,PETSC_ERR_ARG_WRONG,"ctx->species_offset[ctx->num_grids] %" PetscInt_FMT " != ctx->num_species = %" PetscInt_FMT " ???????????",ctx->species_offset[ctx->num_grids],ctx->num_species);
  for (PetscInt grid = 0; grid < ctx->num_grids ; grid++) {
  ...
  if (flg && nt < ctx->num_grids) SETERRQ2(ctx->comm,PETSC_ERR_ARG_WRONG,"-dm_landau_domain_radius: given %" PetscInt_FMT " radius != number grids %" PetscInt_FMT "",nt,ctx->num_grids);
  for (PetscInt grid = 0; grid < ctx->num_grids ; grid++) {
    if (flg && ctx->radius[grid] <= 0) { /* negative is ratio of c */
      ...
      ierr = PetscInfo2(dummy, "Change domain radius to %e for grid %" PetscInt_FMT "\n",(double)(ctx->radius[grid]),grid);CHKERRQ(ierr);
    }
  ...
  if (flg && nt < ctx->num_grids) SETERRQ2(ctx->comm,PETSC_ERR_ARG_WRONG,"-dm_landau_amr_levels_max: given %" PetscInt_FMT " != number grids %" PetscInt_FMT "",nt,ctx->num_grids);
  nt = LANDAU_MAX_GRIDS;
  ...
  if (flg && ctx->num_grids != nt) SETERRQ2(ctx->comm,PETSC_ERR_ARG_WRONG,"-dm_landau_i_radius: %" PetscInt_FMT " != num_species = %" PetscInt_FMT "",nt,ctx->num_grids);
  if (ctx->sphere && ctx->e_radius <= ctx->i_radius[0]) SETERRQ3(ctx->comm,PETSC_ERR_ARG_WRONG,"bad radii: %g < %g < %g",(double)(ctx->i_radius[0]),(double)ctx->e_radius,(double)(ctx->radius[0]));
  /* processing options */
  ...
  if (ctx->verbose > 0) {
    ierr = PetscPrintf(ctx->comm, "masses:        e=%10.3e; ions in proton mass units:   %10.3e %10.3e ...\n",(double)(ctx->masses[0]),(double)(ctx->masses[1]/1.6720e-27),ctx->num_species>2 ? (double)(ctx->masses[2]/1.6720e-27) : 0);CHKERRQ(ierr);
    ierr = PetscPrintf(ctx->comm, "charges:       e=%10.3e; charges in elementary units: %10.3e %10.3e\n", (double)(ctx->charges[0]),(double)(-ctx->charges[1]/ctx->charges[0]),ctx->num_species>2 ? (double)(-ctx->charges[2]/ctx->charges[0]) : 0);CHKERRQ(ierr);
    ierr = PetscPrintf(ctx->comm, "thermal T (K): e=%10.3e i=%10.3e %10.3e. v_0=%10.3e (%10.3ec) n_0=%10.3e t_0=%10.3e, %s, %s\n", (double)(ctx->thermal_temps[0]), (double)(ctx->thermal_temps[1]), (ctx->num_species>2) ? (double)(ctx->thermal_temps[2]) : 0, (double)ctx->v_0, (double)(ctx->v_0/SPEED_OF_LIGHT), (double)ctx->n_0, (double)ctx->t_0, ctx->use_relativistic_corrections ? "relativistic" : "classical", ctx->use_energy_tensor_trick ? "Use trick" : "Intuitive");CHKERRQ(ierr);
    ierr = PetscPrintf(ctx->comm, "Domain radius (AMR levels) grid 0: %10.3e (%" PetscInt_FMT ") ",(double)(ctx->radius[0]),ctx->numAMRRefine[0]);CHKERRQ(ierr);
    for (ii=1;ii<ctx->num_grids;ii++) PetscPrintf(ctx->comm, ", %" PetscInt_FMT ": %10.3e (%" PetscInt_FMT ") ",ii,(double)(ctx->radius[ii]),ctx->numAMRRefine[ii]);
...
PetscErrorCode LandauPrintNorms(Vec X, PetscInt stepi)
{
  ...
  if (dim!=2 && dim!=3) SETERRQ1(PETSC_COMM_SELF, PETSC_ERR_PLIB, "dim= %" PetscInt_FMT "",dim);
  ...
  for (PetscInt grid = 0; grid < ctx->num_grids ; grid++) {
    ...
    for (ii=ctx->species_offset[grid],i0=0;ii<ctx->species_offset[grid+1];ii++,i0++) {
      ...
      if (dim==2) { /* 2/3X + 3V (cylindrical coordinates) */
        ...
        ierr = PetscPrintf(ctx->comm, "%3" PetscInt_FMT ") species-%" PetscInt_FMT ": charge density= %20.13e z-momentum= %20.13e energy= %20.13e",stepi,ii,(double)PetscRealPart(density[ii]),(double)PetscRealPart(zmomentum[ii]),(double)PetscRealPart(energy[ii]));CHKERRQ(ierr);
      } else { /* 2/3Xloc + 3V */
        ...
        if (ctx->use_relativistic_corrections) {
          ...
          if (ii==0 && grid==0) { // do all at once
            ...
            for (PetscInt grid = 0; grid < ctx->num_grids ; grid++) { // yes a grid loop in a grid loop to print nice
              ...
              if (bs != Nf) SETERRQ2(PETSC_COMM_SELF, PETSC_ERR_PLIB, "bs %" PetscInt_FMT " != num_species %" PetscInt_FMT " in Gsub",bs,Nf);
              ierr = VecGetBlockSize(Mfsub,&bs);CHKERRQ(ierr);
              if (bs != Nf) SETERRQ2(PETSC_COMM_SELF, PETSC_ERR_PLIB, "bs %" PetscInt_FMT " != num_species %" PetscInt_FMT "",bs,Nf);
        ...
        ierr = PetscPrintf( ctx->comm, "%3" PetscInt_FMT ") species %" PetscInt_FMT ": density=%20.13e, x-momentum=%20.13e, y-momentum=%20.13e, z-momentum=%20.13e, energy=%21.13e",
                            stepi,ii,(double)PetscRealPart(density[ii]),(double)PetscRealPart(xmomentum[ii]),(double)PetscRealPart(ymomentum[ii]),(double)PetscRealPart(zmomentum[ii]),(double)PetscRealPart(energy[ii]));CHKERRQ(ierr);
  ...
  if (ctx->verbose > 1) {ierr = PetscPrintf(ctx->comm,", %" PetscInt_FMT " sub (vector) threads\n",ctx->subThreadBlockSize);CHKERRQ(ierr);}
...
PetscErrorCode LandauCreateColoring(Mat JacP, DM plex, PetscContainer *container)
{
  ...
  for (cell = cStart, ej = 0 ; cell < cEnd; ++cell, ++ej) {
    ...
    if (numindices>128) SETERRQ1(PETSC_COMM_SELF, PETSC_ERR_PLIB, "too many indices. %" PetscInt_FMT " > 128",numindices);
  ...
  if (ctx && ctx->verbose > 0) {
    ierr = PetscPrintf(ctx->comm, "Made coloring with %" PetscInt_FMT " colors\n", nc);CHKERRQ(ierr);
  }
  PetscFunctionReturn(0);
}
...
PetscErrorCode LandauAssembleOpenMP(PetscInt cStart, PetscInt cEnd, PetscInt totDim, DM plex, PetscSection section, PetscSection globalSection, Mat JacP, PetscScalar elemMats[], PetscContainer container)
{
  ...
  for (colour=0; colour<nc; colour++) {
    ...
    ierr = ISGetLocalSize(is[colour],&csize);CHKERRQ(ierr);
    if (csize>1024) SETERRQ1(PETSC_COMM_SELF, PETSC_ERR_PLIB, "too many elements in color. %" PetscInt_FMT " > 1024",csize);
...
PetscErrorCode LandauCreateMassMatrix(DM pack, Mat *Amat)
{
  ...
  if (N1 != N2) SETERRQ2(PetscObjectComm((PetscObject) pack), PETSC_ERR_PLIB, "Incorrect matrix sizes: |Jacobian| = %" PetscInt_FMT ", |Mass|=%" PetscInt_FMT "",N1,N2);
  ...
  for (PetscInt grid=0 ; grid<ctx->num_grids ; grid++) {
    ...
    for (int i=0 ; i<nloc ; i++) {
      ierr = MatGetRow(B,i,&nzl,&cols,&vals);CHKERRQ(ierr);
      if (nzl>1024) SETERRQ1(PetscObjectComm((PetscObject) pack), PETSC_ERR_PLIB, "Row too big: %" PetscInt_FMT "",nzl);
...
PetscErrorCode LandauIFunction(TS ts, PetscReal time_dummy, Vec X, Vec X_t, Vec F, void *actx)
{
  ...
  ierr = DMGetDimension(pack, &dim);CHKERRQ(ierr);
  if (!ctx->aux_bool) {
    ierr = PetscInfo3(ts, "Create Landau Jacobian t=%g X=%p %s\n",(double)time_dummy,X_t,ctx->aux_bool ? " -- seems to be in line search" : "");CHKERRQ(ierr);
