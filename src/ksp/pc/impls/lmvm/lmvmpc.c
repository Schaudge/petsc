...
typedef struct {
  ...
  PetscBool        needtocallsetfromoptions;
} PC_LMVM;
...
static PetscErrorCode PCSetUp_LMVM(PC pc)
{
  ...
  if (B == ctx->B && ctx->needtocallsetfromoptions) PetscCall(MatSetFromOptions(ctx->B));
  ctx->needtocallsetfromoptions = PETSC_FALSE;
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
static PetscErrorCode PCSetFromOptions_LMVM(PC pc, PetscOptionItems *PetscOptionsObject)
{
  ...
  PetscFunctionBegin;
  /* defer setfromoptions to PCSetUp_LMVM */
  ctx->needtocallsetfromoptions = PETSC_TRUE;
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
.seealso: `PCCreate()`, `PCSetType()`, `PCLMVMSetUpdateVec()`, `PCLMVMSetMatLMVM()`, `PCLMVMGetMatLMVM()`
M*/
