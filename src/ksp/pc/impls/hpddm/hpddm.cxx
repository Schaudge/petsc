...
static PetscErrorCode PCHPDDMCreateSubMatrices_Private(Mat mat, PetscInt n, const IS*, const IS*, MatReuse scall, Mat *submat[])
{
  ...
  PetscFunctionBegin;
  if (n != 1) SETERRQ1(PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "MatCreateSubMatrices() called to extract %" PetscInt_FMT " submatrices, which is different than 1", n);
    ...
    if (data->N > 1 && (data->aux || ismatis || algebraic)) {
      ...
      if (!ismatis && sub == &data->aux && !data->B && subdomains && data->share) {
        ...
        if (size != 1) {
          ...
          } else SETERRQ1(PETSC_COMM_SELF, PETSC_ERR_PLIB, "Number of subdomain solver %" PetscInt_FMT " != 1", size);
        } else {
          Mat        D;
          const char *matpre;
          PetscBool  cmp[2];
