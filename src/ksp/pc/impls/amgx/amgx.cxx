...
static PetscErrorCode PCSetUp_AMGX(PC pc)
{
  ...
  PetscCall(PetscObjectTypeCompareAny((PetscObject)amgx->localA, &is_dev_ptrs, VECCUDA, VECMPICUDA, VECSEQCUDA, ""));
...
/*MC
     PCAMGX - AMGX (i.e. AMG preconditioning)
   ...
   Options Database Key:
    ...
    -pc_amgx_gs_symmetric - enforce symmetric Gauss Seidel smoothing (only applies if GS smoothing selected)
    -pc_amgx_selector <SIZE_2,SIZE_4,SIZE_8,MULTI_PAIRWISE,PMIS,HMIS> - set the AMG coarse selector
    -pc_amgx_presweeps - set the number of AMG presweeps
    -pc_amgx_postsweeps - set the number of AMG postsweeps
...
PETSC_EXTERN PetscErrorCode PCCreate_AMGX(PC pc)
{
  ...
  amgx->exact_coarse_solve = PETSC_FALSE;
  amgx->presweeps = 1;
  amgx->postsweeps = 1;
  amgx->max_levels = 100;
