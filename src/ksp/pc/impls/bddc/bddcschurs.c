...
PetscErrorCode PCBDDCSubSchursSetUp(PCBDDCSubSchurs sub_schurs, Mat Ain, Mat Sin, PetscBool exact_schur, PetscInt xadj[], PetscInt adjncy[], PetscInt nlayers, Vec scaling, PetscBool compute_Stilda, PetscBool reuse_solvers, PetscBool benign_trick, PetscInt benign_n, PetscInt benign_p0_lidx[], IS benign_zerodiag_subs[], Mat change, IS change_primal)
{
  ...
  if (nlayers >= 0 && i) { /* Interior problems can be different from the original one */
    ...
    for (layer=0;layer<nlayers;layer++) {
      ...
      if (n_local_dofs > n_I+n_B) SETERRQ3(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Error querying layer %" PetscInt_FMT ". Out of bound access (%" PetscInt_FMT " > %" PetscInt_FMT ")",layer,n_local_dofs,n_I+n_B);
  ...
  if (i != local_stash_size) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Invalid size of new subset! %" PetscInt_FMT " != %" PetscInt_FMT "",i,local_stash_size);
  ...
  if (!sub_schurs->is_Ej_all) {
    ...
    if (subset_size != local_size) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Error in sub_schurs serial (BtoNmap)! %" PetscInt_FMT " != %" PetscInt_FMT "",subset_size,local_size);
      ...
      if (benign_n) {
        ...
        for (i=0;i<benign_n;i++) {
          ...
          if (matl_dbg_viewer) {
            ...
            ierr = PetscSNPrintf(name,sizeof(name),"Pvs%" PetscInt_FMT "",i);CHKERRQ(ierr);
  ...
  if (matl_dbg_viewer) {
    ...
    for (i=0,cum=0;i<sub_schurs->n_subs;i++) {
      ...
      ierr = PetscSNPrintf(name,sizeof(name),"IE%" PetscInt_FMT "",i);CHKERRQ(ierr);
      ierr = ISGetLocalSize(sub_schurs->is_subs[i],&subset_size);CHKERRQ(ierr);
...
PetscErrorCode PCBDDCSubSchursInit(PCBDDCSubSchurs sub_schurs, const char* prefix, IS is_I, IS is_B, PCBDDCGraph graph, ISLocalToGlobalMapping BtoNmap, PetscBool copycc)
{
...
#if defined(PETSC_HAVE_MUMPS)
  ierr = PetscStrncpy(sub_schurs->mat_solver_type,MATSOLVERMUMPS,sizeof(sub_schurs->mat_solver_type));CHKERRQ(ierr);
#elif defined(PETSC_HAVE_MKL_PARDISO)
  ierr = PetscStrncpy(sub_schurs->mat_solver_type,MATSOLVERMKL_PARDISO,sizeof(sub_schurs->mat_solver_type));CHKERRQ(ierr);
#else
  ierr = PetscStrncpy(sub_schurs->mat_solver_type,MATSOLVERPETSC,sizeof(sub_schurs->mat_solver_type));CHKERRQ(ierr);
#endif
#if defined(PETSC_USE_COMPLEX)
  ...
  ierr = PetscOptionsString("-sub_schurs_mat_solver_type","Specific direct solver to use",NULL,sub_schurs->mat_solver_type,sub_schurs->mat_solver_type,sizeof(sub_schurs->mat_solver_type),NULL);CHKERRQ(ierr);
