  ...
  if (sub_schurs->is_vertices || sub_schurs->is_dir) {
    ...
    if (!deluxe_ctx->n_simple) {
      ...
      if (sub_schurs->is_vertices) {
        ...
        if (nmap != n_com) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Error when mapping simply scaled dofs (is_vertices)! %" PetscInt_FMT " != %" PetscInt_FMT "",nmap,n_com);
      ...
      if (sub_schurs->is_dir) {
        ...
        if (nmap != n_dir) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Error when mapping simply scaled dofs (sub_schurs->is_dir)! %" PetscInt_FMT " != %" PetscInt_FMT "",nmap,n_dir);
      ...
      if (deluxe_ctx->n_simple != n_dir + n_com) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Number of simply scaled dofs %" PetscInt_FMT " is different from the previous one computed %" PetscInt_FMT "",n_dir + n_com,deluxe_ctx->n_simple);
    }
  } else {
    deluxe_ctx->n_simple = 0;
...
static PetscErrorCode PCBDDCScalingSetUp_Deluxe_Private(PC pc)
{
  ...
  if (!deluxe_ctx->seq_n) {
    ...
    newsetup = PETSC_TRUE;
  } else if (deluxe_ctx->seq_n != sub_schurs->n_subs) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Number of deluxe subproblems %" PetscInt_FMT " is different from the sub_schurs %" PetscInt_FMT "",deluxe_ctx->seq_n,sub_schurs->n_subs);
