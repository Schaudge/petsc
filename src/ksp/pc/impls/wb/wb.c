...
PetscErrorCode DMDAGetWireBasketInterpolation(PC pc,DM da,PC_Exotic *exotic,Mat Aglobal,MatReuse reuse,Mat *P)
{
  ...
  */
  ...
  for (i=0; i<Nsurf; i++) {
    ...
    if (PetscAbsScalar(tmp-1.0) > 1.e-10) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Wrong Xsurf interpolation at i %" PetscInt_FMT " value %g",i,(double)PetscAbsScalar(tmp));
  }
#endif
...
#define Endpoint(a,start,b) (a == 0 || a == (b-1-start))
  ...
  */
  ...
  for (i=0; i<Nint; i++) {
    ...
    if (PetscAbsScalar(tmp-1.0) > 1.e-10) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Wrong Xint interpolation at i %" PetscInt_FMT " value %g",i,(double)PetscAbsScalar(tmp));
  }
  ...
  */
  ...
  if (cnt != Ntotal) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Hash table size %" PetscInt_FMT " not equal to total number coarse grid points %" PetscInt_FMT "",cnt,Ntotal);
...
#if defined(PETSC_USE_DEBUG_foo)
  {
    ...
    for (i=0; i<Ng; i++) {
      if (PetscAbsScalar(yy[i]-1.0) > 1.e-10) SETERRQ2(PETSC_COMM_SELF,PETSC_ERR_PLIB,"Wrong p interpolation at i %" PetscInt_FMT " value %g",i,(double)PetscAbsScalar(yy[i]));
    }
