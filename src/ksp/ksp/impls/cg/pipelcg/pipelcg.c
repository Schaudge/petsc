...
static PetscErrorCode KSPSolve_InnerLoop_PIPELCG(KSP ksp)
{
  ...
  for (it = 0; it < max_it + l; ++it) {
    ...
    if (it < l) {
      ...
      PetscCall(MPIU_Iallreduce(MPI_IN_PLACE, &G(0, it + 1), (PetscMPIInt)(it + 2), MPIU_SCALAR, MPIU_SUM, comm, &req(it + 1)));
      ...
      PetscCall(MPIU_Iallreduce(MPI_IN_PLACE, &G(it - l + 1, it + 1), (PetscMPIInt)(l + 1), MPIU_SCALAR, MPIU_SUM, comm, &req(it + 1)));
    }
...
static PetscErrorCode KSPSolve_PIPELCG(KSP ksp)
{
  ...
  while (ksp->its < max_it) { /* OUTER LOOP (gmres-like restart to handle breakdowns) */
    ...
    PetscCall(MPIU_Iallreduce(MPI_IN_PLACE, &G(0, 0), 1, MPIU_SCALAR, MPIU_SUM, comm, &req(0)));
