...
static PetscErrorCode KSPSolve_PREONLY(KSP ksp)
{
  ...
  PetscCheck(ksp->guess_zero != KSP_GUESS_NONZERO, PetscObjectComm((PetscObject)ksp), PETSC_ERR_USER, "Running KSP of preonly doesn't make sense with nonzero initial guess\n\
...
PETSC_EXTERN PetscErrorCode KSPCreate_PREONLY(KSP ksp)
{
  ...
  ksp->ops->view           = NULL;
  ksp->guess_zero          = KSP_GUESS_ZERO_DONT_SET;
  PetscFunctionReturn(PETSC_SUCCESS);
}
