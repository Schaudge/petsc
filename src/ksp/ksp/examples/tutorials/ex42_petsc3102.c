...
static PetscErrorCode PCMGSetupViaCoarsen(PC pc,KSP ksp,DM da_fine)
{
  ...
  for (k=0; k<nlevels; k++) {
    ...
    PetscReal Wi=30.1, Le=3.0*25.0, He=4.75, l=0.5; // TopBridge (TopOpt)
    PetscReal xmin=0.0, xmax=Le+2*l, ymin=0.0, ymax=Wi, zmin=0.0, zmax=He; // TopBridge (TopOpt)
    ierr = DMDASetUniformCoordinates(da_list[k],xmin,xmax,ymin,ymax,zmin,zmax);CHKERRQ(ierr);
  }
  ...
  if(rank==0) { std::cout<<">>> using PCMG Galerkin"<<std::endl; }
  ...
  PetscScalar coarse_rtol = 1.0e-3*rtol;// Make finer
  PetscScalar coarse_atol = 1.0e-50;
  PetscScalar coarse_dtol = 1e3;
  PetscInt coarse_maxits = 200;  
  PetscInt coarse_restart = 200; 
...
static PetscErrorCode solve_stokes_3d_coupled(PetscInt mx,PetscInt my,PetscInt mz)
{
  ...
  int rank; MPI_Comm_rank(PETSC_COMM_WORLD,&rank);
  if(rank==0) { std::cout<<std::endl<<"------------------------------------------------------------------------------------------ Building DMDA, cell properties, quadrature points,..."<<std::endl; }

  ierr          = DMDACreate3d(PETSC_COMM_WORLD,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DM_BOUNDARY_NONE,DMDA_STENCIL_BOX,
                               mx+1,my+1,mz+1,PETSC_DECIDE,PETSC_DECIDE,PETSC_DECIDE,dof,stencil_width,NULL,NULL,NULL,&da_Stokes);CHKERRQ(ierr);

  //!MP
  ierr = DMSetFromOptions(da_Stokes);
  ierr = DMSetUp(da_Stokes);
  ...
  if(rank==0) {std::cout<<std::endl<<"------------------------------------------------------------------------------------------ Building matrix..."<<std::endl; }
  ...
  if(rank==0) { std::cout<<std::endl<<"------------------------------------------------------------------------------------------ Solving system..."<<std::endl; }

  /* SOLVE */
  ierr = KSPCreate(PETSC_COMM_WORLD,&ksp_S);CHKERRQ(ierr); //ex42
  /*ierr = KSPSetOptionsPrefix(ksp_S,"stokes_"); /* stokes */ CHKERRQ(ierr); //ex42*/

  // TopBridge ------------------------------------------------------------------------------------------------------------------
  // The fine grid solver settings
  PetscScalar rtol = 1.0e-5; 
  PetscScalar atol = 1.0e-50;
  PetscScalar dtol = 1.0e15;
  PetscInt restart = 150; 
  PetscInt maxitsGlobal = 150;  

  ierr = KSPSetType(ksp_S,KSPGMRES);
  ierr = KSPGMRESSetRestart(ksp_S,restart);
  ierr = KSPSetTolerances(ksp_S,rtol,atol,dtol,maxitsGlobal);
  ierr = KSPSetInitialGuessNonzero(ksp_S,PETSC_TRUE);
  // ----------------------------------------------------------------------------------------------------------------------------
...

  {
    ...
    ierr = KSPGetPC(ksp_S,&pc);CHKERRQ(ierr);
    ierr = PCSetType(pc,PCMG); // TopBridge
    KSPSetFromOptions(ksp_S); // TopBridge
    ierr = PetscObjectTypeCompare((PetscObject)pc,PCMG,&same);CHKERRQ(ierr);
    if (same) {
      ierr = PCMGSetupViaCoarsen(pc,ksp_S,da_Stokes);CHKERRQ(ierr);
    }
  ...
  KSPType ksptype;
  KSPGetType(ksp_S,&ksptype);
  PCType pctype;
  PCGetType(pc,&pctype);
  PetscInt mmax;
  KSPGetTolerances(ksp_S,NULL,NULL,NULL,&mmax);
...

  {
    ...
    if (stokes_monitor) {
      int rank; MPI_Comm_rank(PETSC_COMM_WORLD,&rank);
      if(rank==0) std::cout<<" >>> KSPMonitorStokesBlocks "<<std::endl;
  ...
  {
    if(rank==0) std::cout<<" >>> write_output "<<std::endl;
...
int main(int argc,char **args)
{
  ...
  int rank; MPI_Comm_rank(PETSC_COMM_WORLD,&rank);
  if(rank==0) { std::cout<<"------------------------------------------------------------------------------------------ Problem size: "<<mx<<" x "<<my<<" x "<<mz<<" = "<<mx*my*mz<<std::endl; }
