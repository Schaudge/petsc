...
PETSC_INTERN PetscErrorCode MatMultTransposeColumnRange(Mat A, Vec xx, Vec yy, PetscInt c_start, PetscInt c_end)
{
  PetscFunctionBegin;
...
PETSC_INTERN PetscErrorCode MatMultTransposeAddColumnRange(Mat A, Vec xx, Vec zz, Vec yy, PetscInt c_start, PetscInt c_end)
{
  PetscFunctionBegin;
...
PETSC_INTERN PetscErrorCode VecCyclicShift(Mat B, Vec X, PetscInt d, Vec cyclic_work_vec)
{
  ...
  PetscCall(VecCopyAsync_Private(X, cyclic_work_vec, dctx));
...
PETSC_INTERN PetscErrorCode MatMove_LR3(Mat B, Mat R, PetscInt m_keep, Mat temp_mat)
{
  Mat_LMVM     *lmvm  = (Mat_LMVM*)B->data;
  PetscInt     M;
  ...
  PetscFunctionBegin;
  if (!temp_mat) PetscCall(MatDuplicate(R, MAT_SHARE_NONZERO_PATTERN, &temp_mat));
  PetscCall(MatGetLocalSize(R, &M, NULL));
  if (M == 0) PetscFunctionReturn(PETSC_SUCCESS);

  PetscCall(MatDenseGetLocalMatrix(R, &mat_local));
  PetscCall(MatDenseGetLocalMatrix(temp_mat, &local_temp));
