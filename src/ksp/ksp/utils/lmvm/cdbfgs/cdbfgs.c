...
/* MatTransposeMatMult, but for CDINPLACE, just updates idx col and idx row. */
static PetscErrorCode MtMT_Internal(Mat H, Mat S, Mat Y, MatReuse scall, PetscReal fill, Mat *STY)
{
...
//  MPI_Comm    comm  = PetscObjectComm((PetscObject)B);
  ...
  for (i=0; i<lmvm->m-1; i++) {
    ...
    case PETSC_MEMTYPE_HIP:
    {
      ...
      for (j=0; j<i+1; j++) {
        query_idx_j = (index + j) % lmvm->m;
        PetscCall(PetscDeviceArrayZero(dctx, &x_array[query_idx_j],nn));
      }

      PetscCall(PetscDeviceCalloc(dctx, memtype_x, lmvm->m-i-1, &buffer));
      for (j=0; j < lmvm->m-i-1; j++) {
        query_idx_j = query_idx_i+j+1  % lmvm->m;
	PetscInt try;
        ...
        if (r_array[query_idx_i] != 0) {
          ...
          // maybe create vector with x[j]/r[i], copy, and set zero elsewhere?
          PetscScalar *tmp1, *tmp2;
          PetscCall(PetscDeviceMalloc(dctx, memtype_x, nn, &tmp1));
          PetscCall(PetscDeviceMalloc(dctx, memtype_x, nn, &tmp2));
	  PetscCall(PetscDeviceMemcpy(dctx, tmp1, &x_array[query_idx_i], sizeof(PetscScalar)));
	  PetscCall(PetscDeviceMemcpy(dctx, tmp2, &r_array[query_idx_i], sizeof(PetscScalar)));
//          PetscCall(PetscDeviceArrayCopy(dctx, tmp1, &x_array[query_idx_j], nn));
//          PetscCall(PetscDeviceArrayCopy(dctx, tmp2, &r_array[query_idx_i], nn));
	  *tmp1 /= *tmp2;
          PetscCall(PetscDeviceMemcpy(dctx, &buffer[j], tmp1, nn));
          PetscCall(PetscDeviceFree(dctx, tmp1));
          PetscCall(PetscDeviceFree(dctx, tmp2));
        } else {
          PetscCall(PetscDeviceArrayZero(dctx, &buffer[j],nn));
        }
      }
      for (j=0, k=i+1; k<lmvm->m; k++, j++) {
        query_idx_j = (index + j) % lmvm->m;
...
//  PetscCall(VecCopy(lbfgs->lwork1, dX));//see what happens
  ...
  PetscMemType ttt, ttt2;
  PetscReal *xxx, *xxx2;
  VecGetArrayAndMemType(lbfgs->diag_vec, &xxx, &ttt);
  VecRestoreArrayAndMemType(lbfgs->diag_vec, &xxx);
  VecGetArrayAndMemType(rwork1_host, &xxx2, &ttt2);
  VecRestoreArrayAndMemType(rwork1_host, &xxx2);
  PetscCall(VecPointwiseMult(rwork1_host, lbfgs->diag_vec, rwork1_host));//technically can have diagvec on host, but one memcpy to avoid one cudasync
  if (lbfgs->bind) {
...
static PetscErrorCode MatRotate_STY_CCW(Mat R)
{
  ...
  VecType      vec_type;
  PetscMemType memtype_vec, memtype;
  MPI_Comm     comm = PetscObjectComm((PetscObject)R);
  ...
  b = lbfgs->idx_rplc;
  PetscCall(PetscGetMemType(lmvm->Xprev, &memtype));
  ...
  if (allocate) {
    ...
    if (lmvm->m > 0) {
      ...
      if (lbfgs->bind) {
        ...
        PetscCall(MatCreateDenseFromVecType(comm, vec_type, lmvm->m, lmvm->m, lmvm->m, lmvm->m, -1, NULL, &lbfgs->StYfull_device));//TODO nonsquare... LDA???
      } else {
