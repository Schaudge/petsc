...
public:
  static PetscErrorCode MultInPlace(PetscDeviceContext, PetscBool, PetscInt, const PetscScalar[], PetscInt, PetscScalar[], PetscInt) noexcept;
  static PetscErrorCode MultInPlaceCyclic(PetscDeviceContext, PetscBool, PetscInt, PetscInt, const PetscScalar[], PetscInt, PetscScalar[], PetscInt) noexcept;
...
template <DeviceType T>
PetscErrorCode UpperTriangular<T>::MultInPlace(PetscDeviceContext dctx, PetscBool hermitian_transpose, PetscInt N, const PetscScalar A[], PetscInt lda, PetscScalar x[], PetscInt stride) noexcept
{
  cupmBlasInt_t      n;
  ...
  PetscCallCUPMBLAS(cupmBlasXtrmv(handle, CUPMBLAS_FILL_MODE_UPPER, hermitian_transpose ? CUPMBLAS_OP_C : CUPMBLAS_OP_N, CUPMBLAS_DIAG_NON_UNIT, n, A, lda, x, stride));
...
template <DeviceType T>
PetscErrorCode UpperTriangular<T>::MultInPlaceCyclic(PetscDeviceContext dctx, PetscBool hermitian_transpose, PetscInt N, PetscInt oldest_index, const PetscScalar A[], PetscInt lda, PetscScalar x[], PetscInt stride) noexcept
{
  ...
  if (!hermitian_transpose) {
    PetscCallCUPMBLAS(cupmBlasXtrmv(handle, CUPMBLAS_FILL_MODE_UPPER, CUPMBLAS_OP_N, CUPMBLAS_DIAG_NON_UNIT, n_old, &A[oldest_index * (lda + 1)], lda, &x[oldest_index], stride));
    PetscCallCUPMBLAS(cupmBlasXgemv(handle, CUPMBLAS_OP_N, n_old, n_new, &sone, &A[oldest_index], lda, x, stride, &sone, &x[oldest_index], stride));
    PetscCallCUPMBLAS(cupmBlasXtrmv(handle, CUPMBLAS_FILL_MODE_UPPER, CUPMBLAS_OP_N, CUPMBLAS_DIAG_NON_UNIT, n_new, A, lda, x, stride));
  } else {
    PetscCallCUPMBLAS(cupmBlasXtrmv(handle, CUPMBLAS_FILL_MODE_UPPER, CUPMBLAS_OP_C, CUPMBLAS_DIAG_NON_UNIT, n_new, A, lda, x, stride));
    PetscCallCUPMBLAS(cupmBlasXgemv(handle, CUPMBLAS_OP_C, n_old, n_new, &sone, &A[oldest_index], lda, &x[oldest_index], stride, &sone, x, stride));
    PetscCallCUPMBLAS(cupmBlasXtrmv(handle, CUPMBLAS_FILL_MODE_UPPER, CUPMBLAS_OP_C, CUPMBLAS_DIAG_NON_UNIT, n_old, &A[oldest_index * (lda + 1)], lda, &x[oldest_index], stride));
  }
...
PETSC_INTERN PetscErrorCode MatUpperTriangularMultInPlace_CUPM(PetscBool hermitian_transpose, PetscInt n, const PetscScalar A[], PetscInt lda, PetscScalar x[], PetscInt stride)
{
  ...
  case PETSC_DEVICE_CUDA:
    PetscCall(UpperTriangular<DeviceType::CUDA>::MultInPlace(dctx, hermitian_transpose, n, A, lda, x, stride));
    break;
#endif
#if PetscDefined(HAVE_HIP)
  case PETSC_DEVICE_HIP:
    PetscCall(UpperTriangular<DeviceType::HIP>::MultInPlace(dctx, hermitian_transpose, n, A, lda, x, stride));
    break;
#endif
  default:
