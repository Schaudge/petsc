...
int main(int argc, char **argv)
{
  ...
  PetscCall(PetscObjectTypeCompare((PetscObject)J0, MATVECDIAGONAL, &is_vectordiag));
  ...
  PetscCall(PetscObjectTypeCompare((PetscObject)B, MATLMVMSYMBROYDEN, &is_symbrdnphi));
  if (is_symbrdnphi) { test_type = TEST_SYMBRDNPHI; }

  PetscBool inverse = PETSC_FALSE;
  PetscCall(PetscObjectTypeCompareAny((PetscObject)B, &inverse, MATLMVMBADBROYDEN, MATLMVMBFGS, ""));
...
/*TEST
  ...
  test:
    ...
    nsize: 2
    args: -m 15 -n 15 -n_iter 5 -B_mat_type lmvmsr1 -B_lmvm_J0_mat_type dense -B_lmvm_J0_pc_type bjacobi -B_lmvm_J0_sub_pc_type lu -B_lmvm_J0_ksp_type gmres -B_lmvm_J0_ksp_max_it 15 -B_lmvm_J0_ksp_rtol 0.0 -B_lmvm_J0_ksp_atol 0.0
  ...
  test:
    ...
    output_file: output/ex1_square.out
    nsize: 2
    args: -m 15 -n 15 -n_iter 5 -B_mat_type lmvmsr1 -B_lmvm_J0_mat_type {{constantdiagonal vecdiagonal}}
