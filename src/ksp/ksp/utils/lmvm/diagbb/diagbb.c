...
static PetscErrorCode MatScale_DiagBB(Mat B, PetscScalar alpha)
{
  ...
  PetscFunctionBegin;
  ierr = VecScale(lbb->invD, alpha);CHKERRQ(ierr);
...
static PetscErrorCode MatUpdate_DiagBB(Mat B, Vec X, Vec F)
{
  ...
  if (lmvm->prev_set) {
    ...
    ierr = VecPointwiseMult(lbb->temp2,lmvm->Fprev,lmvm->Fprev);CHKERRQ(ierr);
    ierr = VecShift(lbb->temp2,lbb->mu);CHKERRQ(ierr);
    ierr = VecPointwiseDivide(lbb->temp,lbb->temp,lbb->temp2);CHKERRQ(ierr);
  
    ierr = VecGetArrayPair(lbb->temp, lbb->invD, &inarray, &outarray);CHKERRQ(ierr);
    ierr = VecGetLocalSize(lbb->invD, &nlocal);CHKERRQ(ierr);
    ...
    ierr = VecRestoreArrayPair(lbb->temp, lbb->invD, &inarray, &outarray);CHKERRQ(ierr);
    /* End DiagBB update */
  }
...
static PetscErrorCode MatReset_DiagBB(Mat B, PetscBool destructive)
{
  ...
  PetscFunctionBegin;
  ierr = VecSet(lbb->invD, 1.);CHKERRQ(ierr);
