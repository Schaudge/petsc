...
int main(int argc,char **args)
{
  ...
  if (norm > tol) {
    ierr = PetscPrintf(PETSC_COMM_WORLD,"1. Norm of error for Ax=b: %g, Iterations %" PetscInt_FMT "\n",(double)norm,its);CHKERRQ(ierr);
  }
  ...
  ierr = KSPGetIterationNumber(ksp,&its);CHKERRQ(ierr);
  if (norm > tol) {
    ierr = PetscPrintf(PETSC_COMM_WORLD,"2. Norm of error for A^T x=b: %g, Iterations %" PetscInt_FMT "\n",(double)norm,its);CHKERRQ(ierr);
  }
  ...
  ierr = KSPGetIterationNumber(ksp,&its);CHKERRQ(ierr);
  if (norm > tol) {
    ierr = PetscPrintf(PETSC_COMM_WORLD,"3. Norm of error for (A+Delta) x=b: %g, Iterations %" PetscInt_FMT "\n",(double)norm,its);CHKERRQ(ierr);
  }
