...
PetscScalar ExactSolution(PetscReal coords[3], UserContext* user)
{
  ...
  case 2:
  default:
    SETERRQ1(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "Exact solution for -problem = [%" PetscInt_FMT "] is not available.\n", user->problem);
  }
}
...
PetscErrorCode ComputeRHS(KSP ksp, Vec b, void *ptr)
{
  ...
  for (moab::Range::iterator iter = elocal->begin(); iter != elocal->end(); iter++) {
    ...
    if (nconn != 3 && nconn != 4) SETERRQ1(PETSC_COMM_WORLD, PETSC_ERR_ARG_WRONG, "Only TRI3/QUAD4 element bases are supported in the current example. n(Connectivity)=%" PetscInt_FMT ".\n", nconn);
...
PetscErrorCode ComputeMatrix(KSP ksp, Mat J, Mat jac, void *ctx)
{
  ...
  for (moab::Range::iterator iter = elocal->begin(); iter != elocal->end(); iter++) {
    ...
    // Get connectivity information:
    ierr = DMMoabGetElementConnectivity(dm, ehandle, &nconn, &connect);CHKERRQ(ierr);
    if (nconn != 3 && nconn != 4) SETERRQ1(PETSC_COMM_WORLD, PETSC_ERR_ARG_WRONG, "Only QUAD4 or TRI3 element bases are supported in the current example. Connectivity=%" PetscInt_FMT ".\n", nconn);
