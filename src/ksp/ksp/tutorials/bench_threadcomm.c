...
typedef struct {
  ...
  PetscBool   printTiming;
  PetscBool   useThreadcomm;
} AppCtx;

typedef struct {
  MPI_Comm           comm;
  PetscInt           M, N; // M, N, a,i,j of the shared matrix B
  const PetscInt    *i, *j;
  const PetscScalar *a;
  AppCtx             user;
} ThreadInfo;

static PetscErrorCode Solve(ThreadInfo *tinfo)
{
  MPI_Comm                   comm = tinfo->comm;
  PetscInt                   M = tinfo->M, N = tinfo->N;
  const PetscInt            *Bi   = tinfo->i;
  const PetscInt            *Bj   = tinfo->j;
  const PetscScalar         *Ba   = tinfo->a;
  ...
  // Scattter the input shared matrix to ranks in comm
  for (PetscInt i = rstart; i < rend; i++) {
    PetscInt nz = Bi[i + 1] - Bi[i]; // nz on row i
    PetscCall(MatSetValues(A, 1, &i, nz, &Bj[Bi[i]], &Ba[Bi[i]], INSERT_VALUES));
  }
  ...
  if (user.matmult) {
    ...
    if (!user.splitKSP) {
      ...
      /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
      PetscCall(PetscPrintf(comm, "Step2  - running KSPSolve()...\n"));
      ...
      /*  Measure only time of PCSetUp() and KSPSolve()                      */
      /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
      PetscCall(PetscPrintf(comm, "Step2a - running PCSetUp()...\n"));
    ...
    /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    PetscCall(PetscPrintf(comm, "Step3  - calculating error norm...\n"));
...
int main(int argc, char **argv)
{
  ...
  PetscCheck(user.size == 1, PETSC_COMM_WORLD, PETSC_ERR_WRONG_MPI_SIZE, "The test can only run with one mpi process");
  ...
  PetscCall(PetscOptionsGetBool(NULL, NULL, "-print_timing", &user.printTiming, NULL));
  ...
  /* cannot use MatGetOwnershipRange() because the matrix has yet to be
   * preallocated; that happens in MatSetPreallocateCOO() */
  PetscCall(PetscSplitOwnership(PetscObjectComm((PetscObject)B), &nlocal, &user.dim));
  PetscCallMPI(MPI_Scan(&nlocal, &user.Istart, 1, MPIU_INT, MPI_SUM, PetscObjectComm((PetscObject)B)));
  user.Istart -= nlocal;
...
#endif
  {
    tinfo.comm = PETSC_COMM_SELF;
    nthreads   = 1;
  }

  PetscCall(MatSeqAIJGetCSRAndMemType(B, &Bi, &Bj, &Ba, &mtype));
  PetscCheck(mtype == PETSC_MEMTYPE_HOST, PETSC_COMM_WORLD, PETSC_ERR_ARG_INCOMP, "Matrix must be MATSEQAIJ on host");
  tinfo.M    = user.dim;
  tinfo.N    = user.dim;
  tinfo.i    = Bi;
  tinfo.j    = Bj;
...
/*TEST
  ...
  test:
    ...
    nsize: {{1 3}}
    output_file: output/bench_threadcomm.out
