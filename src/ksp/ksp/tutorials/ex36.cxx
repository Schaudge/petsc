...
PetscErrorCode ComputeRHS_MOAB(KSP ksp, Vec b, void *ptr)
{
  ...
  for (moab::Range::iterator iter = elocal->begin(); iter != elocal->end(); iter++) {
    ...
    if (nconn != 4 && nconn != 8) SETERRQ1(PETSC_COMM_WORLD, PETSC_ERR_ARG_WRONG, "Only HEX8/TET4 element bases are supported in the current example. n(Connectivity)=%" PetscInt_FMT ".\n", nconn);
