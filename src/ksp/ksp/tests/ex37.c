...
int main(int argc,char **args)
{
  ...
  if (type == PETSC_SUBCOMM_GENERAL) { /* user provides color, subrank and duprank */
    ...
    if (subsize*nsubcomm != size) SETERRQ2(comm,PETSC_ERR_SUP,"This example requires nsubcomm %" PetscInt_FMT " divides size %d",nsubcomm,size);
  ...
  if (norm > 1.e-4 && rank == 0) {
    ierr = PetscPrintf(PETSC_COMM_WORLD,"[%d]  Number of iterations = %3" PetscInt_FMT "\n",rank,its);CHKERRQ(ierr);
