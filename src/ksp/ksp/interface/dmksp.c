...
PetscErrorCode DMGetDMKSP(DM dm,DMKSP *kspdm)
{
  ...
  *kspdm = (DMKSP) dm->dmksp;
  if (!*kspdm) {
    ...
    (*kspdm)->originaldm = dm;
    ierr                 = DMCoarsenHookAdd(dm,DMCoarsenHook_DMKSP,NULL,NULL,NULL,NULL,NULL);CHKERRQ(ierr);
    ierr                 = DMRefineHookAdd(dm,DMRefineHook_DMKSP,NULL,NULL,NULL,NULL,NULL);CHKERRQ(ierr);
  }
  PetscFunctionReturn(0);
}
...
PetscErrorCode DMCopyDMKSP(DM dmsrc,DM dmdest)
{
  ...
  ierr          = DMCoarsenHookAdd(dmdest,DMCoarsenHook_DMKSP,NULL,NULL,NULL,NULL,NULL);CHKERRQ(ierr);
  ierr          = DMRefineHookAdd(dmdest,DMRefineHook_DMKSP,NULL,NULL,NULL,NULL,NULL);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
