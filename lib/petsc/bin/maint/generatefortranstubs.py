...
def GetManSec(petscdir,dir,verbose):
  import re
  ...
  if verbose:
    print('Found mansec '+dir+' '+mansec+' '+submansec)
  return mansec,submansec,bfortsubmansec

def FixDirInterface(petscdir,dir,mansec,submansec,verbose):
  ...
  # save Fortran interface files generated (they merged with others in a post-processing step)
  for filename in [f for f in os.listdir(dir) if re.match(r'f90module[0-9]+.f90', f)]:
    modfile = os.path.join(dir, filename)
    if os.path.exists(modfile):
      if verbose: print('Saving F90 interface for '+modfile)
      fd = open(modfile)
      txt = fd.read()
      fd.close()
      if txt:
        if not os.path.isdir(os.path.join(petscdir,'ftn-auto',mansec)): os.mkdir(os.path.join(petscdir,'ftn-auto',mansec))
        if not os.path.isdir(os.path.join(petscdir,'ftn-auto',mansec,submansec+'-tmpdir')): os.mkdir(os.path.join(petscdir,'ftn-auto',mansec,submansec+'-tmpdir'))
        fname =  os.path.join(petscdir,'ftn-auto',mansec,submansec+'-tmpdir',submansec + '.h90')
...
def processDir(petscdir, bfort, verbose, dirpath, dirnames, filenames):
  ...
  if verbose: print('Processing '+dirpath)
  mansec,submansec,bfortsubmansec = GetManSec(petscdir,dirpath,verbose)
  outdir = os.path.join(petscdir,'ftn-auto',mansec)
  ...
  if newls:
    ...
    for i in range(0, len(newls), split_ct):
      cmd = 'BFORT_CONFIG_PATH='+os.path.join(petscdir,'lib','petsc','conf')+' '+bfort+' '+' '.join(options+newls[i:i+split_ct])+' -f90modfile '+ os.path.join(outdir,'f90module'+str(i)+'.f90')
    ...
    FixDirInterface(petscdir,outdir,mansec,submansec,verbose)

  # remove from list of subdirectories all directories without source code
  dirnames[:] = [name for name in dirnames
                 if name not in ['output','ftn-custom','config','f90-custom','ftn-kernels','petsc-doc-env','systems','ftn-auto',
                 'tests','tutorials','yaml']
...
def processf90interfaces(petscdir,verbose):
  ...
  badSrc = []
  for msfd in mansecs:
    dir = os.path.join(petscdir,'ftn-auto',msfd)
    for f in os.listdir(dir):
        ext = os.path.splitext(f)[1]
        if ext == '.c' or ext == '.cxx' or ext == '.cu':
          FixFile(os.path.join(dir, f))

    badSrc = checkHandWrittenF90Interfaces(badSrc, os.path.join(petscdir,'src',msfd,'f90-mod'))
    ptypes = updatePetscTypesFromMansec(ptypes,os.path.join(petscdir,'src',msfd,'f90-mod'))
  for src in badSrc:
    print('Importing entire package: "'+src[0]+'" line '+str(src[1])+' file '+src[2])
  if len(badSrc): raise RuntimeError
  for ms in mansecs:
     if verbose: print('Processing interfaces for '+ms)
     msfd = os.path.join(petscdir,'src',ms,'f90-mod')
     msfad = os.path.join(petscdir,'ftn-auto',ms)
     for submansec in os.listdir(msfad):
      if submansec == 'f90-mod': continue
      if verbose: print('  for '+submansec)
      if os.path.isdir(os.path.join(msfad,submansec)):
        submansec = submansec[:-7]
        if not os.path.isdir(os.path.join(msfad,'f90-mod')): os.mkdir(os.path.join(msfad,'f90-mod'));
        f90inc = os.path.join(msfad,'f90-mod','petsc'+submansec+'.h90')
...
#     outbuf +=   cppflags + '\n'
     ...
     ff = open(os.path.join(petscdir,'ftn-auto',ms,'makefile'), 'w')
     ff.write(outbuf)
     ff.close()
