...
def processDir(petscdir, bfort, verbose, dirpath, dirnames, filenames):
  ...
  newls2 = []
  for f in filenames:
    if f.endswith(('.hip.cxx','.sycl.cxx','.kokkos.cxx')): # separate out files with two extensions to not confuse bfort
      newls2.append(f)
    elif f.endswith(('.c','.h','.cxx','.cu')):
      newls1.append(f)
  ...
  if newls1 or newls2:
    ...
    split_ct = 10
    j = 0
    for i in range(0, len(newls1), split_ct): # process newls1 in batches, using bfort's default scheme: output to xf.c for input x.c
      cmd = 'BFORT_CONFIG_PATH='+os.path.join(petscdir,'lib','petsc','conf')+' '+bfort+' '+' '.join(options+newls1[i:i+split_ct])+' -f90modfile f90module'+str(i)+'.f90'
      j = i + 1
      try:
    ...
    for ifile in newls2: # process newls2 one at a time with given output file names
      ofile = ifile.split('.',1)[0] + 'f.c' # ex. output to xf.c for input x.hip.cxx
      cmd = 'BFORT_CONFIG_PATH='+os.path.join(petscdir,'lib','petsc','conf')+' '+bfort+' '+' '.join(options+[ifile])+' -o '+ofile+' -f90modfile f90module'+str(j)+'.f90'
      j += 1
      try:
        output = check_output(cmd, cwd=dirpath, shell=True, stderr=subprocess.STDOUT)
      except subprocess.CalledProcessError as e:
