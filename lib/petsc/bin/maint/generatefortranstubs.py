#!/usr/bin/env python3
#
#    Generates Fortran function stubs and module interface definitions for PETSc using the Sowing bfort program
#
...
##########  $PETSC_ARCH/src/MANSEC/f90-mod/ftn-auto-interfaces/*.hf90
  ...
  # generate all the polymorphic Fortran subroutines needed for routines with optional arguments
  for i in classes.keys():
    if i in ['PetscIntStack']: continue
    for j in classes[i].functions: # loop over functions in class
      # check for functions for which we cannot build intefaces
        ...
        for fi in opts:
          ...
          for k in classes[i].functions[j].arguments:
            ...
            if fi[cnt] == 'a':
              ...
              if ktypename == 'PetscReal':
                fd.write('PETSC_NULL_REAL')
              elif ktypename == 'PetscInt':
                fd.write('PETSC_NULL_INTEGER')
            cnt = cnt + 1
          fd.write(',z)\n')
          fd.write('  end subroutine\n')
