...
class Configure(config.base.Configure):
  def __init__(self, framework):
    ...
    self.cxxRestrict      = ' '
    self.c99flag          = None
    self.rlpathflags      = [] # any library found with -L or -rpath that is automatically generated by compilers (C, Fortran) when used to link
                               # this is shared by all compilers so that multiple compilers do not create redundant library entries
    return
  ...
  def checkCLibraries(self):
    ...
    try:
      while 1:
        ...
        if arg == '-rpath' or arg == '-R':
          ...
          if not lib in self.rlpathflags:
            rpathflags.append(lib)
            self.rlpathflags.append(lib)
            self.logPrint('Found '+arg+' library: '+lib, 4, 'compilers')
            clibs.append(self.setCompilers.CSharedLinkerFlag+lib)
          else:
            self.logPrint('Already in rlpathflags, skipping'+arg+' '+lib, 4, 'compilers')
          continue
    ...
    self.clibs = []
    if skipclibraries == 1: return  # only use clibrary information if it is needed by Fortran or C++
    for lib in clibs:
...
Otherwise you need a different combination of C, C++, and Fortran compilers")
  ...
  def checkFortranLibraries(self):
    ...
    fmainlibs = []
    lflags  = []     # -l or -L found
    rpathflags = []  # library found with -rpath
    try:
      while 1:
        arg = next(argIter)
        self.logPrint( 'Checking arg '+arg, 4, 'compilers')
...
#            # check for Nag Fortran library that must be handled as static because shared version does not have all the symbols
        ...
        if m:
          ...
          if not arg in lflags:
            if not lib in self.rlpathflags:
              self.rlpathflags.append(lib)
              self.logPrint('Adding '+lib+' to rlpathflags', 4, 'compilers')
            lflags.append(arg)
            self.logPrint('Found library directory: '+arg, 4, 'compilers')
            flibs.append(arg)
          else:
        ...
        if arg == '-rpath' or arg == '-R':
          ...
          if not lib in self.rlpathflags:
            ...
            self.logPrint('Already in rlpathflags so skipping: '+arg+' '+lib, 4, 'compilers')
          continue
        # Check for '-R/sharedlibpath/'
        m = re.match(r'^-R.*$', arg)
        if m:
    ...
    self.flibs = []
    for lib in flibs:
# since the sharedlinkerflag version is automatically added when compiling we don't need to add one here that ends up being a duplicate in the link line
