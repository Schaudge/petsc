...
class BaseSet(object):
    ...
    def __deepcopy__(self, memo):
        ...
        memo[id(self)] = result
        data = result._data
        for e,elt in enumerate(self):
            data[deepcopy(elt, memo)] = e
        return result
    ...
    def difference(self, other):
        ...
        try:
            ...
            otherdata = Set(other)._data
        for e, elt in enumerate(ifilterfalse(otherdata.has_key, self)):
            data[elt] = e
        return result
...
class Set(BaseSet):
    ...
    def pop(self):
        ...
        raise RuntimeError('This will not work with our current scheme. We must renumber when this happens')
...
if __name__ == '__main__':
    ...
    class SetsTest(unittest.TestCase):
        def testOrdering(self):
            '''Verify that sets created in different orders are equivalent'''
            self.assertEquals(Set([1, 2, 3]), Set([3, 2, 1]))
            self.assertEquals(Set([1, 2, 3]), Set([2, 3, 1]))
            return

        def testUniqueness(self):
            '''Verify that sets created with repeated elements are equivalent'''
            self.assertEquals(Set([1, 2, 3]), Set([3, 2, 1, 2]))
            self.assertEquals(Set([1, 2, 3]), Set([2, 3, 3, 3, 1, 2, 3, 1]))
            return
