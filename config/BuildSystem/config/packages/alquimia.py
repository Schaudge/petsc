  ...
  def postProcess(self):
    ...
    if not self.argDB['with-batch']:
      try:
        ...
        output,err,ret  = config.package.CMakePackage.executeShellCommand('cd '+os.path.join(self.packageDir,'petsc-build')+' && '+self.make.make+' test_install',timeout=50, log = self.log)
        output = output+err
