...
class Mistakes(object):
    ...
    def compareSourceLists(self, root, msources, files):
        ...
        ssources  = set(f for f in files if getlangext(f) in ['.c', '.kokkos.cxx','.cxx', '.cc', '.cu', '.cpp', '.F', '.F90', '.hip.cxx', '.sycl.cxx', 'raja.cxx'])
...
LANGS = dict(kokkos_cxx='KOKKOS', c='C', cxx='CXX', cpp='CPP', cu='CU', F='F',
             F90='F90', hip_cxx='HIP', sycl_cxx='SYCL', raja_cxx='RAJA')
