  ...
  def buildFortranBinding(self):
    if hasattr(self.compilers, 'FC') and self.framework.argDB['with-fortran-bindings']:
      ...
      try:
        ...
        del sys.path[0]
        if 'with-fortran-bindings-inplace' in self.framework.argDB and self.framework.argDB['with-fortran-bindings-inplace']:
          arch = ''
        else:
          arch = self.arch.arch
        generatefortranstubs.main(self.petscdir.dir, arch)
