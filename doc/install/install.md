...
[OpenMPI] defaults to building shared libraries for MPI. However, the binaries generated
...
For development on macOS we recommend installing **both** the Apple Xcode GUI development system (install from the Apple macOS store) and the Xcode Command Line tools [^id6] install with

```console
$ xcode-select --install
```

The Apple compilers are `clang` and `clang++` [^id7]. Apple also provides `/usr/bin/gcc`, which is, confusingly, a wrapper to the `clang` compiler, not the GNU compiler.
...
On Linux - make sure you have compatible [NVIDIA driver](https://developer.nvidia.com/cuda-downloads) installed.
...
The rule for linking may be found in [lib/petsc/conf/rules](PETSC_DOC_OUT_ROOT_PLACEHOLDER/lib/petsc/conf/rules)
...
[^id6]: The two packages provide slightly different (though largely overlapping) functionality which can only be fully used if both packages are installed.

[^id7]: Apple provides customized `clang` and `clang++` for its system. To use the unmodified LLVM project `clang` and `clang++`
